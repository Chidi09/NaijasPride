...
final animeDetailProvider =
    FutureProvider.family<AnimeDetail, int>((ref, id) {
  return ref.watch(animeApiProvider).detail(id);
});

final animeEpisodesProvider =
    FutureProvider.family<List<AnimeEpisode>, int>((ref, id) {
...
class _AnimeDetailScreenState extends ConsumerState<AnimeDetailScreen> {
  Future<void> _onEpisodeTap(
    ...
    final nextEpisode =
        index >= 0 && index + 1 < episodes.length ? episodes[index + 1] : null;
      ...
      Navigator.of(context).push(
        MaterialPageRoute(
          builder: (_) => UnifiedVideoPlayerScreen(
            ...
            nextEpisodeLabel:
                nextEpisode != null ? 'Episode ${nextEpisode.number}' : null,
    ...
    try {
      final result = await ref.read(animeApiProvider).watch(
            widget.id,
            episode.number,
          );
      final skipTimes = await ref.read(animeApiProvider).skipTimes(widget.id, episode.number);
      ...
      if (source is UnresolvedPlaybackSource) {
        ...
        if (embedSources.isEmpty) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(source.reason)),
          );
          return;
        }
  ...
  Widget build(BuildContext context) {
    ...
    return detailAsync.when(
      ...
      data: (detail) {
        final title = detail.title.english ??
            detail.title.romaji ??
            detail.title.native ??
            'Untitled';
                            ...
                            return Column(
                              ...
                              children: [
                                    ...
                                    PressableScale(
                                      ...
                                      child: IconButton(
                                        ...
                                        onPressed: () async {
                                          ...
                                          final existing = await api.getAnimeEpisodeProgress(widget.id, ep.number);
                                ...
                                ...episodes.map(
                                    (ep) => _EpisodeTile(
                                      episode: ep,
                                      onTap: () => _onEpisodeTap(ep, episodes),
                                    ),
                                  ),
                                const SizedBox(height: 24),
                              ],
