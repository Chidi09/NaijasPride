    ...
    @if (auth.currentUser()) {
      <div class="relative" (clickOutside)="closeDropdown()">
        ...
        >
          ...
          @if (unreadCount() > 0) {
            <span class="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 rounded-full bg-red-500 text-white text-[10px] font-bold flex items-center justify-center leading-none">
        ...
        @if (dropdownOpen()) {
          ...
          >
            ...
            <div class="flex items-center justify-between px-4 py-3 border-b border-black/10 dark:border-white/10">
              <span class="text-sm font-semibold text-black dark:text-white">Notifications</span>
              @if (unreadCount() > 0) {
                <button
                  type="button"
                  (click)="markAllRead()"
            ...
            <div class="max-h-96 overflow-y-auto">
              @if (loading()) {
                <div class="px-4 py-6 text-center">
                  <span class="w-5 h-5 border-2 border-black/20 dark:border-white/20 border-t-black dark:border-t-white rounded-full animate-spin inline-block"></span>
                </div>
              } @else if (notifications().length === 0) {
                <div class="px-4 py-8 text-center text-sm text-black/50 dark:text-white/50">
                ...
                @for (notif of notifications(); track notif.id) {
                  <div
                    ...
                    [ngClass]="{ 'bg-blue-50/60 dark:bg-blue-950/30': !notif.read }"
                  ...
                  >
                    <div class="flex items-start gap-3">
                      ...
                      <div class="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center mt-0.5"
                        [ngClass]="{
                          'bg-green-100 dark:bg-green-900/40': notif.type === 'DOWNLOAD_READY',
                          'bg-blue-100 dark:bg-blue-900/40': notif.type === 'COMMENT_REPLY',
                          'bg-purple-100 dark:bg-purple-900/40': notif.type === 'COMMENT_MENTION'
                        }"
                      >
                      ...
                      <div class="flex-1 min-w-0">
                        <p class="text-sm font-semibold text-black dark:text-white leading-tight">{{ notif.title }}</p>
                        <p class="text-xs text-black/60 dark:text-white/60 mt-0.5 line-clamp-2">{{ notif.body }}</p>
                        <p class="text-[10px] text-black/40 dark:text-white/40 mt-1">{{ timeAgo(notif.createdAt) }}</p>
                      </div>
            ...
            <!-- Footer -->
            <div class="px-4 py-2 border-t border-black/10 dark:border-white/10 text-center">
              <a routerLink="/profile" (click)="closeDropdown()" class="text-xs text-black/50 dark:text-white/50 hover:text-black dark:hover:text-white transition-colors">
                View all in profile
              </a>
            </div>
          </div>
...
export class NotificationBellComponent implements OnInit, OnDestroy {
  ...
  private fetchNotifications() {
    ...
    this.http.get<{ data: AppNotification[] }>('/api/v1/notifications?limit=20').subscribe({
      next: (res) => {
