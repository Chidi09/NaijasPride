...
interface CommentUser { id: string; username?: string; email: string; }
interface Comment {
  id: string;
  body: string;
  createdAt: string;
...
@Component({
  ...
  styles: [`
    .mention { color: #800020; font-weight: 600; }
  ...
  template: `
    <section class="mt-10 border-t border-[#d9c4b7] dark:border-white/10 pt-8">
      <h3 class="text-xl font-serif font-bold text-[#24181b] dark:text-white mb-6">
        Discussion
        @if (total() > 0) {
          <span class="text-sm font-normal text-[#9f7d73] dark:text-gray-400 ml-2">({{ total() }})</span>
        }
      </h3>
      ...
      @if (auth.currentUser()) {
        ...
        <div class="mb-8 rounded-lg border border-[#d9c4b7] dark:border-white/10 px-4 py-4 text-sm text-[#725f58] dark:text-gray-400 text-center">
          <a href="/login" class="text-[#800020] hover:underline font-semibold">Sign in</a> to join the discussion.
        </div>
      }
        ...
        <div class="space-y-4">
          @for (i of [1,2,3]; track i) {
            <div class="animate-pulse flex gap-3">
              <div class="w-9 h-9 rounded-full bg-[#e5d2c6] dark:bg-white/10 flex-shrink-0"></div>
              <div class="flex-1 space-y-2">
                <div class="h-3 w-24 bg-[#e5d2c6] dark:bg-white/10 rounded"></div>
                <div class="h-3 w-full bg-[#e5d2c6] dark:bg-white/10 rounded"></div>
                <div class="h-3 w-3/4 bg-[#e5d2c6] dark:bg-white/10 rounded"></div>
              </div>
            </div>
          }
        </div>
      } @else if (comments().length === 0) {
        <p class="text-center text-[#9f7d73] dark:text-gray-500 text-sm py-8">No comments yet. Be the first!</p>
      } @else {
        <div class="space-y-6">
          @for (comment of comments(); track comment.id) {
            <div class="flex gap-3">
              <!-- Avatar -->
              <div class="flex-shrink-0 w-9 h-9 rounded-full bg-[#dfc8bb] dark:bg-[#2a2a2a] flex items-center justify-center text-xs font-bold text-[#6b3d2e] dark:text-white/70 uppercase">
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-baseline gap-2 flex-wrap">
                  <span class="text-sm font-semibold text-[#24181b] dark:text-white">{{ displayName(comment.user) }}</span>
                  <span class="text-xs text-[#9f7d73] dark:text-gray-500">{{ timeAgo(comment.createdAt) }}</span>
                </div>

                <!-- Body -->
                <p class="comment-body text-sm text-[#3a2a25] dark:text-gray-300 mt-1 leading-relaxed" [innerHTML]="renderBody(comment.body)"></p>
                ...
                <div class="flex items-center gap-4 mt-2">
                  ...
                  @if ((comment._count?.replies ?? 0) > 0) {
                    <button
                      ...
                      class="text-xs text-[#9f7d73] dark:text-gray-500 hover:text-[#800020] dark:hover:text-[#800020] transition-colors"
                    >
                      {{ comment.showReplies ? 'Hide' : 'View' }} {{ comment._count?.replies }} {{ (comment._count?.replies ?? 0) === 1 ? 'reply' : 'replies' }}
                    </button>
                  }
                ...
                @if (comment.showReplies) {
                  <div class="mt-4 pl-4 border-l-2 border-[#d9c4b7] dark:border-white/10 space-y-4">
                    @if (comment.repliesLoading) {
                      <div class="text-xs text-[#9f7d73] dark:text-gray-500 animate-pulse">Loading replies…</div>
                    } @else {
                      @for (reply of comment.replies || []; track reply.id) {
                        <div class="flex gap-2.5">
                          <div class="flex-shrink-0 w-7 h-7 rounded-full bg-[#dfc8bb] dark:bg-[#2a2a2a] flex items-center justify-center text-[10px] font-bold text-[#6b3d2e] dark:text-white/70 uppercase">
                            {{ userInitial(reply.user) }}
                          </div>
                          <div class="flex-1">
                            <div class="flex items-baseline gap-2 flex-wrap">
                              <span class="text-xs font-semibold text-[#24181b] dark:text-white">{{ displayName(reply.user) }}</span>
                              <span class="text-[10px] text-[#9f7d73] dark:text-gray-500">{{ timeAgo(reply.createdAt) }}</span>
                            </div>
                            <p class="comment-body text-xs text-[#3a2a25] dark:text-gray-300 mt-0.5 leading-relaxed" [innerHTML]="renderBody(reply.body)"></p>
                            <div class="flex items-center gap-3 mt-1.5">
                              ...
                              >Reply</button>
                              @if (canDelete(reply)) {
                                <button type="button" (click)="deleteComment(reply)" class="text-[10px] text-red-400 hover:text-red-600 transition-colors">Delete</button>
                              }
                            </div>
                          </div>
                        </div>
...
export class CommentSectionComponent implements OnInit {
  ...
  private loadComments(page: number, append: boolean) {
    ...
    this.http.get<{ data: Comment[]; meta: { total: number } }>(
      ...
      next: (res) => {
        ...
        this.comments.update((list) =>
          list.map((c) => {
            ...
            return { ...c, replies, showReplies: true, _count: { replies: count } };
          })
        );
        this.replyText = '';
  ...
  toggleReplies(comment: Comment) {
    ...
    if (current.showReplies) {
      this.comments.update((list) => list.map((c) => c.id === comment.id ? { ...c, showReplies: false } : c));
      return;
    }

    if (current.replies?.length) {
      this.comments.update((list) => list.map((c) => c.id === comment.id ? { ...c, showReplies: true } : c));
      return;
    }

    this.comments.update((list) =>
      list.map((c) => c.id === comment.id ? { ...c, repliesLoading: true, showReplies: true } : c)
    );
    this.http.get<{ data: Comment[] }>(`/api/v1/comments/${comment.id}/replies`).subscribe({
      next: (res) => {
        this.comments.update((list) =>
          list.map((c) => c.id === comment.id ? { ...c, replies: res.data, repliesLoading: false } : c)
        );
      },
      error: () => {
        this.comments.update((list) =>
          list.map((c) => c.id === comment.id ? { ...c, repliesLoading: false, showReplies: false } : c)
        );
      },
    });
  }
  ...
  canDelete(comment: Comment): boolean {
    ...
    return (user as any).id === comment.userId || (user as any).role === 'ADMIN';
  }
