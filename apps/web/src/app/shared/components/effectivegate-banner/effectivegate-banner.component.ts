import { Component, ElementRef, OnDestroy, PLATFORM_ID, computed, effect, inject } from '@angular/core';
  ...
  template: `
    @if (shouldShowBanner()) {
      <div class="my-4 flex justify-center">
        <div class="relative">
          <span class="pointer-events-none absolute -top-4 left-0 text-[10px] uppercase tracking-widest text-gray-600 dark:text-gray-500">
            Advertisement
          </span>
          <div id="effectivegate-banner-wrapper" class="min-h-[50px] min-w-[320px]"></div>
        </div>
      </div>
    }
  `,
...
export class EffectivegateBannerComponent implements OnDestroy {
  ...
  private injectBanner(): void {
    ...
    const wrapper = this.el.nativeElement.querySelector('#effectivegate-banner-wrapper') as HTMLElement | null;
    if (!wrapper) return;
    ...
    this.adScriptService.renderEffectiveGateSlot(wrapper, {
      ...
      height: 50,
      params: {},
      invokeScriptUrl: '//www.effectivegatecpm.com/efea465d4fdc9eabe553fbaecd0c4948/invoke.js',
    });
