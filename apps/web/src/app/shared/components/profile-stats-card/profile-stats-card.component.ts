  ...
  template: `
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
      @for (stat of stats(); track stat.label) {
        <div class="bg-[#f1e5dd] dark:bg-zinc-800/60 border border-[#dcc5b8] dark:border-zinc-700/40 rounded-xl p-4 text-center group transition-colors">
          <div class="w-10 h-10 mx-auto mb-2 rounded-full flex items-center justify-center"
            [style.background]="stat.accent ? stat.accent + '1a' : 'rgba(128,0,32,0.1)'"
          >
            <svg class="w-5 h-5" [style.color]="stat.accent || '#800020'" viewBox="0 0 24 24" fill="currentColor">
              <path [attr.d]="getIconPath(stat.icon)" />
            </svg>
          </div>
          <p class="text-2xl font-bold text-[#24181b] dark:text-white">{{ stat.value }}</p>
          <p class="text-xs text-[#8a756e] dark:text-gray-500 mt-0.5">{{ stat.label }}</p>
        </div>
      }
    </div>
  `,
