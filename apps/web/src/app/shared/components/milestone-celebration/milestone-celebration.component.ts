  ...
  template: `
    @if (milestones.activeCelebration(); as m) {
      ...
      >
        ...
        <div class="absolute inset-0 overflow-hidden pointer-events-none">
          @for (i of particles; track i) {
            <div
              ...
              [style.animation]="'confetti-fall ' + (2 + i * 0.3) + 's ease-out ' + (i * 0.1) + 's forwards'"
            ></div>
          }
        </div>
        ...
        >
          <div class="w-16 h-16 mx-auto mb-4 rounded-full bg-gradient-to-br from-cinema-500 to-amber-500 flex items-center justify-center">
          ...
          <div class="mt-4 h-1 bg-zinc-800 rounded-full overflow-hidden">
            <div class="h-full bg-gradient-to-r from-cinema-500 to-amber-500 rounded-full" style="animation: milestone-progress 5s linear forwards"></div>
          </div>
        </div>
      </div>
    }
