...
@Component({
  ...
  template: `
    <div class="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
      <section
        class="rounded-xl border border-[#dcc5b8] bg-[#fffdf8] dark:border-[#2d1a21] dark:bg-[#140d11]"
      >
        <div class="border-b border-[#dcc5b8] p-6 dark:border-[#2d1a21]">
          ...
          <p class="mt-1 text-sm text-[#7b6660] dark:text-[#9f7d73]">
            Upload EPUB/PDF files, then publish metadata to the library.
          </p>
        </div>

        <div class="space-y-5 p-6">
          <div
            class="rounded-lg border border-[#d6b87a]/40 bg-[#fff7f0] p-4 dark:border-[#5f1327]/40 dark:bg-[#1b1014]"
          >
            <label
              class="mb-2 block text-xs uppercase tracking-[0.2em] text-[#7b6660] dark:text-[#9f7d73]"
              >File Upload</label
            >
            <input
              type="file"
          ...
          <div class="grid gap-4 sm:grid-cols-2">
            <div>
              <label
                class="mb-1 block text-xs text-[#7b6660] dark:text-[#9f7d73]"
                >Type</label
              >
              <select
                [(ngModel)]="kind"
          ...
          <div>
            <label class="mb-1 block text-xs text-[#7b6660] dark:text-[#9f7d73]"
      ...
      >
        <div
          class="border-b border-[#dcc5b8] p-6 flex items-center justify-between dark:border-[#2d1a21]"
        >
          ...
          <button
            type="button"
            (click)="loadBooks()"
            class="text-xs text-[#5f1327] hover:text-[#24181b] dark:text-[#d6b87a] dark:hover:text-white"
          >
            Refresh
          </button>
        </div>
          ...
          @if (isLoadingBooks()) {
            ...
            <div class="space-y-3">
              @for (book of books(); track book.id) {
                <article
                  class="rounded-lg border border-[#dcc5b8] bg-[#fff7f0] p-3 dark:border-[#2d1a21] dark:bg-[#1b1014]"
                >
                  <p
                    class="text-sm font-semibold text-[#24181b] dark:text-white line-clamp-2"
                  >
                    {{ book.title }}
                  </p>
                  <p class="mt-1 text-xs text-[#7b6660] dark:text-[#b59c95]">
                    {{ book.author }} • {{ book.year }}
                  </p>
                  <p
                    class="mt-1 text-[11px] text-[#8a756e] dark:text-[#9f7d73] line-clamp-1"
                  >
                    {{ book.genre.join(", ") }}
                  </p>
                </article>
...
export class AdminBooksComponent {
  ...
  loadBooks() {
    ...
    this.http
      .get<{ status: string; data: Book[] }>("/api/v1/books?page=1&limit=25")
      .subscribe({
        next: (response) => {
      ...
      .subscribe({
        next: async (response) => {
          try {
            ...
            this.fileUploadMessage.set(
              "File uploaded successfully. You can now publish this entry.",
            );
