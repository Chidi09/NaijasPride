...
import { Genre, Movie } from '@naijaspride/types';
    ...
    <div class="max-w-4xl mx-auto space-y-6">
      <div class="bg-[#140d11] border border-[#2d1a21] rounded-xl shadow-2xl overflow-hidden">
        ...
        <div class="p-6 space-y-8">
          ...
          <div class="space-y-4">
            <h3 class="text-lg font-semibold text-[#d6b87a] flex items-center gap-2">
              <span class="w-7 h-7 rounded-full bg-cinema-500 text-white flex items-center justify-center text-xs">1</span>
              Select Video File
            </h3>
            ...
            >
              @if (!selectedFile()) {
                <div class="space-y-4">
                  ...
                  <div>
                    <label class="cursor-pointer bg-cinema-500 hover:bg-cinema-400 text-white px-6 py-2 rounded-lg font-medium inline-block transition-colors">
                      Choose Video File
          ...
          @if (selectedFile()) {
            <div class="space-y-6">
              <h3 class="text-lg font-semibold text-[#d6b87a] flex items-center gap-2">
                <span class="w-7 h-7 rounded-full bg-cinema-500 text-white flex items-center justify-center text-xs">2</span>
                Content Details
              </h3>

              <form [formGroup]="form" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2 md:col-span-2">
                  <label class="block text-sm font-medium text-[#c5aea5]">Movie Title</label>
                  <input formControlName="title" type="text" class="input-field" placeholder="Full title of the movie">
                </div>

                <div class="space-y-2">
                  <label class="block text-sm font-medium text-[#c5aea5]">Release Year</label>
                  <input formControlName="year" type="number" class="input-field">
                </div>

                <div class="space-y-2">
                  <label class="block text-sm font-medium text-[#c5aea5]">Genres (Comma separated)</label>
                  <input formControlName="genre" type="text" class="input-field" placeholder="Drama, Action, Nollywood">
                </div>
...
export class AdminMovieUploadComponent {
  ...
  async startUpload() {
    ...
    try {
      // 1. Get signed URL
      const urlRes = await this.adminService.getUploadUrl({
        fileName: file.name,
  ...
  private async finalizeMovie(storageKey: string, fileSize: number, contentType: string) {
    const raw = this.form.getRawValue();
    
    try {
      const res = await this.adminService.adminCreateMovie({
        title: raw.title!,
