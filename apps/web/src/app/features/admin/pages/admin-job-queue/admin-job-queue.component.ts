  ...
  template: `
    <div class="p-8">
      <div class="flex items-center justify-between mb-8">
        <div>
          <h1 class="text-3xl font-bold text-[#24181b] dark:text-white">
            Job Queue Status
          </h1>
          <p class="text-[#7b6660] dark:text-gray-400 mt-1">
      ...
      @if (queues.length > 0) {
          ...
          @for (queue of queues; track queue.name) {
            ...
            >
              <div class="flex items-center justify-between mb-4">
                <h3
                  class="font-semibold text-[#24181b] dark:text-white capitalize"
                >
                  {{ queue.name }}
                </h3>
                <span
              ...
              <div class="grid grid-cols-2 gap-4 mb-4">
                ...
                >
                  ...
                  <div class="text-xs text-[#7b6660] dark:text-gray-400">
                    Waiting
                  </div>
                </div>
                <div
                ...
                <div
                  class="text-center p-3 bg-gray-50 dark:bg-gray-900/20 rounded-lg"
                >
                  <div
        ...
        <div
          class="text-center py-12 bg-white dark:bg-cinema-800 rounded-xl border border-[#dcc5b8] dark:border-gray-700"
        >
          <div class="mb-4">
          ...
          <h3 class="text-lg font-medium text-[#24181b] dark:text-white mb-2">
            No queues configured
          </h3>
          <p class="text-[#7b6660] dark:text-gray-400">
            Redis is not configured or no queues are active.
          </p>
        </div>
      }
      ...
      @if (selectedQueue) {
        <div
          class="bg-white dark:bg-cinema-800 rounded-xl border border-[#dcc5b8] dark:border-gray-700 overflow-hidden"
        >
          <div
            class="p-4 border-b border-[#dcc5b8] dark:border-gray-700 flex items-center justify-between"
          >
            <h2 class="font-semibold text-[#24181b] dark:text-white">
          ...
          @if (jobsLoading) {
            <div class="p-8 text-center">
              <span
                class="animate-spin h-8 w-8 border-4 border-[#800020] border-t-transparent rounded-full inline-block"
              ></span>
            </div>
            ...
            <div class="overflow-x-auto">
              <table class="w-full text-sm">
                <thead class="bg-[#f1e5dd] dark:bg-black/20">
                  <tr>
                    <th class="p-4 text-left text-[#2a1c1f] dark:text-gray-200">
                      Job ID
                    </th>
                    <th class="p-4 text-left text-[#2a1c1f] dark:text-gray-200">
                      Name
                ...
                <tbody class="divide-y divide-[#dcc5b8] dark:divide-gray-700">
                  @for (job of jobs; track job.id) {
                    <tr
                      class="hover:bg-[#f4e4da] dark:hover:bg-white/5 transition-colors"
                    >
                      <td
                        class="p-4 text-[#24181b] dark:text-white font-mono text-xs"
                      >
                        {{ job.id }}
                      </td>
                      <td class="p-4 text-[#24181b] dark:text-white">
                        {{ job.name }}
                      </td>
                      <td class="p-4">
                        @if (job.progress > 0) {
                          ...
                          <span
                            class="text-xs text-[#7b6660] dark:text-gray-400"
                            >{{ job.progress }}%</span
                          >
                        } @else {
                          <span
                      ...
                      <td class="p-4 text-[#7b6660] dark:text-gray-400">
                        {{ job.timestamp | date: "short" }}
                      </td>
                      <td class="p-4">
                        <button
...
export class AdminJobQueueComponent implements OnInit, OnDestroy {
  ...
  refreshQueues() {
    ...
    this.http
      .get<{ success: boolean; data: QueueStats[]; error?: string }>(
        "/api/v1/admin/queues",
      )
      .pipe(takeUntil(this.destroy$))
      .subscribe({
  ...
  loadJobs() {
    ...
    this.http
      .get<{ success: boolean; data: QueueJob[] }>(
        `/api/v1/admin/queues/${this.selectedQueue}/jobs?status=${this.selectedStatus}`,
      )
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (response) => {
  ...
  removeJob(jobId: string) {
    ...
    this.http
      .delete(`/api/v1/admin/queues/${this.selectedQueue}/jobs/${jobId}`)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: () => {
          this.loadJobs();
