...
@Component({
  ...
  template: `
    <div class="max-w-4xl mx-auto space-y-6">
      ...
      >
        <div
          class="p-6 border-b border-[#dcc5b8] flex justify-between items-center bg-yellow-100/70 dark:bg-yellow-900/5 dark:border-[#2d1a21]"
        >
          <div>
            <h2 class="text-xl font-bold text-[#24181b] dark:text-white">
              Manual Movie Registration
            </h2>
            <p class="text-xs text-[#7b6660] mt-1 dark:text-[#9f7d73]">
              Register external video links or YouTube embeds.
            </p>
          </div>
          <a
            routerLink="/admin/movies/upload"
            class="px-4 py-2 bg-cinema-500 text-white text-xs font-bold rounded-lg hover:bg-cinema-400"
            >Switch to File Upload</a
          >
        </div>

        <div
          class="px-6 py-4 bg-yellow-100/80 border-b border-[#dcc5b8] flex items-start gap-4 dark:bg-yellow-900/10 dark:border-[#2d1a21]"
        >
          <svg
          ...
          <p
            class="text-xs text-yellow-900/80 leading-relaxed dark:text-yellow-200/70"
          >
            <strong>Use this only for External Links:</strong> If you have a
            video file (MP4, MKV) on your computer, please use the
            <strong class="text-[#24181b] dark:text-white">Upload Movie</strong>
            section instead. This form is for movies already hosted elsewhere.
          </p>
        </div>
        ...
        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="p-6 space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-2">
              <label class="block text-sm font-medium text-[#d6b87a]"
                >Title</label
              >
              <input
          ...
          <div class="space-y-2">
            ...
            <div class="flex flex-wrap gap-3">
              @for (g of genres; track g) {
                <label
                  class="inline-flex items-center space-x-2 cursor-pointer bg-[#fff7f0] px-3 py-1 rounded-full border border-[#d6b87a]/40 hover:bg-[#f6e4d7] dark:bg-[#1b1014] dark:border-[#5f1327] dark:hover:bg-[#26141a]"
                >
                  <input
                    type="checkbox"
                    [value]="g"
                    (change)="onCheckboxChange($event, 'genre')"
                    class="text-cinema-500 rounded focus:ring-cinema-500"
                  />
              ...
              @for (q of qualities; track q) {
                ...
                >
                  <input
                    type="checkbox"
                    [value]="q"
                    (change)="onCheckboxChange($event, 'quality')"
                    class="text-cinema-500 rounded focus:ring-cinema-500"
                  />
          ...
          <div
            class="p-4 bg-[#fff7f0] rounded-lg space-y-4 border border-[#dcc5b8] dark:bg-[#1b1014] dark:border-[#2d1a21]"
          >
            <h3 class="font-bold text-[#24181b] dark:text-white">
              Download Links
            </h3>
            <div formGroupName="fileUrls" class="grid grid-cols-1 gap-4">
              @for (q of selectedQualities; track q) {
                <div>
                  <label
                    class="block text-xs font-bold text-[#9f7d73] uppercase mb-1"
                    >{{ q }} URL</label
                  >
                  <input
                    [formControlName]="q"
              ...
              @if (selectedQualities.length === 0) {
                <p class="text-sm text-[#9f7d73] italic">
                  Select qualities above to add links.
                </p>
              }
            </div>
          </div>
          ...
          >
            ...
            <div class="space-y-2">
              ...
              <p class="text-xs text-[#9f7d73]">
                Enter the YouTube video ID for streaming (optional)
              </p>
            </div>
            ...
            <div class="flex items-center gap-2">
              <input
                ...
                class="text-cinema-500 rounded focus:ring-cinema-500"
              />
              <label class="text-sm text-[#6f5b54] dark:text-[#c5aea5]"
                >Stream Only (No downloads available)</label
              >
            </div>
          </div>
          ...
          <div
            class="pt-6 border-t border-[#dcc5b8] flex justify-end gap-4 dark:border-[#2d1a21]"
          >
            <button
              type="button"
              routerLink="/admin/movies"
              class="px-6 py-2 text-[#7b6660] hover:text-[#24181b] dark:text-[#9f7d73] dark:hover:text-white font-medium"
            >
              Cancel
            </button>
            <button
          ...
          @if (mutation.isError()) {
            <div
              class="bg-red-900/30 text-red-300 p-4 rounded-lg border border-red-700/60"
            >
    ...
    if (this.form.valid) {
      ...
      const payload: CreateMovieRequest & {
        ...
        fileUrls: Object.fromEntries(
          Object.entries(raw.fileUrls || {}).filter(
