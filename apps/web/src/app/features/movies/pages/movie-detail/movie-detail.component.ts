  ...
  template: `
    @if (query.isPending()) {
      <div class="animate-pulse">
        <div
          class="h-[400px] bg-[#e5d2c6] dark:bg-cinema-800 w-full mb-8"
        ></div>
    ...
    @if (query.isError()) {
      <div class="container mx-auto px-4 py-20 text-center">
        ...
        <button
          type="button"
          (click)="goBack()"
          class="text-cinema-500 hover:text-cinema-100 mt-4 block mx-auto"
        >
          ← Back
        </button>
      </div>
    ...
    @if (query.isSuccess(); as success) {
      @if (query.data()?.data; as movie) {
        <div
          class="relative w-full bg-[var(--bg-primary)] text-[var(--text-primary)] overflow-hidden mb-8"
        >
          <div
          ...
          ></div>
          <div
            class="absolute inset-0 bg-gradient-to-t from-[#f8f0e9] via-[#f8f0e9]/70 to-white/20 dark:from-cinema-900 dark:via-cinema-900/70 dark:to-black/20"
          ></div>

          <div
            class="relative container mx-auto px-4 py-12 md:py-20 flex flex-col md:flex-row gap-8 items-center md:items-end"
          >
            <button
              type="button"
              (click)="goBack()"
            ...
            <div
              class="w-48 md:w-64 flex-shrink-0 rounded-sm shadow-2xl overflow-hidden border-2 border-white/10"
            >
              <img
                [src]="movie.posterUrl || movie.thumbnailUrl"
                [alt]="normalizeTitle(movie.title)"
                class="w-full h-auto"
              />
            </div>
            ...
            <div class="flex-grow text-center md:text-left space-y-4">
              <div class="flex flex-wrap gap-2 justify-center md:justify-start">
                @if (movie.metadata?.nollywood) {
                  <span
                    class="bg-cinema-500 text-white text-xs font-bold px-2 py-1 rounded-sm"
                    >Nollywood</span
                  >
                }
                <span
                  class="bg-black/10 text-[#24181b] dark:bg-white/10 dark:text-white text-xs font-bold px-2 py-1 rounded-sm"
                  >{{ movie.year }}</span
                >
                <span
                  class="bg-green-500/20 text-green-400 text-xs font-bold px-2 py-1 rounded-sm"
                  >{{ movie.rating }}% Match</span
                >
                @if (movie.isStreamOnly) {
                  <span
                    class="bg-red-500 text-white text-xs font-bold px-2 py-1 rounded-sm"
                    >STREAM ONLY</span
                  >
                }
              </div>
              ...
              <h1
                class="text-3xl md:text-5xl font-serif font-bold leading-tight"
              >
                {{ normalizeTitle(movie.title) }}
              </h1>
              @if (movie.tagline) {
                <p
                  class="text-base md:text-lg text-[#6f5b54] dark:text-gray-300 italic"
                >
                  {{ movie.tagline }}
                </p>
              }
              ...
              <div
                class="text-[#725f58] dark:text-gray-400 text-sm md:text-base flex flex-wrap gap-x-4 gap-y-1 justify-center md:justify-start"
              >
              ...
              <div class="flex flex-wrap gap-2 justify-center md:justify-start">
                @if (movie.imdbRating) {
                  <span
                    class="bg-yellow-400 text-black text-xs font-bold px-3 py-1 rounded-sm"
                    >IMDb {{ movie.imdbRating.toFixed(1) }}</span
                  >
                }
                @if (movie.rottenTomatoes) {
                  <span
                    class="bg-red-600 text-white text-xs font-bold px-3 py-1 rounded-sm"
                    >Tomatometer {{ movie.rottenTomatoes }}</span
                  >
                }
                @if (movie.tmdbRating) {
                  <span
                    class="bg-sky-500 text-white text-xs font-bold px-3 py-1 rounded-sm"
                    >TMDB {{ movie.tmdbRating.toFixed(1) }}</span
                  >
                }
              </div>
              ...
              <!-- Engagement stats -->
              @if (
                (movie.viewCount ?? 0) > 0 || (movie.downloadCount ?? 0) > 0
              ) {
                <div
                  class="flex flex-wrap gap-3 text-xs text-[#725f58] dark:text-gray-400 justify-center md:justify-start"
                >
                ...
                @if (auth.currentUser()) {
                  ...
                  <button
                    ...
                    [title]="
                      isSubscribedToNotifications()
                        ...
                        : 'Notify me when available in HD'
                    "
                  >
                  ...
                  <button
                    ...
                    [disabled]="
                      ...
                      downloadRequestStatus() === 'SEARCHING' ||
                      downloadRequestStatus() === 'QUEUED'
                    "
        ...
        <div
          class="container mx-auto px-4 pb-20 grid grid-cols-1 lg:grid-cols-3 gap-8"
        >
          <div class="lg:col-span-2 space-y-8">
            <section>
              <h3
                class="text-xl font-serif font-bold text-[#24181b] dark:text-white mb-3"
              >
                Storyline
              </h3>
              <p
                class="text-[#725f58] dark:text-gray-400 leading-relaxed text-lg"
              >
                {{
                  movie.overview ||
                    movie.description ||
              ...
              <div
                class="mt-6 grid grid-cols-2 md:grid-cols-3 gap-4 border-t border-[#d9c4b7] dark:border-white/10 pt-6"
              >
                <div>
                  <span
                    class="block text-[10px] uppercase tracking-widest text-[#9f7d73] mb-1"
                    >Status</span
                  >
                  <span
                    class="text-sm font-semibold text-[#24181b] dark:text-white"
                    >{{ movie.status || "Released" }}</span
                  >
                </div>
                <div>
                  <span
            ...
            <section
              class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-[#f0e4da]/30 dark:bg-cinema-800/20 p-6 rounded-lg border border-[#d9c4b7] dark:border-white/5"
            >
              <div>
                <h4 class="font-bold text-[#24181b] dark:text-white mb-2">
                  Direction
                </h4>
                ...
                <h4 class="font-bold text-[#24181b] dark:text-white mt-4 mb-2">
                  Original Studio
                </h4>
                <p class="text-[#725f58] dark:text-gray-400">
                  Nollywood Distribution Network
                </p>
              </div>
              <div>
                <h4 class="font-bold text-[#24181b] dark:text-white mb-2">
                ...
                <div class="flex flex-wrap gap-2">
                  @for (g of movie.genre; track g) {
                    <span
                      class="bg-white/50 dark:bg-white/5 text-[#24181b] dark:text-gray-300 text-xs px-2 py-1 rounded border border-[#d9c4b7] dark:border-white/10"
                      >{{ g }}</span
                    >
                  }
                </div>
                ...
                @if ((movie.cast || []).length > 0) {
                  <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    @for (actor of movie.cast; track actor.id) {
                      <div
                        class="bg-[#f0e4da] dark:bg-cinema-800/70 border border-[#d9c4b7] dark:border-white/10 rounded-lg p-3 text-center"
                      >
                        @if (actor.photoUrl) {
                          <img
                            [src]="actor.photoUrl"
                            [alt]="actor.name"
                            class="w-14 h-14 rounded-full object-cover mx-auto mb-2 border border-[#d9c4b7] dark:border-white/10"
                          />
                        } @else {
                          <div
                            class="w-14 h-14 rounded-full bg-[#dfc8bb] dark:bg-cinema-700 text-[#6b3d2e] dark:text-cinema-200 mx-auto mb-2 flex items-center justify-center text-xs font-bold"
                          >
                            {{ actorInitials(actor.name) }}
                          </div>
                        }
                        <p
                          class="text-sm text-[#24181b] dark:text-white font-semibold leading-tight"
                        >
                          {{ actor.name }}
                        </p>
                        <p
                          class="text-xs text-[#725f58] dark:text-gray-400 leading-tight"
                        >
                  ...
                  <div class="flex flex-wrap gap-2">
                    @for (actor of movie.metadata?.cast || []; track actor) {
                      <span
                        class="bg-[#f0e4da] dark:bg-cinema-800 text-[#24181b] dark:text-gray-300 text-sm px-3 py-1 rounded-sm"
                        >{{ actor }}</span
                      >
                    }
                  </div>
          ...
          <div class="lg:col-span-1">
            <div
              class="bg-[#f8f0e9] dark:bg-cinema-800/50 backdrop-blur-sm border border-[#d9c4b7] dark:border-white/5 p-6 sticky top-24"
            >
              <div class="text-center py-4">
                <div
                  class="w-16 h-16 bg-cinema-500/20 rounded-full flex items-center justify-center mx-auto mb-4"
                >
                  <svg
                    class="w-8 h-8 text-cinema-500"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
                ...
                <h3
                  class="text-lg font-serif font-bold text-[#24181b] dark:text-white mb-2"
                >
                  Streaming Available
                </h3>
                <p class="text-[#725f58] dark:text-gray-400 text-sm mb-4">
                  This title plays instantly via YouTube or multi-provider
                  embeds.
                </p>
                @if (canWatch(movie)) {
                  <a
                  ...
                  <div
                    class="rounded-sm border border-[#d9c4b7] dark:border-white/10 px-4 py-3 text-sm text-[#725f58] dark:text-gray-400"
                  >
        ...
        @if (similarMoviesSignal() && similarMoviesSignal().length > 0) {
          <div class="mt-12 border-t border-[#d9c4b7] dark:border-white/5 pt-8">
            <h2
              class="text-2xl font-serif font-bold text-[#24181b] dark:text-white mb-6"
            >
              More Like This
            </h2>
            ...
            >
              @for (movie of similarMoviesSignal(); track movie.id) {
                <a [routerLink]="['/movies', movie.slug]" class="group block">
                  <div
                    class="relative aspect-[2/3] rounded-lg overflow-hidden bg-[#e5d2c6] dark:bg-cinema-800"
                  >
                    @if (movie.posterUrl || movie.thumbnailUrl) {
                      ...
                      <div
                        class="w-full h-full flex items-center justify-center text-[#9f7d73] dark:text-gray-600"
                      >
                        <svg
                          class="w-12 h-12"
                          fill="currentColor"
                  ...
                  <div class="mt-2">
                    <h3
                      class="text-sm font-medium text-[#24181b] dark:text-white line-clamp-1 group-hover:text-cinema-500 transition-colors"
                    >
                      {{ normalizeTitle(movie.title) }}
                    </h3>
                    <p class="text-xs text-[#9f7d73]">
                      {{ movie.year }} • {{ movie.rating || "0" }}% Match
                    </p>
                  </div>
                </a>
              }
...
export class MovieDetailComponent {
    ...
    toObservable(this.slug).pipe(
      switchMap((slug) =>
        !slug
          ? of([] as Movie[])
          : this.http
              .get<{ data: Movie[] }>(`/api/v1/movies/${slug}/similar`)
              .pipe(
    ...
    effect(() => {
      ...
      if (movie) {
        ...
        this.meta.updateTag({
          ...
          content: movie.description || "Watch now on NaijasPride.",
        });
        this.meta.updateTag({
          property: "og:image",
        ...
        this.meta.updateTag({
          property: "og:url",
          content: `https://naijaspride.com/movies/${movie.slug}`,
        });
  ...
  private checkNotificationStatus(movieId: string) {
    this.http
      ...
      }>(`/api/v1/movies/notifications/check/${movieId}`)
      .subscribe({
  ...
  requestDownload(movieId: string) {
    ...
    if (status === "PENDING" || status === "SEARCHING" || status === "QUEUED")
      return;
    this.downloadRequestStatus.set("PENDING");
    this.http
      .post<{
        data: { status: string };
      }>("/api/v1/download-requests", { movieId })
      .subscribe({
