    ...
    <main style="max-width:1600px; margin:0 auto; padding:32px 24px;">
      ...
      @if (loadError()) {
        <div style="text-align:center; padding:60px 20px;">
          <div
            style="width:56px;height:56px;margin:0 auto 16px;border-radius:50%;background:rgba(239,68,68,0.1);display:flex;align-items:center;justify-content:center;"
          >
            <svg
          ...
          <p style="color:#6b5b54;margin-bottom:16px;">
            Couldn't load movies right now.
          </p>
          <button
            (click)="retryLoad()"
            style="background:#800020;color:#fff;font-weight:700;font-size:13px;padding:10px 28px;border-radius:9999px;border:none;cursor:pointer;transition:background 0.2s;"
            onmouseenter="this.style.background='#a3213a'"
      ...
      <section style="margin-bottom:52px;">
        ...
        >
          <h2
            style="font-size:22px; font-weight:700; letter-spacing:-0.01em; margin:0; color:#f9f9f2;"
          >
            Stream Cinema
          </h2>
        ...
        <p
          style="font-size:13px; color:#6b5b54; margin:0 0 20px 0; font-style:italic;"
        >
          Handpicked for your next binge
        </p>
        ...
        @if (isLoading() && streamOnly().length === 0) {
              ...
              <div>
                ...
                <div style="display:flex;gap:10px;margin-top:10px;">
                  ...
                  <div style="flex:1;">
                    ...
                    <div
                      class="skeleton"
                      style="height:9px;width:55%;margin-top:7px;"
                    ></div>
                  </div>
                </div>
              </div>
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            @for (movie of streamOnly(); track movie.id) {
              <app-movie-card
                [movie]="movie"
                [progress]="getMovieProgress(movie.id)"
              />
            }
          </div>
        }
...
export class MoviesEditorialLandingComponent implements OnInit, OnDestroy {
  ...
  comingSoon = signal<
    Array<MovieSummary & { _count?: { notifications: number } }>
  >([]);

  trendingDownload = computed(() =>
    this.trending().filter((m) => !m.isStreamOnly),
  );
  latestUploadsDownload = computed(() =>
    this.latestUploads().filter((m) => !m.isStreamOnly),
  );
  newReleasesDownload = computed(() =>
    this.newReleases().filter((m) => !m.isStreamOnly),
  );
  ...
  private loadFeatured() {
    ...
    this.http.get<FeaturedResponse>("/api/v1/movies/featured").subscribe({
      next: (res) => {
        ...
        const hero =
          res.data.mostWatched.find((m) => !m.isStreamOnly) ||
          res.data.mostWatched[0] ||
          null;
