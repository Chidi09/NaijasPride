  ...
  styles: [`
    ...
    @media (min-width: 768px) {
      .pill-player {
        ...
        left: 50%;
        right: auto;
        transform: translateX(calc(-50% + 40px)); /* +40px = half of 80px sidebar */
        width: 600px;
        border-radius: 999px;
        ...
        box-shadow:
          ...
          /* color glow */
          0 0 60px -8px var(--now-playing-color-alpha, rgba(128, 0, 32, 0.25));
    ...
    .pill-art:hover img { transform: scale(1.08); }
    ...
    .ctrl-btn.active-color { color: var(--now-playing-color, #800020); }
    ...
    .play-btn {
      ...
      border: none;
      cursor: pointer;
      box-shadow: 0 4px 16px var(--now-playing-color-alpha, rgba(128,0,32,0.5));
      ...
      <div class="pill-player">
        ...
        <div class="flex-1 min-w-0">
          ...
          @if (player.queue().length > 1) {
            <p class="queue-pos">{{ player.queueIndex() + 1 }}&thinsp;/&thinsp;{{ player.queue().length }}</p>
          }
        </div>

        <!-- Waveform equalizer — visible when playing -->
        <div class="waveform" [ngClass]="player.isPlaying() ? 'active' : ''" aria-hidden="true">
        ...
        >
          @if (player.isPlaying()) {
            ...
            <svg width="14" height="14" fill="currentColor" viewBox="0 0 24 24" style="margin-left:1px">
...
export class MiniPlayerComponent implements OnDestroy {
  ...
  @HostBinding('style.--now-playing-color-alpha') nowPlayingColorAlpha = 'rgba(128,0,32,0.35)';
