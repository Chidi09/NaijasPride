...
@Component({
  ...
  template: `
    <div
      class="min-h-screen bg-[var(--music-bg)] text-[var(--music-text)] pb-28"
    >
      @if (loading()) {
        <div
          class="flex items-center justify-center py-24 text-[var(--music-text-muted)]"
        >
          Loading...
        </div>
      }
      ...
      @if (error()) {
        <div class="max-w-4xl mx-auto px-4 py-16 text-center">
          ...
          <a
            routerLink="/music"
            class="mt-4 inline-block text-[#800020] hover:underline"
            >Back to Music</a
          >
        </div>
      }
        ...
        <div class="max-w-7xl mx-auto px-4 pt-6">
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2">
              <!-- YouTube embed -->
              <div
                class="aspect-video w-full rounded-xl overflow-hidden bg-black shadow-2xl"
              >
                @if (embedUrl()) {
                  <iframe
                    [src]="embedUrl()!"
              ...
              <div class="mt-4">
                <div class="flex items-start justify-between gap-4">
                  <div>
                    <h1
                      class="text-2xl font-bold leading-tight text-[var(--music-text)]"
                    >
                      {{ video()!.title }}
                    </h1>
                    <div class="flex items-center gap-2 mt-1 flex-wrap">
                      ...
                      <span class="text-[var(--music-text-muted)]"
                        >&middot;</span
                      >
                      <span class="text-[var(--music-text-muted)] text-sm">{{
                        video()!.year
                      }}</span>
                    </div>
                    <div
                      class="flex flex-wrap items-center gap-3 mt-2 text-sm text-[var(--music-text-muted)]"
                    >
                  ...
                  <div class="flex items-center gap-2 flex-shrink-0">
                    ...
                    <button
                      ...
                      [style.backgroundColor]="
                        video()!.isLiked
                          ? '#800020'
                          : 'var(--music-surface-strong)'
                      "
                      [style.color]="
                        video()!.isLiked ? '#ffffff' : 'var(--music-text)'
                      "
            ...
            <div class="lg:col-span-1">
              <h2 class="text-lg font-semibold mb-4 text-[var(--music-text)]">
                Up Next
              </h2>
              @if (related().length > 0) {
                <div class="space-y-3">
                  @for (rel of related(); track rel.id) {
                    ...
                    >
                      <div
                        class="w-20 h-20 rounded-md overflow-hidden flex-shrink-0"
                      >
                        <img
                      ...
                      <div class="min-w-0">
                        <p
                          class="text-sm font-medium text-[var(--music-text)] leading-tight truncate"
                        >
                          {{ rel.title }}
                        </p>
                        <p
                          class="text-xs text-[var(--music-text-muted)] mt-0.5 truncate"
                        >
                          {{ rel.artist }}
                        </p>
                        <p class="text-xs text-[var(--music-text-muted)] mt-1">
                          {{ formatCount(rel.viewCount) }} views
                        </p>
                      </div>
                    </div>
                  }
                ...
                <p class="text-[var(--music-text-muted)] text-sm">
                  No related videos
                </p>
              }
            </div>
