  ...
  template: `
    <div class="container mx-auto px-4 py-12">
      ...
      <div class="flex items-center gap-6 mb-12">
        ...
        <div>
          <h1 class="text-3xl font-serif text-white">My Library</h1>
      ...
      <div class="border-b border-gray-800 mb-8 flex gap-8">
        <button 
          ...
          class="pb-4 text-gray-500 hover:text-white border-b-2 border-transparent transition-all"
        >
          My Watchlist
        </button>
        <button 
      ...
      @if (query.isPending()) {
        <div class="animate-pulse space-y-4">
          <div class="h-64 bg-cinema-800 rounded"></div>
        </div>
      }
      ...
      @if (query.data(); as response) {
        @if (response.data; as profile) {
              ...
              @for (movie of profile.watchlist; track movie.id) {
                <a [routerLink]="['/movies', movie.slug]" class="group">
                  <div class="bg-cinema-800 rounded overflow-hidden transition-transform group-hover:scale-105">
                    ...
                    >
                    <div class="p-3">
                      <h3 class="text-white font-medium text-sm truncate">{{ movie.title }}</h3>
          ...
          @if (activeTab === 'history') {
            <div class="bg-cinema-800 rounded-lg overflow-hidden">
              <table class="w-full text-left text-sm text-gray-400">
                <thead class="bg-black/20 text-gray-200">
                  <tr>
                ...
                <tbody class="divide-y divide-gray-700">
                  @for (item of profile.downloadHistory; track item.id) {
                    <tr class="hover:bg-white/5 transition-colors">
                      <td class="p-4">
                        <a [routerLink]="['/movies', item.movie.slug]" class="text-white font-medium hover:text-cinema-500">
                          {{ item.movie.title }}
                        </a>
                      </td>
                      <td class="p-4">{{ item.timestamp | date:'mediumDate' }}</td>
                      <td class="p-4">
                        <span class="bg-gray-700 px-2 py-1 rounded text-xs">{{ item.quality }}</span>
                      </td>
                      <td class="p-4">
                        <a 
          ...
          @if (profile.recommendations.length > 0) {
            <div class="mt-12">
              <h2 class="text-xl font-serif text-white mb-6">Recommended for You</h2>
              <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
