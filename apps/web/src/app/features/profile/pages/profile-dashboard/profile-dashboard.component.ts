...
import {
  ...
  ProfileStat,
} from "../../../../shared/components/profile-stats-card/profile-stats-card.component";
  ...
  template: `
    <div class="container mx-auto px-4 py-12 text-[var(--text-primary)]">
      ...
      <div class="flex items-center gap-6 mb-12">
        <div
          class="w-20 h-20 rounded-full bg-cinema-500 flex items-center justify-center text-3xl font-serif text-white"
        >
          {{ auth.currentUser()?.email?.charAt(0) | uppercase }}
        </div>
        <div>
          <h1 class="text-3xl font-serif text-[#24181b] dark:text-white">
            My Library
          </h1>
          <p class="text-[#7b6660] dark:text-gray-400">
            {{ auth.currentUser()?.email }}
          </p>
          @if (auth.currentUser()?.role === "ADMIN") {
            <span class="text-xs bg-red-600 text-white px-2 py-0.5 rounded"
              >ADMIN</span
            >
          }
          @if (authState.isGuest()) {
            <span class="text-xs bg-yellow-600 text-white px-2 py-0.5 rounded"
              >GUEST</span
            >
          }
        </div>
      ...
      @if (authState.isGuest()) {
        <div
          class="mb-8 bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
        >
          <div>
            <p
              class="text-[#24181b] dark:text-yellow-300 font-semibold text-sm"
            >
              You're browsing as a guest
            </p>
            <p class="text-[#725f58] dark:text-yellow-400/70 text-xs mt-0.5">
              Create a free account to save your watchlist, track history, and
              unlock more features.
            </p>
          </div>
          <a
            routerLink="/register"
            class="shrink-0 bg-cinema-500 hover:bg-cinema-400 text-white text-sm font-bold px-5 py-2 rounded-full transition-colors"
          >
            Create Account
          </a>
        </div>
      ...
      <!-- Tabs -->
      <div
        class="border-b border-[#d8c2b8] dark:border-gray-800 mb-8 flex gap-8"
      >
        <button
        ...
        >
          ...
          @if (continueWatching.length > 0) {
            <span
              class="ml-2 bg-cinema-500 text-white text-xs px-2 py-0.5 rounded-full"
        ...
        >
          ...
          @if (auth.currentUser()?.isPremium) {
            <span
              class="ml-2 bg-green-500 text-white text-xs px-2 py-0.5 rounded-full"
              >PRO</span
            >
          }
        </button>
      ...
      @if (query.isError()) {
        <div class="text-center py-20">
          <div
            class="w-14 h-14 mx-auto mb-4 rounded-full bg-red-500/10 flex items-center justify-center"
          >
            <svg
          ...
          <p class="text-[#8a756e] dark:text-gray-500 mb-4">
            Failed to load profile.
          </p>
          <button
            (click)="query.refetch()"
            class="bg-cinema-500 hover:bg-cinema-400 text-white text-sm font-bold px-6 py-2 rounded-full transition-colors"
          >
            Retry
          </button>
        </div>
      ...
      @if (query.data(); as response) {
        @if (response.data; as profile) {
          ...
          @if (activeTab === "continue") {
            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
              @for (item of continueWatching; track item.id) {
                <div class="group">
                  <div
                    class="bg-[#f1e5dd] dark:bg-cinema-800 rounded overflow-hidden transition-transform group-hover:scale-105"
                  >
                    <a
                      [routerLink]="[
                        '/watch',
                    ...
                    >
                      ...
                      >
                        <div
                          class="w-12 h-12 rounded-full bg-cinema-500 flex items-center justify-center"
                        >
                          <svg
                    ...
                    <div class="p-3">
                      ...
                      >
                        <h3
                          class="text-[#24181b] dark:text-white font-medium text-sm truncate group-hover:text-cinema-500 transition-colors"
                        >
                          {{ item.movie.title }}
                        </h3>
                      </a>
              ...
              @if (continueWatching.length === 0) {
                <div
                  class="col-span-full text-center py-20 text-[#8a756e] dark:text-gray-500"
                >
                  <div class="mb-4">
                    <span
                  ...
                  <a
                    routerLink="/movies"
                    class="text-cinema-500 hover:text-cinema-400"
                    >Start watching something →</a
                  >
                </div>
              }
            </div>
              ...
              @for (movie of profile.watchlist; track movie.id) {
                ...
                >
                  ...
                  >
                    ...
                    <div class="p-3">
                      <h3
                        class="text-[#24181b] dark:text-white font-medium text-sm truncate"
                      >
                        {{ movie.title }}
                      </h3>
                      <p class="text-[#8a756e] dark:text-gray-500 text-xs">
                        {{ movie.year }}
                      </p>
                    </div>
                  </div>
          ...
          @if (activeTab === "history") {
            <div
              class="bg-[#f1e5dd] dark:bg-cinema-800 rounded-lg overflow-hidden"
            >
              <table
                class="w-full text-left text-sm text-[#735f58] dark:text-gray-400"
              >
                <thead
                  class="bg-[#ead7cc] dark:bg-black/20 text-[#2a1c1f] dark:text-gray-200"
                >
                  <tr>
                ...
                <tbody class="divide-y divide-[#dcc5b8] dark:divide-gray-700">
                  @for (item of profile.downloadHistory; track item.id) {
                    <tr
                      class="hover:bg-[#ead7cc] dark:hover:bg-white/5 transition-colors"
                    >
                      <td class="p-4">
                        <a
                          [routerLink]="[
                          ...
                          class="text-[#24181b] dark:text-white font-medium hover:text-cinema-500"
                        >
                          {{ item.movie.title }}
                        </a>
                      </td>
                      ...
                      <td class="p-4">
                        <span
                          class="bg-[#dcc5b8] dark:bg-gray-700 px-2 py-1 rounded text-xs"
                          >{{ item.quality }}</span
                        >
                      </td>
                      <td class="p-4">
                        @if (item.movie.fileUrls?.[item.quality]) {
                          ...
                          <span
                            class="text-[#8a756e] dark:text-gray-600 text-sm"
                            >Unavailable</span
                          >
                        }
                      </td>
              ...
              @if (profile.downloadHistory.length === 0) {
                <div
                  class="text-center py-20 text-[#8a756e] dark:text-gray-500"
                >
          ...
          @if (mlRecommendations().length > 0) {
            <div
              class="mt-12 border-t border-[#dcc5b8] dark:border-zinc-800 pt-10"
            >
              <div class="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p class="text-xs text-[#8a756e] dark:text-gray-500 mt-1">
                    {{
                      recommendationReason() === "trending"
                        ? "Start watching to get personalised picks"
                ...
                <a
                  routerLink="/movies"
                  class="text-[#800020] text-sm font-medium hover:underline"
                  >Browse all →</a
                >
              </div>
              <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                @for (movie of mlRecommendations(); track movie.id) {
                  ...
                  >
                    <div
                      class="bg-[#f1e5dd] dark:bg-cinema-800 rounded-lg overflow-hidden transition-transform duration-300 group-hover:scale-105 group-hover:shadow-xl"
                    >
                      @if (movie.thumbnailUrl) {
                        ...
                        <div
                          class="w-full aspect-[2/3] flex items-center justify-center bg-[#dfc8bb] dark:bg-cinema-700"
                        >
                          <span
                      ...
                      <div class="p-2">
                        <h3
                          class="text-[#24181b] dark:text-white font-medium text-xs truncate group-hover:text-[#800020] dark:group-hover:text-[#d6b87a] transition-colors"
                        >
                          {{ movie.title }}
                        </h3>
                        <p
                          class="text-[#8a756e] dark:text-gray-500 text-[11px]"
                        >
                          {{ movie.year }}
                        </p>
                      </div>
          ...
          @if (isLoadingRecommendations()) {
            ...
            >
              <div
                class="h-5 w-48 bg-[#e5d2c6] dark:bg-cinema-800 rounded animate-pulse mb-6"
              ></div>
              <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                @for (i of [1, 2, 3, 4, 5, 6]; track i) {
                  <div
                    class="aspect-[2/3] bg-[#e5d2c6] dark:bg-cinema-800 rounded-lg animate-pulse"
                  ></div>
                }
              </div>
            </div>
  ...
  private loadRecommendations() {
    this.http
      .get<{
        ...
        data: MovieSummary[];
        reason: string;
      }>("/api/v1/profile/recommendations", { params: { limit: "12" } })
      .subscribe({
        next: (res) => {
