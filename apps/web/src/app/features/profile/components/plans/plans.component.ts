    ...
    <div class="bg-black min-h-screen py-20 px-4 text-white font-sans">
      <div class="text-center max-w-2xl mx-auto mb-16">
        ...
        <p class="text-gray-400">
          Watch everything without interruptions. Cancel anytime.
        </p>
      </div>

      @if (isSubscribed()) {
        <div
          class="max-w-xl mx-auto mb-10 rounded-xl border border-green-700/40 bg-green-900/20 px-5 py-4 text-center"
        >
          <div
            class="flex items-center justify-center gap-2 text-green-300 font-semibold text-sm"
          >
            <app-check-icon [size]="16" fillColor="#4ade80" />
      ...
      @if (loadingPlans()) {
        <div class="flex justify-center py-16">
          <div
            class="w-10 h-10 border-4 border-red-600/30 border-t-red-600 rounded-full animate-spin"
          ></div>
        </div>
      } @else if (plansError()) {
        ...
        <div class="grid md:grid-cols-2 gap-8 max-w-3xl mx-auto">
          @for (plan of plans(); track plan.slug) {
            <div
              ...
              [ngClass]="
                isAnnual(plan)
                  ? 'border-2 border-red-600 shadow-2xl bg-zinc-900'
                  : 'border border-gray-700 bg-zinc-900/50'
              "
            >
              @if (isAnnual(plan)) {
                <div
                  class="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-red-600 px-4 py-1 rounded-full text-xs font-bold tracking-wider whitespace-nowrap"
                >
                  BEST VALUE
                </div>
              }
              ...
              @if (isCurrentPlan(plan)) {
                <div
                  class="absolute top-3 right-3 bg-green-600/20 text-green-400 text-[10px] font-bold px-2 py-0.5 rounded-full border border-green-600/40"
                >
                  CURRENT PLAN
                </div>
              }
              ...
              <p class="text-gray-500 text-sm mb-6">
                {{
                  isAnnual(plan) ? "Billed once a year" : "Billed every month"
                }}
              </p>
              ...
              <div class="mb-6">
                ...
                <span class="text-gray-500 ml-1">{{
                  isAnnual(plan) ? "/year" : "/month"
                }}</span>
                @if (isAnnual(plan)) {
                  <div class="mt-1 text-sm text-red-400 font-medium">
              ...
              @if (isCurrentPlan(plan) && renewalDate()) {
                <div
                  class="mb-4 rounded-lg bg-green-900/20 border border-green-700/30 px-3 py-2 text-xs text-green-400/80 text-center"
                >
...
export class PlansComponent implements OnInit {
  ...
  ngOnInit() {
    this.http
      .get<{ success: boolean; data: ApiPlan[] }>("/api/v1/plans")
      .subscribe({
        next: (res) => {
  ...
  subscribe(plan: ApiPlan) {
    if (!this.authState.isAuthenticated()) {
      this.router.navigate(["/register"], {
        queryParams: { redirect: "/profile/plans" },
      });
      return;
    }
    ...
    this.http
      ...
      }>("/api/v1/payments/initialize", { plan: plan.slug })
      .subscribe({
        next: (res) => {
        ...
        error: (err) => {
          this.loading.set(null);
          this.error.set(
            err?.error?.message || "Could not start payment. Please try again.",
          );
        },
      });
  }
