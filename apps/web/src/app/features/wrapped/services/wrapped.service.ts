...
export class WrappedService {
  ...
  getMyWrapped(period: string): Observable<WrappedData | null> {
    ...
    if (this._cache.has(cacheKey)) {
      return this._cache.get(cacheKey)!.pipe(catchError(() => of(null)));
    }

    const request$ = this.http
      .get<{ success: boolean; data: WrappedData }>(`/api/v1/wrapped/${period}`)
      .pipe(
  ...
  getAvailablePeriods(): Observable<string[]> {
    return this.http
      .get<{ success: boolean; data: string[] }>("/api/v1/wrapped/periods")
      .pipe(
  ...
  regenerateWrapped(period: string): Observable<WrappedData | null> {
    ...
    return this.http
      .post<{
        success: boolean;
        data: WrappedData;
      }>(`/api/v1/wrapped/${period}/generate`, {})
      .pipe(
        map((response) => (response.success ? response.data : null)),
        tap((data) => {
          if (data) {
  ...
  getPublicWrapped(
    userId: string,
    period: string,
  ): Observable<PublicWrappedData | null> {
    return this.http
      .get<{
        success: boolean;
        data: PublicWrappedData;
      }>(`/api/v1/wrapped/public/${userId}/${period}`)
      .pipe(
        map((response) => (response.success ? response.data : null)),
        catchError(() => of(null)),
      );
