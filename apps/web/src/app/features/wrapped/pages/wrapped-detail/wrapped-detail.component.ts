...
import { WrappedService, WrappedData } from "../../services/wrapped.service";
...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-[#0a0a0a] text-[#f9f9f2]">
      <!-- Header -->
      <header
        class="fixed top-0 left-0 right-0 z-50 bg-gradient-to-b from-[#0a0a0a] to-transparent px-6 py-4"
      >
        <div class="max-w-md mx-auto flex items-center justify-between">
          ...
          <div class="flex items-center gap-3">
            <span class="font-['Cinzel'] text-lg font-bold text-[#d6b87a]">{{
              periodLabel()
            }}</span>
      ...
      @if (loading()) {
        <div class="min-h-screen flex items-center justify-center">
          <div class="text-center space-y-4">
            <div
              class="w-16 h-16 border-4 border-[#d6b87a]/20 border-t-[#d6b87a] rounded-full animate-spin mx-auto"
            ></div>
      ...
      @if (error()) {
        <div class="min-h-screen flex items-center justify-center px-6">
          <div class="text-center max-w-sm space-y-6">
            ...
            <h1 class="font-['Cinzel'] text-2xl font-bold">
              Wrapped Not Ready
            </h1>
      ...
      @if (wrappedData() && !loading() && !error()) {
        <div class="min-h-screen flex items-center justify-center py-20">
          <!-- Card Counter -->
          <div
            class="fixed bottom-8 left-1/2 -translate-x-1/2 z-40 bg-[#120a0d]/90 backdrop-blur-sm rounded-full px-6 py-3 border border-[#5f1327]"
          >
            <p class="text-sm text-[#a88a78]">
              <span class="text-[#d6b87a] font-bold">{{
                currentCardIndex() + 1
              }}</span>
              /
              {{ cards().length }}
          ...
          @if (showSwipeHint()) {
            <div
              class="fixed bottom-24 left-1/2 -translate-x-1/2 z-40 animate-bounce"
            >
              <p
                class="text-sm text-[#a88a78] bg-[#120a0d]/80 px-4 py-2 rounded-full"
              >
                <span
      ...
      @if (showShareModal()) {
        ...
        >
          ...
          >
            <h2 class="font-['Cinzel'] text-xl font-bold text-center">
              Share Your Wrapped
            </h2>
...
export class WrappedDetailComponent implements OnInit {
  ...
  cards = computed(() => {
    ...
    if (data.hero)
      cardList.push({ type: "hero", url: data.hero, label: "Overview" });
    if (data.topMovie)
      cardList.push({
        type: "topMovie",
        url: data.topMovie,
    ...
    if (data.topBook)
      cardList.push({ type: "topBook", url: data.topBook, label: "Top Book" });
    if (data.genres)
      cardList.push({ type: "genres", url: data.genres, label: "Genres" });
    if (data.summary)
      cardList.push({ type: "summary", url: data.summary, label: "Summary" });
    ...
    this.wrappedService.regenerateWrapped(this.period()).subscribe({
      next: (data) => {
        ...
        if (data) {
          ...
          this.error.set(
            "Not enough activity to generate a Wrapped yet. Watch some movies!",
          );
        }
      },
      error: (err: unknown) => {
