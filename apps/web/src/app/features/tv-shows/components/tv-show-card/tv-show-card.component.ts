...
@Component({
  ...
  template: `
    ...
    >
      <!-- Glow Effect -->
      <div
        class="absolute -inset-0.5 bg-gradient-to-r from-[#800020]/0 via-[#800020]/0 to-[#800020]/0 opacity-0 blur transition duration-500 group-hover:from-[#800020]/20 group-hover:via-[#800020]/10 group-hover:to-[#800020]/20 group-hover:opacity-100"
      ></div>
      ...
      <!-- Poster Image -->
      <div
        class="relative aspect-[2/3] overflow-hidden bg-gradient-to-b from-zinc-800 to-zinc-900"
      >
        <img
        ...
        <!-- Gradient Overlay -->
        <div
          class="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent opacity-60 transition-opacity duration-500 group-hover:opacity-80"
        ></div>

        <!-- Hover Info Overlay -->
        <div
          class="absolute inset-0 flex flex-col justify-end p-3 opacity-0 transition-all duration-500 translate-y-4 group-hover:translate-y-0 group-hover:opacity-100"
        >
          <div
            class="transform transition-all duration-500 group-hover:translate-y-0"
          >
              ...
              <div class="mt-1 flex items-center gap-1 text-white/70">
                ...
                >
                  <path
                    ...
                    stroke-linejoin="round"
                    stroke-width="2"
                    d="M15 10l4.55-2.28A1 1 0 0121 8.62v6.76a1 1 0 01-1.45.9L15 14m-9 3h6a2 2 0 002-2V9a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2z"
                  />
                </svg>
                <span class="text-xs"
                  >{{ formatCount(show.viewCount ?? 0) }} views</span
                >
              </div>
            }
          </div>
        ...
        <!-- Year Badge -->
        <div
          class="absolute left-2 top-2 rounded-full bg-black/60 px-2 py-0.5 text-[10px] font-medium text-white backdrop-blur-sm"
        >
          {{ show.year }}
        </div>
      </div>
      ...
      <div class="relative p-3">
        <h3
          class="line-clamp-2 text-sm font-semibold text-white transition-colors duration-300 group-hover:text-[#800020]"
        >
          {{ show.title }}
        </h3>
        <!-- Genre Tags -->
        <div class="mt-2 flex flex-wrap gap-1">
          @for (genre of (show.genre || []).slice(0, 2); track genre) {
            <span
              class="rounded bg-white/5 px-1.5 py-0.5 text-[9px] text-white/50"
              >{{ genre }}</span
            >
          }
        </div>
