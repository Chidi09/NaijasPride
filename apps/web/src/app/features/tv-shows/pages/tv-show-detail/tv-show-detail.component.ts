      ...
      @if (query.isLoading()) {
        ...
        >
          ...
          <div class="relative p-6 md:p-8">
            ...
            <div
              class="mt-4 grid grid-cols-1 gap-6 md:grid-cols-[220px_1fr] md:items-end"
            >
              <div
              ...
              <div>
                <div class="mb-3 flex flex-wrap items-center gap-2 text-xs">
                  ...
                  <span class="rounded-full bg-white/10 px-2 py-1 text-white/80"
                    >{{ show()!.seasons.length }} seasons</span
                  >
                  <span class="rounded-full bg-white/10 px-2 py-1 text-white/80"
                    >{{ totalEpisodeCount() }} episodes</span
                  >
                ...
                <div class="mt-4 flex flex-wrap gap-2">
                  ...
                  @if (show()!.trailerUrl) {
                    <a
                      ...
                      class="rounded-full border border-white/20 px-5 py-2 text-sm text-white/90 hover:bg-white/10"
                    >
                      Watch Trailer
                    </a>
                  }
          ...
          <div class="relative z-10 border-t border-white/10 p-6 md:p-8">
            <div class="mb-3 flex items-center justify-between gap-2">
              ...
              <span class="text-xs text-white/55"
                >Optimized episode browser</span
              >
            </div>
              ...
              <div class="space-y-3">
                @for (episode of orderedEpisodes(); track episode.id) {
                  <button
                    ...
                    (click)="
                      watchEpisode(season.seasonNumber, episode.episodeNumber)
                    "
                  >
                    <span class="min-w-0 text-sm text-white">
                      <span class="mr-2 text-white/60"
                        >E{{ episode.episodeNumber }}</span
                      >
...
export class TvShowDetailComponent {
  ...
  selectedSeason = computed(() => {
    ...
    return (
      seasons.find(
        (season) => season.seasonNumber === this.selectedSeasonNumber(),
      ) || seasons[0]
    );
  });
