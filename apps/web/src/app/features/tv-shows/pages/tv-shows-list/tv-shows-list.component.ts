  ...
  template: `
    @if (useLivingRoomShell()) {
          ...
          >
            ...
            >
              ...
              <p class="mt-5 max-w-2xl text-base leading-8 text-white/68">
                Discover trending series, latest releases, and award-winning
                television from around the world in the new living-room layout.
              </p>
              ...
              >
                <div class="flex flex-col gap-3 md:flex-row md:items-center">
                  ...
                  <div class="flex gap-3">
                    <select
                      class="rounded-2xl border border-white/15 bg-white/[0.05] px-4 py-3 text-sm text-white outline-none"
                      [ngModel]="genre()"
                    ...
                    <button
                      ...
                      class="rounded-2xl bg-[#800020] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#a00030]"
                    >
                      Search
                    </button>
                    <button
                ...
                <div class="mb-10">
                  <div class="mb-4 flex items-center justify-between">
                    <div>
                      ...
                      >
                        {{ sectionQuery(key).data()?.data?.length || 0 }} shows
                      </p>
                    </div>
                    <button
                      type="button"
      ...
      <section class="relative min-h-screen overflow-hidden bg-[#0a0a0a]">
          ...
          >
            @if (featuredShow(); as show) {
              <div class="max-w-2xl animate-fade-in-up">
                <p
                  class="text-[11px] uppercase tracking-[0.28em] text-[#800020] font-semibold mb-3"
                >
            ...
            <div class="mt-8 max-w-lg">
              <div class="relative group">
                <div
                  class="absolute -inset-0.5 rounded-xl bg-gradient-to-r from-[#800020]/50 to-[#a00030]/50 opacity-0 blur transition duration-500 group-focus-within:opacity-100"
                ></div>
                <div class="relative flex items-center">
                  <svg
                    class="pointer-events-none absolute left-4 h-5 w-5 text-white/40"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                  ...
                  @if (q()) {
                    <button
                      class="absolute right-4 rounded-full p-1 text-white/40 transition hover:text-white"
                      (click)="q.set(''); onSearchInput('')"
                    >
                      <svg
                        class="h-4 w-4"
              ...
              @if (showSuggestions()) {
                <div
                  class="animate-scale-in absolute z-50 mt-2 w-full max-w-lg overflow-hidden rounded-xl border border-white/10 bg-[#0f0f0f]/95 shadow-2xl backdrop-blur-xl"
                >
                  @if (suggestionLoading()) {
                    <div class="flex items-center gap-3 px-4 py-4">
                      <div
                        class="h-4 w-4 animate-spin rounded-full border-2 border-[#800020] border-t-transparent"
                      ></div>
                  ...
                  @for (item of suggestions(); track item.id) {
                    <a
                      class="group flex items-center gap-3 border-b border-white/5 px-4 py-3 transition last:border-0 hover:bg-white/5"
                      [routerLink]="['/tv-shows', item.slug]"
                      (click)="onSuggestionSelect()"
                    >
                      <div
                        class="relative h-12 w-9 overflow-hidden rounded-md bg-white/5"
                      >
                        <img
                          [src]="
                            item.posterUrl ||
                      ...
                      <div class="min-w-0 flex-1">
                        <p
                          class="truncate text-sm font-medium text-white transition group-hover:text-[#800020]"
                        >
                          {{ item.title }}
                        </p>
                        <p class="truncate text-xs text-white/40">
                          {{ item.year }} • {{ item.seasonCount }} seasons
                        </p>
                      </div>
                      <svg
                        class="h-4 w-4 text-white/20 transition group-hover:translate-x-1 group-hover:text-[#800020]"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
        ...
        <!-- Filters Section -->
        <div
          class="animate-fade-in-up animation-delay-300 relative z-10 border-b border-white/5 bg-black/20 backdrop-blur-sm"
        >
...
export class TvShowsListComponent implements OnInit {
  ...
  trendingQuery = this.tvQuery.getShowsQuery(
    computed(() => ({ page: 1, limit: 9, sortBy: "trending" as const })),
  );
  latest2026Query = this.tvQuery.getShowsQuery(
    computed(() => ({
      page: 1,
      limit: 9,
  ...
  latest2025Query = this.tvQuery.getShowsQuery(
    computed(() => ({
      page: 1,
      limit: 9,
      sortBy: "latest" as const,
  ...
  highestRatedQuery = this.tvQuery.getShowsQuery(
    computed(() => ({ page: 1, limit: 9, sortBy: "popular" as const })),
  );
  awardWinningQuery = this.tvQuery.getShowsQuery(
    computed(() => ({
      page: 1,
      limit: 9,
