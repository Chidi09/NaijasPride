    ...
    @if (useCinemaShell()) {
      ...
      >
        ...
        @if (query.isLoading()) {
          ...
          <div
            class="mt-6 grid gap-6 xl:grid-cols-[1.2fr,0.8fr] xl:items-start"
          >
            <div class="space-y-4">
              <div
            ...
            <aside class="space-y-4">
              ...
              >
                ...
                <p class="mt-3 text-lg font-semibold text-white">
                  Auto-next is {{ autoNext() ? "on" : "off" }} for this series.
                </p>
                <label
                  class="mt-5 inline-flex items-center gap-3 rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-white/90"
                >
                  <input
                    type="checkbox"
                    class="h-4 w-4"
                    [checked]="autoNext()"
                    (change)="toggleAutoNext($event)"
                  />
                  Auto next episode
                </label>
              </div>
              ...
              >
                ...
                <div class="mt-4 flex flex-col gap-3">
                  ...
                  <a
                    [routerLink]="['/tv-shows', slug()]"
                    class="inline-flex items-center justify-center gap-3 rounded-[1.5rem] border border-white/10 bg-black/20 px-5 py-4 text-sm font-semibold text-white/80 transition hover:bg-white/[0.08] hover:text-white"
                  >
      ...
      <div class="min-h-screen bg-[#0a0a0a] px-4 py-4 md:px-8">
        ...
        @if (query.isLoading()) {
          ...
          <div
            class="mt-2 mb-4 flex flex-wrap items-center justify-between gap-3"
          >
            <div>
            ...
            <label
              class="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/5 px-3 py-1.5 text-xs text-white/90"
            >
              <input
                type="checkbox"
                class="h-3.5 w-3.5"
          ...
          <div class="mt-4 flex flex-wrap gap-2">
            ...
            <a
              [routerLink]="['/tv-shows', slug()]"
              class="rounded-full border border-white/20 px-4 py-2 text-xs text-white/80 hover:text-white"
              >Episode List</a
            >
          </div>
        }
...
export class TvWatchRoomComponent {
  ...
  currentEpisode = computed(() => {
    const row = this.show();
    if (!row) return null;
    const season = row.seasons.find(
      (entry) => entry.seasonNumber === this.seasonNumber(),
    );
    return (
      season?.episodes.find(
        (entry) => entry.episodeNumber === this.episodeNumber(),
      ) || null
    );
  });
  ...
  nextEpisodeTarget = computed(() => {
    ...
    const orderedSeasons = [...row.seasons].sort(
    ...
    const orderedEpisodes = [...currentSeason.episodes].sort(
      (a, b) => a.episodeNumber - b.episodeNumber,
    );
    const episodeIndex = orderedEpisodes.findIndex(
    ...
    if (seasonIndex < orderedSeasons.length - 1) {
      ...
      const firstEpisode = [...nextSeason.episodes].sort(
        (a, b) => a.episodeNumber - b.episodeNumber,
      )[0];
  ...
  constructor() {
    effect(
      () => {
        ...
        this.seasonNumber.set(
          Number.isFinite(season) && season > 0 ? season : 1,
        );
        this.episodeNumber.set(
          Number.isFinite(episode) && episode > 0 ? episode : 1,
        );
