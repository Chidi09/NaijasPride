...
@Component({
  ...
  template: `
    <div class="np-sidebar-shell h-full">
      <div
        class="flex items-center justify-between gap-3 border-b border-[var(--np-reader-border)] px-4 py-3"
      >
        <div class="min-w-0">
          <p class="truncate text-sm font-semibold">Reader</p>
          <p class="truncate text-[11px] text-[var(--np-reader-muted)]">
            {{ bookTitle || "Loading..." }}
          </p>
        </div>
        <button
      ...
      <div class="h-full overflow-auto px-4 py-4">
        ...
        @if (isSearching) {
          ...
          <mat-nav-list class="mt-2">
            @for (hit of searchResults; track hit.cfi) {
              <a
                mat-list-item
                (click)="goToCfi.emit(hit.cfi)"
                class="cursor-pointer"
              >
                <span matListItemTitle class="text-sm">{{ hit.excerpt }}</span>
                <span
                  matListItemLine
                  class="text-[11px] text-[var(--np-reader-muted)]"
                  >{{ hit.href }}</span
                >
              </a>
            }
        ...
        <div
          class="rounded-lg border border-[var(--np-reader-accent)] bg-[var(--np-reader-surface)] p-4"
        >
          <p
            class="text-[11px] uppercase tracking-[0.18em] text-[var(--np-reader-accent)] font-semibold"
          >
            Reading Mode
          </p>
          <p class="mt-1 text-xs text-[var(--np-reader-muted)]">
            Choose how to navigate through the book
          </p>
          ...
          @if (readerType === "pdf") {
            <p class="mt-2 text-xs text-[var(--np-reader-muted)] italic">
              PDF files only support page mode
            </p>
          }
        </div>
        ...
        @if (readerType === "epub") {
          <div
            class="mt-6 rounded-lg border border-[var(--np-reader-border)] p-3"
          >
            <p
              class="text-[11px] uppercase tracking-[0.18em] text-[var(--np-reader-accent)] font-medium"
            >
              Layout
            </p>
            <div class="mt-3 grid grid-cols-3 gap-2">
              <button
                mat-stroked-button
                type="button"
                (click)="setSpread.emit('auto')"
                [class.mat-accent]="spread === 'auto'"
              >
                Auto
              </button>
              <button
                mat-stroked-button
                type="button"
                (click)="setSpread.emit('single')"
                [class.mat-accent]="spread === 'single'"
              >
                Single
              </button>
              <button
                mat-stroked-button
                type="button"
                (click)="setSpread.emit('double')"
                [class.mat-accent]="spread === 'double'"
              >
                Double
              </button>
            </div>
          ...
          >
            ...
            <div class="mt-3 grid grid-cols-3 gap-2">
              <button
                mat-stroked-button
                type="button"
                (click)="setFontFamily.emit('serif')"
                [class.mat-accent]="fontFamily === 'serif'"
              >
                Serif
              </button>
              <button
                mat-stroked-button
                type="button"
                (click)="setFontFamily.emit('sans')"
                [class.mat-accent]="fontFamily === 'sans'"
              >
                Sans
              </button>
              <button
                mat-stroked-button
                type="button"
                (click)="setFontFamily.emit('mono')"
                [class.mat-accent]="fontFamily === 'mono'"
              >
                Mono
              </button>
            </div>
          ...
          <div class="mt-6">
            <p
              class="text-[11px] uppercase tracking-[0.18em] text-[var(--np-reader-muted)]"
            >
              PDF
            </p>
            <p class="mt-2 text-xs text-[var(--np-reader-muted)]">
        ...
        <div class="np-sidebar-panel mt-6 p-3">
          ...
          <div class="mt-3 grid grid-cols-3 gap-2">
            <button
              mat-stroked-button
              type="button"
              (click)="setTheme.emit('paper')"
              [class.mat-accent]="theme === 'paper'"
            >
              Paper
            </button>
            <button
              mat-stroked-button
              type="button"
              (click)="setTheme.emit('sepia')"
              [class.mat-accent]="theme === 'sepia'"
            >
              Sepia
            </button>
            <button
              mat-stroked-button
              type="button"
              (click)="setTheme.emit('night')"
              [class.mat-accent]="theme === 'night'"
            >
              Night
            </button>
          </div>
        ...
        @if (readerType === "epub") {
          <div class="mt-6">
            ...
            <div class="mt-2 flex items-center gap-3">
              <span class="w-10 text-xs text-[var(--np-reader-muted)]"
                >{{ fontSize }}%</span
              >
              <mat-slider class="w-full" [min]="80" [max]="180" [step]="5">
                <input
                  matSliderThumb
                  [value]="fontSize"
                  (valueChange)="setFontSize.emit($event)"
                />
              </mat-slider>
            </div>
          </div>
            ...
            <div class="mt-2 flex items-center gap-3">
              ...
              <mat-slider class="w-full" [min]="1.2" [max]="2.2" [step]="0.1">
                <input
                  matSliderThumb
                  [value]="lineHeight"
                  (valueChange)="setLineHeight.emit($event)"
                />
              </mat-slider>
            </div>
          </div>
            ...
            <div class="mt-6">
              ...
              <div class="mt-3 flex items-center justify-between gap-3">
                <button
                  mat-stroked-button
                  type="button"
                  (click)="toggleAutoScroll.emit()"
                >
                  {{
                    autoScrollEnabled ? "Stop Auto-Scroll" : "Start Auto-Scroll"
                  }}
                </button>
                <span class="text-xs text-[var(--np-reader-muted)]"
              ...
              <div class="mt-2">
                <mat-slider class="w-full" [min]="4" [max]="80" [step]="2">
                  <input
                    ...
                    (valueChange)="setAutoScrollSpeed.emit($event)"
                  />
                </mat-slider>
              </div>
            </div>
          ...
          <div class="mt-6">
            ...
            <div class="mt-2 flex items-center gap-3">
              <span class="w-12 text-xs text-[var(--np-reader-muted)]"
                >{{ pdfZoom * 100 | number: "1.0-0" }}%</span
              >
              <mat-slider class="w-full" [min]="0.6" [max]="2.6" [step]="0.1">
                <input
                  matSliderThumb
                  [value]="pdfZoom"
                  (valueChange)="setPdfZoom.emit($event)"
                />
              </mat-slider>
            </div>
          </div>
        ...
        <div class="mt-6">
          ...
          <div class="mt-3 flex items-center gap-2">
            <button
              mat-stroked-button
              type="button"
              (click)="toggleHighlightMode.emit()"
            >
          ...
          <div class="mt-3 grid grid-cols-4 gap-2">
            <button
              mat-stroked-button
              type="button"
              (click)="setHighlightColor.emit('yellow')"
              [disabled]="highlightColor === 'yellow'"
            >
              Yellow
            </button>
            <button
              mat-stroked-button
              type="button"
              (click)="setHighlightColor.emit('green')"
              [disabled]="highlightColor === 'green'"
            >
              Green
            </button>
            <button
              mat-stroked-button
              type="button"
              (click)="setHighlightColor.emit('blue')"
              [disabled]="highlightColor === 'blue'"
            >
              Blue
            </button>
            <button
              mat-stroked-button
              type="button"
              (click)="setHighlightColor.emit('pink')"
              [disabled]="highlightColor === 'pink'"
            >
              Pink
            </button>
          </div>
          ...
          @if (highlights.length > 0) {
            <mat-nav-list class="mt-2">
              @for (hl of highlights; track hl.id) {
                <div
                  class="flex items-center justify-between gap-3 rounded px-2 py-1 hover:bg-black/5"
                >
                  <button
                    mat-button
                    type="button"
                    class="min-w-0 flex-1 justify-start"
                    (click)="goToHighlight.emit(hl)"
                  >
                  ...
                  <button
                    ...
                    (click)="deleteHighlight.emit(hl.id)"
                  >
            ...
            <p class="mt-2 text-xs text-[var(--np-reader-muted)]">
              {{
                highlightMode
                  ? "Select text (EPUB) or drag on page (PDF) to highlight."
                  : "No highlights yet."
              }}
            </p>
          }
              ...
              <div class="mt-2 flex items-center gap-3">
                ...
                <mat-slider class="w-full" [min]="0.6" [max]="1.6" [step]="0.1">
                  <input
                    matSliderThumb
                    [value]="ttsRate"
                    (valueChange)="ttsRateChange.emit($event)"
                  />
                </mat-slider>
              </div>
            </div>
        ...
        @if (toc.length === 0) {
          ...
          <mat-nav-list class="mt-2">
            @for (entry of toc; track entry.href) {
              <a
                mat-list-item
                (click)="goToHref.emit(entry.href)"
                class="cursor-pointer"
              >
                <span
                  matListItemTitle
                  class="text-sm"
                  [style.paddingLeft.px]="entry.level * 12"
                  >{{ entry.label }}</span
                >
              </a>
            }
