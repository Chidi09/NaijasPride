...
export class ReaderStorageService {
    ...
    const normalized: ReaderSettings = {
      flow:
        saved.flow === 'paginated' || saved.flow === 'scrolled'
          ...
          : defaults.flow,
      spread:
        saved.spread === 'auto' || saved.spread === 'single' || saved.spread === 'double'
          ? saved.spread
          : defaults.spread,
      theme:
        saved.theme === 'paper' || saved.theme === 'sepia' || saved.theme === 'night'
          ? saved.theme
          : defaults.theme,
      fontFamily:
        saved.fontFamily === 'serif' || saved.fontFamily === 'sans' || saved.fontFamily === 'mono'
          ? saved.fontFamily
          : defaults.fontFamily,
      fontSize:
      ...
      ttsRate:
        typeof saved.ttsRate === 'number'
          ...
          : defaults.ttsRate,
      ttsVoiceUri: typeof saved.ttsVoiceUri === 'string' ? saved.ttsVoiceUri : defaults.ttsVoiceUri,
    };
  ...
  loadEpubProgress(slug: string): EpubProgress | null {
    ...
    if (!cfi) return null;
    const at = typeof saved.at === 'number' && Number.isFinite(saved.at) ? saved.at : 0;
  ...
  loadBookmarks(slug: string): BookmarkEntry[] {
    ...
    if (!localStorage.getItem(this.BOOKMARKS_PREFIX_V2 + slug) && normalized.length > 0) {
  ...
  saveHighlights(slug: string, highlights: HighlightEntry[]): void {
    try {
      localStorage.setItem(this.HIGHLIGHTS_PREFIX_V1 + slug, JSON.stringify(highlights.slice(0, 1000)));
    } catch {
      // ignore
    }
  }
