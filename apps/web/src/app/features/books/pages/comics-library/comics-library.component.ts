...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterLink, FormsModule, MatButtonModule, MatCardModule, MatFormFieldModule, MatInputModule, SymbolIconComponent, TvFocusGroupDirective],
  template: `
      ...
      <div appTvFocusGroup [tvAutoFocus]="true" class="flex min-h-screen w-full overflow-hidden bg-[#090609] text-[#f6efe8] books-theme">
        ...
        <main class="flex-1 overflow-y-auto">
          <section class="relative min-h-[72vh] overflow-hidden">
            <div class="absolute inset-0 bg-[linear-gradient(135deg,#201117_0%,#10090d_48%,#050406_100%)]"></div>
            ...
            <div class="relative z-10 flex min-h-[72vh] max-w-5xl flex-col justify-center px-8 py-12 md:px-12 xl:px-20">
              <div class="flex items-center gap-3 text-xs uppercase tracking-[0.22em] text-white/55">
                <span class="rounded-full border border-[#d0a97a]/40 bg-[#d0a97a]/10 px-3 py-1 text-[#ecd8b7]">Comics Library</span>
                <span>ReadComicsOnline</span>
              </div>
              <h1 class="mt-5 text-5xl font-black leading-[0.95] text-white md:text-7xl">Western comics, graphic novels, and fresh issues.</h1>
              <p class="mt-5 max-w-2xl text-base leading-8 text-white/68">A shelf-first layout for desktop and TV, using your existing NaijasPride branding instead of the mockup palette.</p>

              <div class="mt-8 flex max-w-2xl flex-col gap-3 rounded-[1.75rem] border border-white/10 bg-white/[0.04] p-4 backdrop-blur-md md:flex-row md:items-center">
                <div class="flex flex-1 items-center gap-3 rounded-2xl bg-black/20 px-4 py-3">
                  ...
                  <input [(ngModel)]="query" (keyup.enter)="search()" class="w-full bg-transparent text-sm text-white outline-none placeholder:text-white/35" placeholder="Search comics" />
                </div>
                <div class="flex gap-3">
                  ...
                  <button type="button" (click)="clearSearch()" class="rounded-2xl border border-white/15 bg-white/[0.05] px-5 py-3 text-sm font-semibold text-white/80 transition hover:bg-white/[0.08]">Clear</button>
                </div>
              </div>
            </div>
          </section>
          ...
          <div class="space-y-12 px-8 pb-16 md:px-12 xl:px-20">
            ...
            <section>
              <div class="mb-5 flex items-center justify-between">
                <h2 class="text-2xl font-bold text-white">{{ searchResults().length > 0 ? 'Search Results' : 'Trending Comics' }}</h2>
                <a routerLink="/books" class="text-sm font-medium text-[#d0a97a] hover:text-[#ead9bf]">Back to Books</a>
              </div>
              <div class="flex gap-5 overflow-x-auto pb-2">
                @for (comic of visibleComics(); track comic.id) {
                  <a [routerLink]="['/books/comics', toRouteParam(comic.id)]" class="group block w-44 flex-shrink-0">
                    ...
                    <p class="truncate text-xs text-white/50">{{ comic.latestChapter ? 'Latest ' + comic.latestChapter : 'Graphic novel' }}</p>
                  </a>
                }
              </div>
            </section>
...
export class ComicsLibraryComponent {
  ...
  shelfNavItems = [
    ...
    { label: 'Comics', link: '/books/comics', icon: 'library_books', active: true },
  ...
  visibleComics(): ComicSummary[] {
    return (this.searchResults().length > 0 ? this.searchResults() : this.discoverTrending()).slice(0, 14);
  }
