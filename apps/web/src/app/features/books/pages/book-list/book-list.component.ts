    ...
    <div class="container mx-auto px-4 py-12 books-theme">
      <div class="flex flex-wrap items-start justify-between gap-4 mb-8">
        <div>
          ...
          <p class="text-[#8a756e] dark:text-gray-400 mt-2">
            Discover our collection of Nollywood books and magazines
          </p>
        </div>
        <div class="flex flex-wrap gap-2">
          <a mat-stroked-button color="primary" routerLink="/books">Hub</a>
          <a mat-stroked-button color="primary" routerLink="/books/light-novels"
            >Light Novels</a
          >
                ...
                <div class="np-cover-body">
                  ...
                  <div class="np-cover-meta">
                    ...
                    @if ((book.viewCount ?? 0) > 0) {
                      • {{ formatCompactCount(book.viewCount ?? 0) }} views
                    }
                  </div>
      ...
      @if (!isLoading() && books().length === 0) {
        <div class="text-center py-24 text-[#8a756e] dark:text-gray-400">
          <span class="material-symbols-outlined text-6xl" aria-hidden="true"
            >menu_book</span
          >
          <p class="text-lg font-serif text-[#24181b] dark:text-white mt-4">
            No books available yet.
          </p>
          <p class="text-[#9a857d] dark:text-gray-500">
            Check back soon for our growing library.
          </p>
        </div>

        @if (isTrendingLoading()) {
          <p
            class="text-sm text-[#9a857d] dark:text-gray-500 text-center -mt-10 mb-8"
          >
          ...
          <section class="mt-6">
            <div class="mb-4 flex items-center justify-between">
              <h2 class="text-xl font-serif text-[#24181b] dark:text-white">
                Trending Manga Right Now
              </h2>
              <a
                routerLink="/books/manga"
                class="text-sm text-[#d6b87a] hover:text-white"
                >Open Manga Library</a
              >
            </div>
            <div class="np-cover-grid">
              @for (manga of trendingManga(); track manga.id) {
                <mat-card class="np-cover-card">
                  <a
                    [routerLink]="['/books/manga', toRouteParam(manga.id)]"
                    class="np-cover-link"
                  >
                    <div class="np-cover-media">
                      @if (manga.coverUrl) {
...
export class BookListComponent {
  ...
  private resolveTrendingSourceId(): string {
    const savedSource = localStorage
      .getItem("np_manga_source")
      ?.trim()
      .toLowerCase();
  ...
  loadBooks() {
    ...
    this.http
      .get<{
        ...
        data: Book[];
        meta: PaginationMeta;
      }>(`/api/v1/books?page=${this.currentPage()}&kind=book`)
      .subscribe({
        next: (response) => {
  ...
  private isLightNovel(book: Book): boolean {
    ...
    if (genres.some((entry) => String(entry).toLowerCase() === "light novel"))
      return true;
