    ...
    @if (useEditorialShell()) {
      ...
      >
        ...
        <main class="flex-1 overflow-y-auto">
          <section class="relative min-h-[72vh] overflow-hidden">
            <div
              class="absolute inset-0 bg-[linear-gradient(135deg,#221014_0%,#12090d_48%,#050406_100%)]"
            ></div>
            <div
            ...
            >
              ...
              >
                Curated manga, manhwa, and manhua for desktop and TV.
              </h1>
              <p class="mt-5 max-w-2xl text-base leading-8 text-white/68">
                Same data, new presentation. Browse, switch sources, and pick up
                where you left off with a more premium shelf layout.
              </p>
              ...
              <section>
                <div class="mb-5 flex items-center justify-between">
                  <h2 class="text-2xl font-bold text-white">
                    {{ showHome() ? "Trending Now" : "Search Results" }}
                  </h2>
                  <span class="text-sm text-[#d0a97a]">{{
                    selectedSourceLabel()
                  }}</span>
                </div>
                <div class="flex gap-5 overflow-x-auto pb-2">
                  @for (entry of editorialItems(); track entry.id) {
                    <a
                      [routerLink]="[
                        detailRouteFor(entry.id),
                        toRouteParam(entry.id),
                      ]"
              ...
              <section>
                ...
                <div class="flex gap-5 overflow-x-auto pb-2">
                  @for (fav of favorites(); track fav.id) {
                    <a
                      [routerLink]="[
                        detailRouteFor(fav.mangaId),
                        toRouteParam(fav.mangaId),
                      ]"
                      class="group block w-44 flex-shrink-0"
                    >
                      <div
              ...
              <section>
                ...
                <div class="space-y-3">
                  @for (item of history(); track item.id) {
                    <div
                      class="rounded-[1.5rem] border border-white/10 bg-white/[0.04] p-4"
                    >
                      <div
                      ...
                      >
                        <div>
                          <p class="text-sm font-semibold text-white">
                            Chapter {{ item.chapterId.slice(0, 10) }}...
                          </p>
                        ...
                        <a
                          [routerLink]="[
                            readerRouteFor(item.chapterId),
                            toRouteParam(item.chapterId),
                          ]"
                          [queryParams]="{ mangaId: item.mangaId }"
                          class="rounded-2xl bg-[#800020] px-4 py-2 text-sm font-semibold text-white"
                          >Continue</a
                        >
                      </div>
                    </div>
      ...
      <div class="container mx-auto px-4 py-10 books-theme">
        ...
        <div class="mb-6 flex flex-wrap items-start justify-between gap-4">
          <div class="min-w-0">
            <h1
              class="text-3xl md:text-4xl font-serif text-[var(--text-primary)]"
            >
              Manga Library
            </h1>
            <p class="mt-1 text-sm text-[var(--text-muted)]">
              Manga, Manhwa and Manhua from multiple sources.
            </p>
          </div>
          <div class="flex gap-2">
            <a mat-stroked-button color="primary" routerLink="/books/comics"
              >Comics</a
            >
        ...
        >
          ...
          <div class="flex flex-col gap-3 sm:flex-row sm:items-end">
            ...
            >
              ...
              @if (hasActiveFilters()) {
                <span
                  class="ml-1 inline-flex h-4 w-4 items-center justify-center rounded-full bg-[var(--cinema-500)] text-[10px] text-white"
                  >!</span
                >
              }
            </button>
          ...
          @if (showFilters()) {
            <div class="mt-4 border-t border-[var(--border-color)] pt-4">
              <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                ...
                >
                  ...
                  >
                    ...
                    <mat-option value="latestUploadedChapter"
                      >Latest Updates</mat-option
                    >
                ...
                >
                  <mat-select
                    [ngModel]="originalLanguage()"
                    (ngModelChange)="originalLanguage.set($event)"
                    aria-label="Language"
                  >
                ...
                >
                  <mat-select
                    [ngModel]="demographic()"
                    (ngModelChange)="demographic.set($event)"
                ...
                >
                  <mat-select
                    ...
                    (ngModelChange)="contentRating.set($event)"
                ...
                >
                  <input
                    ...
                    type="number"
                    [ngModel]="year()"
                    (ngModelChange)="year.set($event || null)"
              ...
              @if (groupedTags().length) {
                <div class="mt-3">
                  <p
                    class="mb-2 text-[11px] uppercase tracking-wider text-[var(--text-muted)]"
                  >
                    Tags
                  </p>
                  <div
                    class="max-h-52 overflow-auto rounded-xl border border-[var(--border-color)] p-3"
                    style="background: var(--bg-secondary)"
                  >
                    @for (group of groupedTags(); track group.group) {
                      <div class="mb-3">
                        <p
                          class="mb-1 text-[11px] uppercase tracking-wide text-[var(--accent)]"
                        >
                          {{ group.group }}
                        </p>
                        <mat-chip-listbox>
        ...
        @if (isSwitchingSource()) {
          ...
          <mat-tab-group
            [selectedIndex]="
              activeTab() === 'search' ? 0 : activeTab() === 'favorites' ? 1 : 2
            "
                ...
                @if (showHome()) {
                    ...
                    @if (isDiscoverLoading()) {
                      ...
                      @if (discover(); as d) {
                            ...
                            <div class="np-cover-grid">
                              @for (manga of d.trending; track manga.id) {
                                <mat-card class="np-cover-card">
                                  <a
                                    [routerLink]="[
                                      detailRouteFor(manga.id),
                                      toRouteParam(manga.id),
                                    ]"
                                    class="np-cover-link"
                                  >
                        ...
                        @if (
                          ...
                          >
                            <p class="text-sm text-[var(--text-muted)]">
                              No discover sections available for this source.
                        ...
                        >
                          <p class="text-sm text-[var(--text-muted)]">
                            Start searching to find manga, or switch sources to
                  ...
                  <div class="mt-6">
                    ...
                    @if (
                      ...
                      >
                        <p class="text-sm text-[var(--text-muted)]">
                          No manga found. Try a different search or loosen
                          filters.
                        </p>
                      </mat-card>
                    }
                        ...
                        <mat-card class="np-cover-card">
                          ...
                          <div class="px-3 pb-3">
                            <button
                              ...
                              type="button"
                              (click)="
                                toggleFavorite(manga); $event.stopPropagation()
                              "
                            >
                              <span class="flex items-center gap-2">
                                <app-star-icon
                                  [size]="16"
                                  [filled]="isFavorite(manga.id)"
                                  fillColor="#fbbf24"
                                  strokeColor="#fbbf24"
                                />
                                {{
                  ...
                  @for (item of history(); track item.id) {
                    ...
                    >
                      ...
                      >
                        <div>
                          <p
                            class="text-sm text-[var(--text-primary)] font-medium"
                          >
                            Chapter {{ item.chapterId.slice(0, 10) }}...
                          </p>
...
export class MangaLibraryComponent implements OnInit {
  ...
  selectedSourceLabel() {
    return (
      this.sources().find((source) => source.id === this.selectedSource())
        ?.displayName || this.selectedSource()
    );
  }
  ...
  private buildSearchQuery() {
    ...
    if (this.originalLanguage())
      params.append("originalLanguage", this.originalLanguage());
    if (this.contentRating())
      params.append("contentRating", this.contentRating());
  ...
  detailRouteFor(entityId: string) {
    return this.extractSource(entityId) === "readcomicsonline"
      ? "/books/comics"
      : "/books/manga";
  }

  readerRouteFor(chapterId: string) {
    return this.extractSource(chapterId) === "readcomicsonline"
      ? "/books/comics/read"
      : "/books/manga/read";
  }
  ...
  private loadSources() {
    this.http
      .get<{
        status: string;
        data: MangaSource[];
      }>("/api/v1/books/manga/sources")
      .subscribe({
        next: (response) => {
          const available = response.data || [];
          this.sources.set(available);
          if (
            !available.find((source) => source.id === this.selectedSource()) &&
            available.length > 0
          ) {
            const preferred = ["weebcentral", "asura", "mangadex"];
            const selected =
              preferred.find((sourceId) =>
                available.some((source) => source.id === sourceId),
              ) || available[0].id;
  ...
  private loadSourceHealth() {
    this.http
      .get<{
        status: string;
        data: MangaSourceHealth[];
      }>("/api/v1/books/manga/sources/health")
      .subscribe({
  ...
  search() {
    ...
    this.http
      .get<{
        status: string;
        data: MangaSummary[];
      }>(`${this.sourcePath()}/search${query ? `?${query}` : ""}`)
      .subscribe({
        next: (response) => {
  ...
  toggleTag(tagId: string) {
    this.selectedTagIds.update((items) =>
      items.includes(tagId)
        ? items.filter((id) => id !== tagId)
        : [...items, tagId],
    );
  }
  ...
  loadTags() {
    this.http
      .get<{ status: string; data: MangaTag[] }>(`${this.sourcePath()}/tags`)
      .subscribe({
        next: (response) => {
  ...
  loadDiscover() {
    ...
    this.http
      .get<{
        status: string;
        data: MangaDiscoverPayload;
      }>(`${this.sourcePath()}/discover?limit=10`)
      .subscribe({
        next: (response) => {
    ...
    this.http.delete(`/api/v1/books/manga/favorites/${mangaId}`).subscribe({
      next: () => {
        ...
        this.favorites.update((list) =>
          list.filter((item) => item.mangaId !== mangaId),
        );
      },
    });
  }
  ...
  loadFavorites() {
    ...
    this.http
      .get<{
        status: string;
        data: MangaFavorite[];
      }>("/api/v1/books/manga/favorites")
      .subscribe({
        next: (response) => {
          this.favorites.set(response.data);
          this.favoriteIds.set(
            new Set(response.data.map((item) => item.mangaId)),
          );
        },
        error: () => {
          this.favorites.set([]);
  ...
  loadHistory() {
    ...
    this.http
      .get<{
        status: string;
        data: ReadingHistory[];
      }>("/api/v1/books/manga/history?limit=50")
      .subscribe({
        next: (response) => {
  ...
  removeHistoryEntry(chapterId: string) {
    ...
    this.http
      .delete(`/api/v1/books/manga/history/${encodeURIComponent(chapterId)}`)
      .subscribe({
        next: () => {
