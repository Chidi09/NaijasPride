...
const parseSourceEntityId = (
  entityId: string,
): { sourceId: string; rawId: string } | null => {
  ...
  template: `
    @if (useEditorialShell()) {
      <div
        appTvFocusGroup
        [tvAutoFocus]="true"
        class="min-h-screen overflow-x-hidden bg-[#090609] text-[#f6efe8] books-theme"
      >
        @if (detail(); as manga) {
          <section class="relative min-h-[76vh] overflow-hidden">
            <div
              class="absolute inset-0 bg-cover bg-center"
              [style.background-image]="heroBackdrop(manga)"
            ></div>
            <div
              class="absolute inset-0 bg-[linear-gradient(90deg,rgba(9,6,9,0.96)_0%,rgba(9,6,9,0.72)_42%,rgba(9,6,9,0.18)_100%),linear-gradient(0deg,rgba(9,6,9,1)_0%,rgba(9,6,9,0.38)_45%,rgba(9,6,9,0)_100%)]"
            ></div>

            <header
              class="relative z-10 flex items-center justify-between px-8 py-8 md:px-12 xl:px-16"
            >
              <a
                [routerLink]="libraryRootPath()"
                class="inline-flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-3 text-sm font-medium text-white/80 transition hover:bg-white/[0.08] hover:text-white"
              >
                <app-symbol-icon
                  name="arrow_back"
                  [size]="22"
            ...
            <div
              class="relative z-10 flex min-h-[76vh] flex-col justify-end px-8 pb-12 md:px-12 xl:px-16"
            >
              <div class="max-w-5xl">
                <div
                ...
                <div
                  class="mt-4 flex flex-wrap items-center gap-6 text-sm text-white/65"
                >
                  @if (manga.author) {
                  ...
                  @if (manga.tags?.length) {
                    <span class="inline-flex items-center gap-2"
                      ><app-symbol-icon
                        ...
                        [size]="18"
                      ></app-symbol-icon
                      >{{ manga.tags.slice(0, 3).join(", ") }}</span
                    >
                  }
                  <span class="inline-flex items-center gap-2"
                    ><app-symbol-icon
                ...
                <div class="mt-8 flex flex-wrap gap-4">
                  @if (continueReadingChapterId(); as contId) {
                    <a
                      ...
                      [queryParams]="{
                        title: manga.title,
                        chapter: continueReadingChapterLabel() || '',
                        mangaId: manga.id,
                      }"
                  ...
                  @if (
                    ...
                    <button
                      ...
                      [ngClass]="
                        library.isWatchingManga(currentMangaId())
                          ? 'text-[#800020]'
                          : 'text-white/90'
                      "
                      [title]="
                        library.isWatchingManga(currentMangaId())
                          ? 'Stop chapter notifications'
                          : 'Notify me when new chapters drop'
                      "
                    >
                      <app-symbol-icon
                        [name]="
                        ...
                        "
                        [size]="24"
                        [fill]="library.isWatchingManga(currentMangaId())"
                      ></app-symbol-icon>
                      {{
                        library.isWatchingManga(currentMangaId())
                          ? "Notifying"
            ...
            <section>
              <div class="mb-5 flex items-center justify-between">
                ...
                <div class="flex gap-2">
                  <button
                    type="button"
                    class="rounded-full border border-white/15 bg-white/[0.04] p-2 text-white/70 transition hover:bg-white/[0.08]"
              ...
              <div class="flex gap-5 overflow-x-auto pb-3">
                @for (
                  chapter of visibleChapters().slice(0, 18);
                  track chapter.id
                ) {
                  @if (chapter.isExternal && chapter.externalUrl) {
                    <a
                      ...
                      class="group block w-56 flex-shrink-0"
                    >
                      <div
                        class="rounded-[1.6rem] border border-white/10 bg-white/[0.04] p-5 transition hover:bg-white/[0.06]"
                      >
                        <div
                          class="flex h-32 items-center justify-center rounded-2xl bg-black/20 text-white/60"
                        >
                          <app-symbol-icon
                            name="open_in_new"
                            [size]="32"
                        ...
                        <p
                          class="mt-4 line-clamp-2 text-sm font-semibold text-white"
                        >
                    ...
                    >
                      ...
                      >
                        ...
                        <p class="mt-1 text-xs text-white/45">
                          {{
                            chapter.publishedAt
                              ? (chapter.publishedAt | date: "MMM d, y")
                              : "Tap to read"
                          }}
                        </p>
                      </div>
              ...
              <section>
                ...
                <div class="flex gap-5 overflow-x-auto pb-3">
                  @for (item of similar().slice(0, 12); track item.id) {
                    <a
                      [routerLink]="[detailBasePath(), toRouteParam(item.id)]"
      ...
      <div class="container mx-auto px-4 py-10 books-theme">
        ...
        @if (isLoading()) {
          <section class="grid gap-6 md:grid-cols-[280px_1fr]">
            ...
            <mat-card class="np-surface-card p-6">
              <div class="space-y-3">
                <div
                  class="h-8 w-3/4 animate-pulse rounded bg-[#e5d2c6] dark:bg-cinema-800"
                ></div>
                <div
                  class="h-4 w-2/3 animate-pulse rounded bg-[#e5d2c6] dark:bg-cinema-800"
                ></div>
                <div
                  class="h-4 w-full animate-pulse rounded bg-[#e5d2c6] dark:bg-cinema-800"
                ></div>
                <div
                  class="h-4 w-5/6 animate-pulse rounded bg-[#e5d2c6] dark:bg-cinema-800"
                ></div>
                <div
                  class="h-4 w-4/6 animate-pulse rounded bg-[#e5d2c6] dark:bg-cinema-800"
                ></div>
              </div>
            </mat-card>
          </section>
        ...
        @if (detail(); as manga) {
          <section class="mb-8 grid gap-6 md:grid-cols-[280px_1fr]">
            ...
            <div class="min-w-0">
              <h1
                class="text-2xl font-semibold text-[var(--text-primary)] md:text-3xl"
              >
                {{ manga.title }}
              </h1>
              ...
              <mat-chip-set class="mt-3" aria-label="Manga metadata">
                ...
                @if (manga.status) {
                  <mat-chip>{{ manga.status }}</mat-chip>
                }
                @if (manga.year) {
                  <mat-chip>{{ manga.year }}</mat-chip>
                }
                @if (manga.originalLanguage) {
                  <mat-chip>{{ manga.originalLanguage }}</mat-chip>
                }
                @if (manga.contentRating) {
                  <mat-chip>{{ manga.contentRating }}</mat-chip>
                }
                @if (manga.publicationDemographic) {
                  <mat-chip>{{ manga.publicationDemographic }}</mat-chip>
                }
              </mat-chip-set>
              ...
              <mat-card class="np-surface-card mt-4 p-4">
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  ...
                  <div>
                    ...
                    <p class="text-sm text-[var(--text-primary)]">
                      {{ isChaptersLoading() ? "Loading…" : chapters().length }}
                    </p>
                  </div>
                  @if (manga.availableTranslatedLanguages?.length) {
                    <div class="sm:col-span-2 lg:col-span-2">
                      ...
                      <p class="text-sm text-[var(--text-primary)]">
                        {{ manga.availableTranslatedLanguages.length }}
                        available
                      </p>
                    </div>
                  }
              ...
              <mat-card class="np-surface-card mt-4 p-4">
                ...
                <p
                  class="mt-2 text-sm text-[var(--text-secondary)] leading-relaxed"
                >
                  {{ manga.description || "No description available." }}
                </p>
              </mat-card>
              ...
              <div class="mt-5 flex flex-wrap gap-2 items-center">
                ...
                @if (auth.currentUser()) {
                  <button
                    ...
                    [title]="
                      library.isWatchingManga(currentMangaId())
                        ? 'Stop notifications for new chapters'
                        : 'Notify me when new chapters drop'
                    "
                  >
                    {{
                ...
                @if (
                  mangaOffline.downloadedChapterCount(currentMangaId()) > 0
                ) {
                  <span
                    class="text-xs text-green-500 border border-green-500/30 px-2 py-1 rounded"
                  >
                    {{ mangaOffline.downloadedChapterCount(currentMangaId()) }}
                    chapter(s) offline
                  </span>
                }
              </div>
          ...
          <mat-card class="np-surface-card mb-8 p-4">
              ...
              <div>
                ...
                <p class="mt-1 text-xs text-[var(--text-muted)]">
                  @if (isChaptersLoading()) {
                    ...
                    Showing {{ visibleFilteredChapterCount() }} of
            ...
            @if (hasExternalChapters()) {
              <div
                class="mb-3 rounded border border-amber-700/50 bg-amber-900/20 px-3 py-2 text-xs text-amber-700 dark:text-amber-300"
              >
                Some chapters are hosted externally and will open on their
                original source.
              </div>
            }
            @if (!isChaptersLoading() && chapters().length === 0) {
              <p class="text-sm text-[#8a756e] dark:text-gray-400">
                No chapters found.
              </p>
            }
            @if (isChaptersLoading()) {
              <div class="space-y-2">
                @for (_ of [1, 2, 3, 4, 5]; track _) {
                  <div
                    class="h-14 animate-pulse rounded border border-[#d8c2b8] dark:border-zinc-800 bg-[#e5d2c6] dark:bg-zinc-900/60"
                  ></div>
                }
              </div>
            }
            @if (!isChaptersLoading() && chapters().length > 0) {
              <div class="np-chapters-scroll space-y-2">
                <div class="np-chapters-toolbar">
                        ...
                        >
                          ...
                          >
                            ...
                            @if (detail(); as current) {
                              @for (
                                lang of current.availableTranslatedLanguages;
                                track lang
                              ) {
                  ...
                  <div
                    class="mt-2 flex items-center justify-between gap-2 text-[11px] uppercase tracking-wide text-[var(--text-muted)]"
                  >
                    <span
                      >Order:
                      {{
                  ...
                  @if (chapter.isExternal && chapter.externalUrl) {
                    ...
                    >
                      <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0">
                          <div class="np-chapter-title">
                            ...
                            @if (chapterSubtitle(chapter)) {
                              <span class="np-chapter-subtitle">{{
                        ...
                        <span
                          class="shrink-0 rounded border border-amber-600/50 px-2 py-0.5 text-[11px] text-amber-700 dark:text-amber-300"
                          >Open</span
                        >
                      </div>
                    </a>
                          ...
                          <div class="min-w-0">
                            <div class="np-chapter-title">
                              ...
                              @if (mangaOffline.isAvailable(chapter.id)) {
                                <span class="ml-1 text-[10px] text-green-500"
                                  >● offline</span
                                >
                              }
                              @if (chapterSubtitle(chapter)) {
                            ...
                            @if (
                              mangaOffline.getStatus(chapter.id) ===
                                "downloading" ||
                              mangaOffline.getStatus(chapter.id) === "queued"
                            ) {
                              <div
                                class="mt-1 h-0.5 w-full bg-[#d9c4b7] dark:bg-white/10 rounded overflow-hidden"
                              >
                                <div
                                  class="h-full bg-cinema-500 transition-all"
                                  [style.width.%]="
                                    mangaOffline.getProgress(chapter.id)
                                  "
                                ></div>
                              </div>
                            }
                            @if (
                              mangaOffline.getStatus(chapter.id) === "error"
                            ) {
                      ...
                      @if (mangaOffline.isSupported) {
                        @if (mangaOffline.isAvailable(chapter.id)) {
                          <button
                            ...
                            (click)="
                              removeChapterOffline(chapter.id);
                          ...
                          <span
                            class="shrink-0 w-4 h-4 border-2 border-cinema-400/30 border-t-cinema-400 rounded-full animate-spin inline-block"
                          ></span>
                        } @else {
                          <button
                            type="button"
          ...
          @if (supportsSimilar()) {
            <section>
              <h2 class="mb-4 text-lg font-semibold text-[var(--text-primary)]">
                Similar {{ isComicsMode() ? "Comics" : "Manga" }}
              </h2>
              @if (isSimilarLoading()) {
        ...
        @if (showBackToTop()) {
          <button
            ...
            type="button"
            (click)="scrollToTop()"
            class="fixed bottom-6 right-5 z-20"
          >
            Top
          </button>
        }
...
export class MangaDetailComponent implements OnInit {
  ...
  visibleChapters = computed(() =>
    this.filteredChapters().slice(0, this.visibleChapterCount()),
  );
  hasMoreChapters = computed(
    () => this.filteredChapters().length > this.visibleChapterCount(),
  );
  filteredChapterCount = computed(() => this.filteredChapters().length);
  visibleFilteredChapterCount = computed(() =>
    Math.min(this.visibleChapterCount(), this.filteredChapters().length),
  );
  hasExternalChapters = computed(() =>
    this.chapters().some((chapter) => chapter.isExternal),
  );
  ...
  private resolveLibraryMode() {
    ...
    this.libraryMode.set(
      routePath.startsWith("books/comics") ? "comics" : "manga",
    );
  }
  ...
  toggleChapterOrder() {
    this.chapterOrder.update((current) =>
  ...
  private loadChapters(mangaId: string) {
    ...
    this.http
      .get<{ status: string; data: MangaChapter[] }>(endpoint)
      .subscribe({
        next: (response) => {
  ...
  private loadData(mangaId: string) {
    ...
    this.http
      .get<{ status: string; data: MangaDetail }>(detailEndpoint)
      .subscribe({
        next: (response) => {
    ...
    if (parsed.sourceId === "mangadex") {
      ...
      this.http
        .get<{ status: string; data: MangaSummary[] }>(similarEndpoint)
        .subscribe({
          next: (response) => {
    ...
    if (this.isAuthenticated()) {
      this.http
        ...
        }>(`/api/v1/books/manga/favorites/${mangaId}/check`)
        .subscribe({
      ...
      this.library.loadChapterWatches().catch(() => {
        /* ignore */
      });
    } else {
      this.favorite.set(false);
  ...
  toggleFavorite() {
    ...
    this.http
      .post("/api/v1/books/manga/favorites", {
        mangaId: manga.id,
        title: manga.title,
  ...
  downloadChapter(
    ...
    this.http
      .get<{ status: string; data: { pages: string[] } }>(endpoint)
      .subscribe({
        next: (res) => {
        ...
        error: (err) =>
          console.error(
            "[MangaDetail] Failed to fetch pages for offline download",
            err,
          ),
      });
  }
  ...
  chapterSubtitle(chapter: MangaChapter): string | null {
    ...
    if (
      chapterPatterns.some(
        (pattern) => normalizedTitle === pattern.toLowerCase(),
      )
    ) {
      return null;
    }
