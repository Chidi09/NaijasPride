...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-[#f7efe8] dark:bg-cinema-900 flex items-center justify-center p-4">
      <div class="max-w-md w-full text-center space-y-6">

        @if (status === 'verifying') {
          <div class="w-16 h-16 border-4 border-cinema-500/30 border-t-cinema-500 rounded-full animate-spin mx-auto"></div>
          <p class="text-[#725f58] dark:text-gray-400">Verifying your payment...</p>
        }
        ...
        @if (status === 'success') {
          ...
          <a routerLink="/browse" class="inline-block bg-cinema-500 hover:bg-cinema-400 text-white font-bold px-8 py-3 rounded-full transition-colors">
            Start Watching
          </a>
        }
        ...
        @if (status === 'failed') {
          ...
          <h1 class="text-2xl font-serif font-bold text-[#24181b] dark:text-white">Payment Not Completed</h1>
          <p class="text-[#725f58] dark:text-gray-400">
            {{ errorMessage || 'Your payment could not be verified. If you were charged, please contact support.' }}
          </p>
          <div class="flex gap-4 justify-center">
            ...
            <a routerLink="/browse" class="inline-block border border-cinema-500/60 text-cinema-500 font-bold px-6 py-3 rounded-full hover:bg-cinema-500/10 transition-colors">
              Go to Browse
            </a>
          </div>
        }
    ...
    this.http.post('/api/v1/payments/verify', { reference }).subscribe({
      next: (res: any) => {
        if (res?.success) {
          ...
          setTimeout(() => this.router.navigate(['/browse']), 3000);
        } else {
          this.status = 'failed';
