...
@Component({
  ...
  template: `
    <div appTvFocusGroup [tvAutoFocus]="true" class="tv-stage flex h-screen w-full overflow-hidden bg-[#080608] text-[#f6efe8]">
        ...
        <div class="mt-auto rounded-[2rem] border border-[#800020]/30 bg-[#800020]/10 p-4">
          ...
          <div class="mt-3 flex items-center gap-3">
            ...
            <div class="hidden xl:block min-w-0">
              ...
              <p class="truncate text-xs text-white/50">{{ membershipLabel() }}</p>
            </div>
          </div>
        </div>
      </aside>
      ...
      <main class="flex-1 overflow-y-auto">
        <section class="relative min-h-[66vh] overflow-hidden border-b border-white/10">
          <div class="absolute inset-0 bg-cover bg-center" [style.background-image]="heroBackground()"></div>
          <div class="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(208,169,122,0.18),transparent_25%),linear-gradient(90deg,rgba(8,6,8,0.96)_0%,rgba(8,6,8,0.74)_42%,rgba(8,6,8,0.16)_100%)]"></div>
          <div class="relative z-10 flex min-h-[66vh] flex-col justify-center px-8 py-10 md:px-12 xl:px-20">
            <div class="mb-4 flex flex-wrap items-center gap-3 text-xs uppercase tracking-[0.24em] text-white/60">
              <span class="rounded-full border border-[#d0a97a]/40 bg-[#d0a97a]/10 px-3 py-1 text-[#ecd8b7]">Featured Tonight</span>
              @if (heroMetaPrimary(); as meta) {
                <span>{{ meta }}</span>
              }
            </div>

            <h1 class="max-w-4xl text-4xl font-black leading-[0.95] text-white md:text-6xl xl:text-7xl">{{ heroTitle() }}</h1>
            <p class="mt-5 max-w-2xl text-sm leading-7 text-white/70 md:text-base">{{ heroDescription() }}</p>
            ...
            <div class="mt-8 flex flex-wrap items-center gap-4">
              ...
              >
                <app-symbol-icon name="play_arrow" [size]="30" [fill]="true"></app-symbol-icon>
                Play Now
              </a>
              <a
        ...
        <div class="space-y-10 px-8 py-8 md:px-12 xl:px-20 xl:py-10">
          <section>
            <div class="mb-5 flex items-center justify-between">
              ...
              <a routerLink="/library" class="text-sm font-medium text-[#d0a97a] hover:text-[#ead9bf]">Open Library</a>
            </div>
                ...
                <a [routerLink]="['/movies', item.movie.slug || item.movie.id]" class="group block w-72 flex-shrink-0">
                  <div class="relative aspect-video overflow-hidden rounded-[1.6rem] border border-white/10 bg-white/[0.04]">
                    ...
                    <div class="absolute inset-x-0 bottom-0 h-1.5 bg-white/10">
                      <div class="h-full bg-[#d0a97a]" [style.width.%]="item.progressPercentage"></div>
                    </div>
                    <div class="absolute bottom-4 left-4 right-4 flex items-end justify-between gap-3">
                      <div class="min-w-0">
                        <p class="truncate text-lg font-semibold text-white">{{ item.movie.title }}</p>
                        <p class="text-xs uppercase tracking-[0.2em] text-white/55">{{ item.progressPercentage | number:'1.0-0' }}% watched</p>
                      </div>
                      <span class="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#800020]/85 text-white shadow-lg">
                        <app-symbol-icon name="play_arrow" [size]="28" [fill]="true"></app-symbol-icon>
                      </span>
                    </div>
                  </div>
                </a>
          ...
          <section>
            <div class="mb-5 flex items-center justify-between">
              ...
              <a routerLink="/movies" class="text-sm font-medium text-[#d0a97a] hover:text-[#ead9bf]">Browse Movies</a>
            </div>
            <div class="flex gap-5 overflow-x-auto pb-2">
                ...
                <a [routerLink]="['/movies', movie.slug || movie.id]" class="group block w-52 flex-shrink-0">
                  <div class="relative aspect-[2/3] overflow-hidden rounded-[1.6rem] border border-white/10 bg-white/[0.04]">
                    ...
                    <div class="absolute bottom-4 left-4 right-4">
                      <p class="truncate text-base font-semibold text-white">{{ movie.title }}</p>
                      <p class="text-xs uppercase tracking-[0.2em] text-white/55">{{ movie.year || 'Featured' }}</p>
                    </div>
                  </div>
                </a>
              }
          ...
          <div class="grid gap-10 xl:grid-cols-[1.15fr,0.85fr]">
            <section>
              <div class="mb-5 flex items-center justify-between">
                ...
                <a routerLink="/anime" class="text-sm font-medium text-[#d0a97a] hover:text-[#ead9bf]">Open Anime</a>
              </div>
              <div class="grid grid-cols-2 gap-4 md:grid-cols-4">
                @for (entry of trendingAnime().slice(0, 4); track entry.id) {
                  <a [routerLink]="['/anime', entry.id]" class="group block overflow-hidden rounded-3xl border border-white/10 bg-white/[0.04]">
                    <div class="aspect-[3/4] overflow-hidden bg-black/30">
                      <img [src]="entry.coverImage?.extraLarge || entry.coverImage?.large || ''" [alt]="animeTitle(entry)" class="h-full w-full object-cover transition duration-500 group-hover:scale-105" referrerpolicy="no-referrer" />
                    </div>
                    <div class="space-y-1 p-4">
                      <p class="truncate text-sm font-semibold text-white">{{ animeTitle(entry) }}</p>
                      <p class="truncate text-[11px] uppercase tracking-[0.18em] text-white/50">{{ (entry.genres || []).slice(0, 2).join(' / ') || 'Anime' }}</p>
                    </div>
                  </a>
                }
              </div>
            ...
            <section>
              <div class="mb-5 flex items-center justify-between">
                ...
                <a routerLink="/music" class="text-sm font-medium text-[#d0a97a] hover:text-[#ead9bf]">Open Music</a>
              </div>
              <div class="space-y-3">
                @for (item of streamMovies().slice(0, 4); track item.id) {
                  <a [routerLink]="['/movies', item.slug || item.id]" class="group flex items-center gap-4 rounded-3xl border border-white/10 bg-white/[0.04] p-3 transition hover:bg-white/[0.06]">
                    <div class="relative aspect-video w-40 overflow-hidden rounded-2xl bg-black/30">
                      <img [src]="item.thumbnailUrl || item.posterUrl || ''" [alt]="item.title" class="h-full w-full object-cover transition duration-500 group-hover:scale-105" referrerpolicy="no-referrer" />
                      <div class="absolute inset-0 flex items-center justify-center bg-black/20">
                        <span class="flex h-12 w-12 items-center justify-center rounded-2xl bg-black/50 text-white backdrop-blur-sm">
                          <app-symbol-icon name="play_arrow" [size]="28" [fill]="true"></app-symbol-icon>
                        </span>
                      </div>
                    </div>
                    <div class="min-w-0 flex-1">
                      ...
                      <p class="truncate text-xs uppercase tracking-[0.18em] text-white/50">{{ item.year || 'Stream now' }}</p>
                    </div>
                  </a>
                }
              </div>
...
export class TvHomeExperienceComponent {
  ...
  featuredMovie = computed(() => this.downloadMovies()[0] || this.continueWatching()[0]?.movie || null);

  heroTitle = computed(() => this.featuredMovie()?.title || 'Your culture capital, now TV-first.');
  heroDescription = computed(() => {
    const movie = this.featuredMovie() as (MovieSummary & { description?: string | null }) | null;
  ...
  heroBackground = computed(() => {
    const movie = this.featuredMovie() as (MovieSummary & { backdropUrl?: string | null }) | null;
    const image = movie?.backdropUrl || movie?.posterUrl || movie?.thumbnailUrl || '';
  ...
  heroMetaPrimary = computed(() => {
    ...
    return [movie?.year].filter(Boolean).join(' • ') || 'Premium big-screen mode';
  });
  ...
  animeTitle(entry: AnimeCard): string {
    return entry.title?.english || entry.title?.romaji || entry.title?.native || 'Anime';
  }
}
