  ...
  template: `
    @if (pwaService.isTV()) {
      <div
        appTvFocusGroup
        [tvAutoFocus]="true"
        class="flex h-screen overflow-hidden bg-[#090609] text-[#f6efe8]"
      >
        <aside
        ...
        <main class="flex-1 overflow-y-auto px-8 py-8 md:px-12 xl:px-16">
          <header class="mb-8 flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <p class="mt-3 max-w-2xl text-sm text-white/55">
                Your movies, books, manga progress, and continue-watching picks
                are arranged for the big screen.
              </p>
            </div>
            <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
              ...
              >
                ...
                <p class="mt-2 text-2xl font-black text-white">
                  {{
                    (summary()?.offlineMangaCount || 0) +
                      (summary()?.offlineBookCount || 0)
                  }}
                </p>
              </div>
            </div>
          ...
          <section>
            ...
            <div
              class="grid grid-cols-2 gap-5 md:grid-cols-3 xl:grid-cols-5 2xl:grid-cols-6"
            >
              @for (item of tvSavedItems(); track item.key) {
                <a [routerLink]="item.link" class="group block">
                  <div
                    class="relative aspect-[2/3] overflow-hidden rounded-[1.6rem] border border-white/10 bg-white/[0.04] shadow-[0_12px_40px_rgba(0,0,0,0.24)]"
                  >
                    @if (item.coverUrl) {
                      <img
                        [src]="item.coverUrl"
                    ...
                    ></div>
                    <div
                      class="absolute left-4 top-4 rounded-full bg-[#800020]/85 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.22em] text-white"
                    >
                      {{ item.badge }}
                    </div>
      ...
      <div class="page-body px-4 py-8 md:px-8 lg:px-10 max-w-[1400px] mx-auto">
        ...
        <div class="mb-8 flex flex-col gap-1">
          <h1 class="text-3xl font-extrabold tracking-tight text-[#f9f9f2]">
            Your Media Universe
          </h1>
          <p class="text-sm text-[#a88a78] max-w-xl">
            ...
            @if (!isLoading()) {
              ...
              <span class="text-[#800020] font-semibold"
                >{{ watchlistCount() }} watchlisted</span
              >
              and
              <span class="text-[#800020] font-semibold"
                >{{
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
          ...
          >
            <!-- Thumbnail area -->
            <div
              class="relative w-full aspect-[4/3] rounded-xl overflow-hidden bg-[#181818] card-img-wrap"
            >
              ...
              ></div>
              <div
                class="absolute top-2.5 left-2.5 bg-[#800020]/90 text-white text-[9px] font-black px-2 py-1 rounded tracking-widest uppercase"
              >
                Watchlist
              </div>
            </div>
            ...
            <div>
              <div class="flex items-start justify-between">
                <h3
                  class="text-base font-bold text-[#f9f9f2] group-hover:text-[#c0304a] transition-colors leading-tight"
                >
                  Movie Watchlist
                </h3>
                <svg
                  class="h-5 w-5 text-[#800020] flex-shrink-0 mt-0.5"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              <div class="flex items-center gap-2 mt-2">
                @if (isLoading()) {
                  <div
                    class="h-4 w-16 animate-pulse rounded bg-[#1e1e1e]"
                  ></div>
                } @else {
                  <span
                    class="px-2 py-0.5 rounded bg-white/8 text-[#f9f9f2] text-[10px] font-bold uppercase"
                    >{{ watchlistCount() }} ITEMS</span
                  >
                }
              </div>
              <p class="text-[#a88a78] text-xs mt-2 line-clamp-1">
            ...
            <div>
              ...
              <div class="flex items-center gap-2 mt-2">
                @if (isLoading()) {
                  ...
                  <span
                    class="px-2 py-0.5 rounded bg-white/8 text-[#f9f9f2] text-[10px] font-bold uppercase"
                    >{{ summary()?.bookFavCount || 0 }} SAVED</span
                  >
                  <span class="text-[#4a4a4a] text-[10px] font-bold uppercase"
                    >ACTIVE</span
                  >
                }
              </div>
              <p class="text-[#a88a78] text-xs mt-2 line-clamp-1">
                Novels and light novels you love
              </p>
            </div>
          </a>
          ...
          >
            ...
            >
              ...
              ></div>
              <div
                class="absolute top-2.5 left-2.5 bg-white/15 text-white text-[9px] font-black px-2 py-1 rounded tracking-widest uppercase"
              >
                Manga
              </div>
            </div>
            <div>
              ...
              <div class="flex items-center gap-2 mt-2">
                @if (isLoading()) {
                  ...
                  <span
                    class="px-2 py-0.5 rounded bg-white/8 text-[#f9f9f2] text-[10px] font-bold uppercase"
                    >{{ summary()?.mangaFavCount || 0 }} TITLES</span
                  >
                  <span class="text-[#4a4a4a] text-[10px] font-bold uppercase"
                    >{{ summary()?.chapterWatchCount || 0 }} WATCHING</span
                  >
                }
              </div>
              <p class="text-[#a88a78] text-xs mt-2 line-clamp-1">
                Comics and manga you follow
              </p>
            </div>
          </a>
        ...
        <section>
          <div class="mb-5 flex items-center justify-between">
            <h2 class="text-xl font-bold text-[#f9f9f2] tracking-tight">
              Recent Activity
            </h2>
            <a
              routerLink="/profile"
          ...
          <div class="rounded-2xl overflow-hidden border border-[#1a1a1a]">
            <table class="w-full text-left activity-table">
              <thead>
                <tr class="bg-white/5">
                  <th
                    class="px-5 py-4 text-[10px] font-bold text-[#4a4a4a] uppercase tracking-widest"
                  >
                    Item
                  </th>
                  <th
                    class="hidden sm:table-cell px-5 py-4 text-[10px] font-bold text-[#4a4a4a] uppercase tracking-widest"
                  >
                    Type
                  </th>
                  <th
                  ...
                  <th
                    class="hidden md:table-cell px-5 py-4 text-[10px] font-bold text-[#4a4a4a] uppercase tracking-widest"
                  >
                    Progress
                  </th>
                  <th
                    class="hidden lg:table-cell px-5 py-4 text-[10px] font-bold text-[#4a4a4a] uppercase tracking-widest text-right"
                  >
                    Go
                  </th>
                </tr>
              ...
              <tbody>
                @if (isLoadingActivity()) {
                    ...
                    <tr>
                      <td class="px-5 py-4">
                        <div class="flex items-center gap-4">
                          <div
                            class="h-12 w-9 animate-pulse rounded-lg bg-[#181818] flex-shrink-0"
                          ></div>
                          <div
                            class="h-3 w-40 animate-pulse rounded bg-[#181818]"
                          ></div>
                        </div>
                      </td>
                      <td class="hidden sm:table-cell px-5 py-4">
                        <div
                          class="h-3 w-16 animate-pulse rounded bg-[#181818]"
                        ></div>
                      </td>
                      <td class="px-5 py-4">
                        <div
                          class="h-3 w-20 animate-pulse rounded bg-[#181818]"
                        ></div>
                      </td>
                      <td class="hidden md:table-cell px-5 py-4">
                        <div
                          class="h-1.5 w-24 animate-pulse rounded-full bg-[#181818]"
                        ></div>
                      </td>
                  ...
                  <tr>
                    <td
                      colspan="5"
                      class="px-5 py-12 text-center text-sm text-[#4a4a4a]"
                    >
                    ...
                    <tr class="cursor-pointer" [routerLink]="row.link">
                      <td class="px-5 py-4">
                        <div class="flex items-center gap-4">
                          <!-- Thumbnail -->
                          <div
                            class="h-12 w-9 flex-shrink-0 overflow-hidden rounded-lg bg-[#181818]"
                          >
                            @if (row.thumbnailUrl) {
                              <img
                          ...
                          <span
                            class="text-sm font-semibold text-[#d4d0c8] line-clamp-1"
                            >{{ row.title }}</span
                          >
                        </div>
                      </td>
                      <td class="hidden sm:table-cell px-5 py-4">
                        <span class="text-xs font-medium text-[#a88a78]">{{
                          row.meta
                        }}</span>
                      </td>
                      <td class="px-5 py-4">
                      ...
                      <td class="hidden md:table-cell px-5 py-4">
                        @if (row.progress > 0) {
                          <div class="flex items-center gap-3">
                            <div
                              class="flex-1 h-1.5 rounded-full bg-[#1e1e1e] max-w-[100px]"
                            >
                              <div
                            ...
                            <span
                              class="text-[10px] text-[#a88a78] font-medium w-8"
                              >{{ row.progress | number: "1.0-0" }}%</span
                            >
                          </div>
                        } @else {
                      ...
                      <td class="hidden lg:table-cell px-5 py-4 text-right">
                        <div class="flex justify-end">
                          <div
                            class="h-8 w-8 rounded-lg border border-[#1e1e1e] flex items-center justify-center text-[#a88a78] hover:border-[#800020]/50 hover:text-[#800020] transition"
                          >
                            <svg
                              class="h-3.5 w-3.5"
                              fill="none"
        ...
        @if (isLoadingContinueReading() || continueReadingBooks().length > 0) {
          <section class="mt-10">
            <div class="mb-4 flex items-center justify-between">
              <h2 class="text-base font-semibold text-[#f9f9f2]">
                Continue Reading
              </h2>
              <a
                routerLink="/books"
                class="text-xs font-medium text-[#800020] hover:text-[#a0002a] transition"
                >Browse books</a
              >
            </div>
              ...
              @if (isLoadingContinueReading()) {
                @for (i of [1, 2, 3, 4, 5]; track i) {
                  <div class="flex-shrink-0 w-28">
                    ...
                    ></div>
                    <div
                      class="mt-2 h-3 w-3/4 animate-pulse rounded bg-[#181818]"
                    ></div>
                  </div>
                }
              } @else {
                @for (book of continueReadingBooks(); track book.id) {
                  ...
                  >
                    <div
                      class="relative aspect-[2/3] overflow-hidden rounded-xl bg-[#181818]"
                    >
                      <img
                        [src]="getBookCover(book.slug, book.coverUrl)"
                        [alt]="book.title"
                    ...
                    <p class="mt-2 truncate text-xs font-medium text-[#f9f9f2]">
                      {{ book.title }}
                    </p>
                    @if (getBookProgress(book.slug); as progress) {
                      <p class="text-[10px] text-[#a88a78]">
...
export class UnifiedLibraryComponent implements OnInit {
  ...
  watchlistCount = computed(
    () => this.profileQuery.data()?.data?.watchlist?.length || 0,
  );
  ...
  isLoadingActivity = computed(
    () =>
      this.isLoadingWatchHistory() ||
      this.isLoadingContinueReading() ||
      this.isLoadingMangaHistory(),
  );
  ...
  activityRows = computed((): ActivityRow[] => {
    ...
    for (const item of this.continueWatching().slice(0, 8)) {
      rows.push({
        ...
        statusColor:
          item.progressPercentage >= 95 ? "text-emerald-400" : "text-[#800020]",
        dotColor:
          item.progressPercentage >= 95 ? "bg-emerald-500" : "bg-[#800020]",
    ...
    for (const item of this.mangaHistory().slice(0, 6)) {
      const pct =
        item.totalPages > 0
          ? Math.max(
              ...
              Math.min(
                100,
                Math.round(((item.pageIndex + 1) / item.totalPages) * 100),
              ),
            )
          : item.isCompleted
            ? 100
      ...
      rows.push({
        ...
        statusColor:
          item.isCompleted || pct >= 95
            ? "text-emerald-400"
            : "text-violet-400",
        dotColor:
          item.isCompleted || pct >= 95 ? "bg-emerald-500" : "bg-violet-500",
        progress: pct,
  ...
  private loadMangaHistory() {
    this.http
      .get<{ status?: string; data?: MangaHistoryRow[] }>(
        "/api/v1/books/manga/history",
        {
          params: { limit: "12" },
        },
      )
      .subscribe({
        next: (res) => {
          const rows = (res.data || []).filter(
            (item) => item.pageIndex >= 0 || item.isCompleted,
          );
  ...
  private loadBookProgress(
    slugs: string[],
    onDone?: (progressBySlug: Record<string, number>) => void,
  ) {
    ...
    const requests = uniqueSlugs.map((slug) =>
      this.http
        ...
        .pipe(
          map((r) => ({
            slug,
            percentage: Math.max(0, Math.min(100, r?.data?.page ?? 0)),
          })),
