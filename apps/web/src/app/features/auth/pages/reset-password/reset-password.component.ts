    ...
    <div class="relative min-h-screen overflow-hidden bg-[#f7efe8] dark:bg-[#0b0708] px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div class="relative mx-auto w-full max-w-md rounded-2xl border border-[#d8b7a8] dark:border-[#5f1327]/60 bg-white/90 dark:bg-[#120a0d]/90 p-8 shadow-[0_18px_60px_rgba(0,0,0,0.18)] dark:shadow-[0_18px_60px_rgba(0,0,0,0.55)] backdrop-blur-sm">
        <div>
          ...
          <p class="mt-4 text-center text-xs uppercase tracking-[0.28em] text-[#8a5f1c] dark:text-[#d6b87a]">Set New Password</p>
        ...
        @if (!token) {
          <div class="mt-8 text-center">
            ...
            <h3 class="text-xl font-semibold text-[#2a1c1f] dark:text-[#f3e5d8] mb-2">Invalid Reset Link</h3>
            <p class="text-[#7b6660] dark:text-[#d7c4b6] mb-6">
              This password reset link is invalid or has expired.
            </p>
            <a [routerLink]="['/forgot-password']" class="inline-flex justify-center rounded-lg border border-[#992143] bg-[#800020] px-6 py-3 text-sm font-semibold uppercase tracking-[0.14em] text-[#fff5f2] transition hover:bg-[#660019]">
              Request New Link
            </a>
          </div>
        } @else if (success) {
          <div class="mt-8 text-center">
            ...
            <h3 class="text-xl font-semibold text-[#2a1c1f] dark:text-[#f3e5d8] mb-2">Password Updated</h3>
            <p class="text-[#7b6660] dark:text-[#d7c4b6] mb-6">
              Your password has been reset successfully. You can now sign in with your new password.
            </p>
            <a [routerLink]="['/login']" class="inline-flex justify-center rounded-lg border border-[#992143] bg-[#800020] px-6 py-3 text-sm font-semibold uppercase tracking-[0.14em] text-[#fff5f2] transition hover:bg-[#660019]">
              Sign In Now
            </a>
          </div>
        } @else {
          <form [formGroup]="form" (ngSubmit)="onSubmit()" class="mt-8 space-y-5">
            <div class="space-y-3">
              <div>
                <label class="mb-2 block text-xs font-semibold uppercase tracking-[0.2em] text-[#8a5f1c] dark:text-[#d6b87a]">New Password</label>
                <input
                ...
                @if (form.get('password')?.invalid && form.get('password')?.touched) {
                  <p class="mt-1 text-xs text-[#c94866]">Password must be at least 6 characters</p>
                }
              </div>
              <div>
                <label class="mb-2 block text-xs font-semibold uppercase tracking-[0.2em] text-[#8a5f1c] dark:text-[#d6b87a]">Confirm New Password</label>
                <input
                ...
                @if (form.errors?.['passwordMismatch'] && form.get('confirmPassword')?.touched) {
                  <p class="mt-1 text-xs text-[#c94866]">Passwords do not match</p>
                }
              </div>
            </div>
...
export class ResetPasswordComponent implements OnInit {
  ...
  onSubmit() {
    if (this.form.valid && this.token) {
      ...
      this.http.post<ResetPasswordResponse>('/api/v1/auth/reset-password', {
        token: this.token,
