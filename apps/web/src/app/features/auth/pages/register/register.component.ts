...
interface SignupResponse {
  success: boolean;
  data?: { id: string; email: string; name: string | null; isPremium: boolean; role: string };
  error?: string;
}
      ...
      <div class="relative mx-auto w-full max-w-md rounded-2xl border border-[#d8b7a8] dark:border-[#5f1327]/60 bg-white/90 dark:bg-[#120a0d]/90 p-8 shadow-[0_18px_60px_rgba(0,0,0,0.18)] dark:shadow-[0_18px_60px_rgba(0,0,0,0.55)] backdrop-blur-sm">
        ...
        @if (success) {
          <div class="mt-8 text-center">
            ...
            <h3 class="text-xl font-semibold text-[#2a1c1f] dark:text-[#f3e5d8] mb-2">Welcome Aboard!</h3>
          ...
          <form [formGroup]="form" (ngSubmit)="onSubmit()" class="mt-8 space-y-5">
            <div class="space-y-3">
              <div>
                <label class="mb-2 block text-xs font-semibold uppercase tracking-[0.2em] text-[#8a5f1c] dark:text-[#d6b87a]">Full Name</label>
                <input
              ...
              <div>
                ...
                @if (form.get('email')?.invalid && form.get('email')?.touched) {
                  <p class="mt-1 text-xs text-[#c94866]">Please enter a valid email</p>
                }
              </div>
              <div>
              ...
              <div>
                <label class="mb-2 block text-xs font-semibold uppercase tracking-[0.2em] text-[#8a5f1c] dark:text-[#d6b87a]">Confirm Password</label>
                <input
            ...
            <p class="text-center text-sm text-[#7b6660] dark:text-[#d7c4b6]">
              ...
              <a [routerLink]="['/login']" class="font-semibold text-[#800020] dark:text-[#d6b87a] hover:text-[#5f1327] dark:hover:text-[#f3e5d8] transition">
                Sign in
              </a>
            </p>
          </form>
...
export class RegisterComponent implements OnInit, OnDestroy {
  ...
  onSubmit() {
    if (this.form.valid) {
      ...
      this.http.post<SignupResponse>('/api/v1/auth/signup', {
        name,
        email,
        password
      }).subscribe({
