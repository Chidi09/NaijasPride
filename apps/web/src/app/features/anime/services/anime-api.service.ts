...
export class AnimeApiService {
  ...
  search(params: AnimeSearchParams = {}) {
    ...
    return this.http.get<{ success: boolean; data: AnilistSearchResult }>(
      "/api/v1/anime/search",
      { params: query },
    );
  }
  ...
  getAnime(id: number) {
    return this.http.get<{ success: boolean; data: AnilistMedia }>(
      `/api/v1/anime/${id}`,
    );
  }

  getEpisodes(id: number, provider = "auto") {
    return this.http.get<{ success: boolean; data: AnimeEpisodesResponse }>(
  ...
  getWatchSources(
    ...
    return this.http.get<{ success: boolean; data: AnimeWatchResponse }>(
  ...
  getProgress(anilistId: number) {
    return this.http.get<{ success: boolean; data: AnimeWatchProgress[] }>(
      `/api/v1/anime/progress/${anilistId}`,
    );
  }

  getHistory(limit = 10) {
    return this.http.get<{ success: boolean; data: AnimeWatchProgress[] }>(
      `/api/v1/anime/history?limit=${limit}`,
    );
  }
}
