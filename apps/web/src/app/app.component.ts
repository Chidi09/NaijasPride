...
@Component({
  ...
  template: `
    <div
      class="min-h-screen flex flex-col bg-[var(--bg-primary)] text-[var(--text-primary)] transition-colors duration-300"
    >
      <!-- ═══════════════════════════════════════════════════════ -->
      <!-- WEB MODE: Classic Navbar (hidden in PWA/app mode)      -->
      <!-- ═══════════════════════════════════════════════════════ -->
      @if (
        !readerState.navbarHidden() &&
        !readerState.homePageActive() &&
        !pwaService.isAppMode()
      ) {
        <app-navbar />
      }
      ...
      <main
        ...
        [class.md:pl-20]="
          pwaService.isAppMode() && !readerState.homePageActive()
        "
      >
        <router-outlet />
      </main>
...
export class AppComponent implements OnInit {
  ...
  private readonly defaultViewportContent =
  ...
  constructor() {
    effect(() => {
      const shouldLockViewport =
        this.pwaService.isAppMode() && !!this.authState.currentUser();
  ...
  protected showFooter(): boolean {
    if (this.readerState.navbarHidden() || this.pwaService.isAppMode())
      return false;
  ...
  private applyAppViewportMode(locked: boolean): void {
    ...
    if (viewportMeta) {
      viewportMeta.setAttribute(
        "content",
        locked ? this.lockedViewportContent : this.defaultViewportContent,
      );
    }
