...
export class AdScriptService {
  ...
  private readonly effectiveGateSocialBarScriptId =
    "effectivegate-socialbar-script";
  private readonly effectiveGateSocialBarUrl =
    "https://pl28821993.effectivegatecpm.com/e2/17/7f/e2177fff15ecad68b2c2b7e699359650.js";

  ensureAdSenseAutoAdsScript(): void {
    if (!this.canUseDom()) return;

    if (
      this.hasScriptById(this.adsenseAutoAdsScriptId) ||
      this.hasScriptBySrc(this.adsenseAutoAdsUrl)
    )
      return;
  ...
  ensureEffectiveGateSocialBarScript(): void {
    if (!this.canUseDom()) return;
    if (
      this.hasScriptById(this.effectiveGateSocialBarScriptId) ||
      this.hasScriptBySrc(this.effectiveGateSocialBarUrl)
    )
      return;
  ...
  renderEffectiveGateSlot(
    container: HTMLElement,
    options: EffectiveGateSlotOptions,
  ): void {
    if (!this.canUseDom()) return;
    if (!container || !options.slotId) return;

    const existingSlot = container.querySelector(
      `[data-effectivegate-slot-id="${options.slotId}"]`,
    );
    ...
    this.slotRenderChain = this.slotRenderChain.then(() => {
      ...
      if (
        this.activeEffectiveGateSlot &&
        this.activeEffectiveGateSlot.slotId !== options.slotId
      ) {
  ...
  clearEffectiveGateSlot(container: HTMLElement, slotId: string): void {
    ...
    const slotNodes = container.querySelectorAll(
      `[data-effectivegate-slot-id="${slotId}"]`,
    );
    slotNodes.forEach((node) => node.remove());

    if (
      this.activeEffectiveGateSlot?.slotId === slotId &&
      this.activeEffectiveGateSlot.container === container
    ) {
  ...
  unloadAllAdScripts(): void {
    ...
    const adsenseBySrc = document.querySelector(
      `script[src="${this.adsenseAutoAdsUrl}"]`,
    );
    if (adsenseBySrc) {
      adsenseBySrc.remove();
