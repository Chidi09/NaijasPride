...
function readTvOverride(): boolean | null {
  ...
  const queryValue = new URLSearchParams(window.location.search).get(TV_QUERY_PARAM)?.trim().toLowerCase();
  if (queryValue) {
  ...
  const storedValue = window.localStorage.getItem(TV_STORAGE_KEY)?.trim().toLowerCase();
...
export function detectTvEnvironment(): boolean {
  ...
  if (tvSignatures.some((signature) => ua.includes(signature) || platform.includes(signature))) {
    return true;
  }

  const isMobileUa = /android|iphone|ipad|ipod|mobile/.test(ua);
  const isDesktopUa = /windows nt|macintosh|x11|linux/.test(ua) && !ua.includes('android');
  const screenWidth = Math.max(window.innerWidth || 0, window.screen?.width || 0);
  const screenHeight = Math.max(window.innerHeight || 0, window.screen?.height || 0);
  ...
  return maxSide >= 1280 && minSide >= 720 && coarsePointer && !anyHover && !isMobileUa && !isDesktopUa;
}
