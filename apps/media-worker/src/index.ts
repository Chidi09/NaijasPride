...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (!('body' in object) || !object.body) {
      return new Response('Not Modified', { status: 304, headers: CORS_HEADERS });
    }
    ...
    if (parsedRange && object.range && typeof object.range.offset === 'number' && typeof object.range.length === 'number') {
