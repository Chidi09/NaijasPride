...
import type WebTorrentClass from "webtorrent";
import type { Torrent, TorrentFile } from "webtorrent";
// webtorrent v2+ is ESM-only — loaded via dynamic import() at runtime
let WebTorrent: typeof WebTorrentClass | undefined;
...
if (!REDIS_URL) {
  ...
  console.error(
    "[Worker] Set REDIS_URL to your Redis instance (e.g., Redis Labs, Railway)",
  );
  process.exit(1);
}
...
const TORRENT_DOWNLOAD_DIR = (
  process.env.TORRENT_DOWNLOAD_DIR ||
  path.join(os.tmpdir(), "naijaspride-downloads")
).trim();
// Minimum free bytes required before accepting a torrent job (default 15 GB)
const TORRENT_MIN_FREE_BYTES = parsePositiveInt(
  process.env.TORRENT_MIN_FREE_DISK_GB
    ? String(
        Math.round(
          Number(process.env.TORRENT_MIN_FREE_DISK_GB) * 1024 * 1024 * 1024,
        ),
      )
    : undefined,
  15 * 1024 * 1024 * 1024,
...
const FFPROBE_PATH =
  (process.env.FFPROBE_PATH || "ffprobe").trim() || "ffprobe";
const STORAGE_PUBLIC_BASE_URL = (
  process.env.STORAGE_PUBLIC_BASE_URL ||
  process.env.S3_PUBLIC_BASE_URL ||
  ""
).trim();
const TORRENT_WORKER_CONCURRENCY = parsePositiveInt(
  process.env.TORRENT_WORKER_CONCURRENCY,
  2,
  1,
  4,
);
const TORRENT_JOB_TIMEOUT_MS = parsePositiveInt(
  process.env.TORRENT_JOB_TIMEOUT_MS,
  60 * 60 * 1000,
  5 * 60 * 1000,
  4 * 60 * 60 * 1000,
);
...
const invalidateMovieCaches = async (slug?: string | null): Promise<void> => {
  try {
    ...
    console.warn(
      `[Worker] Cache invalidation failed: ${getErrorMessage(error)}`,
    );
  }
};
  ...
  return new Promise((resolve, reject) => {
    ...
    proc.on("close", (code) => {
      ...
      try {
        ...
        reject(
          new Error(`ffprobe returned invalid JSON: ${getErrorMessage(error)}`),
        );
      }
    });
  });
      ...
      (torrent: Torrent) => {
        void (async () => {
          ...
          try {
            ...
            const mp4Files = videoFiles.filter((f: TorrentFile) =>
              /\.mp4$/i.test(f.name),
            );
            const candidates = (
              mp4Files.length > 0 ? mp4Files : videoFiles
            ).sort(
              (a: TorrentFile, b: TorrentFile) =>
            ...
            const safeName = (() => {
              ...
              const cleaned = base
                ...
                .replace(/[^a-z0-9._-]/gi, "-");
              return (
                cleaned.replace(/-+/g, "-").replace(/^-|-$/g, "") || "video"
              );
            })();
            ...
            console.log(
              `[Worker] Downloading ${file.name} (${(file.length / 1024 / 1024).toFixed(1)} MB)...`,
            );
            await pipeline(
            ...
            if (isMkv) {
              ...
              if (TORRENT_REMUX_MKV) {
                try {
                  ...
                  console.warn(
                    `[Worker] ffprobe failed: ${getErrorMessage(error)}`,
                  );
                }
              }
              ...
              if (TORRENT_REMUX_MKV && remuxPlan?.canRemux) {
                ...
                try {
                  ...
                  console.warn(
                    `[Worker] MKV remux failed: ${getErrorMessage(error)}`,
                  );
                  remuxPlan = {
                    ...remuxPlan,
                    canRemux: false,
                    reason: "ffmpeg remux failed",
                  };
                }
              }
            ...
            if (TORRENT_PACKAGE_HLS) {
              ...
              try {
                ...
                console.error(
                  `[Worker] HLS packaging failed: ${getErrorMessage(error)}`,
                );
...
const worker = new Worker(
  ...
  async (job) => {
    ...
    const { magnetLink, movieId } = job.data as {
      magnetLink: string;
      movieId: string;
    };
    ...
    if (freeBytes > 0 && freeBytes < TORRENT_MIN_FREE_BYTES) {
      const requiredGB = (TORRENT_MIN_FREE_BYTES / 1024 / 1024 / 1024).toFixed(
        1,
      );
      console.warn(
        `[Worker] Job ${job.id} SKIPPED — only ${freeGB} GB free, need ${requiredGB} GB. Job will be retried later.`,
      );
      // Throw a non-fatal error so BullMQ retries after backoff rather than marking failed.
      throw Object.assign(
        new Error(
          `Insufficient disk space: ${freeGB} GB free, need ${requiredGB} GB`,
        ),
        { skipMovie: true },
      );
    }
    ...
    try {
      ...
      prisma.downloadRequest
        .findMany({
          where: { movieId, status: "QUEUED" },
          select: { id: true, userId: true },
        })
        .then(async (requests: Array<{ id: string; userId: string }>) => {
          if (!requests.length) return;
          await prisma.notification.createMany({
            data: requests.map((r: { id: string; userId: string }) => ({
              userId: r.userId,
...
console.log(
  `[Worker] HLS Packaging: ${TORRENT_PACKAGE_HLS ? "enabled" : "disabled"}`,
);
console.log(
  `[Worker] MKV Remuxing: ${TORRENT_REMUX_MKV ? "enabled" : "disabled"}`,
);
console.log(
  `[Worker] MKV Transcode fallback: ${TORRENT_TRANSCODE_MKV ? "enabled" : "disabled"}`,
);
...
void (async () => {
  try {
    await fs.promises.mkdir(path.join(TORRENT_DOWNLOAD_DIR, "_transcode"), {
      recursive: true,
    });
    await fs.promises.mkdir(path.join(TORRENT_DOWNLOAD_DIR, "_hls"), {
      recursive: true,
    });
    console.log("[Worker] Download subdirectories ready");
  } catch (err) {
    console.error(
      "[Worker] FATAL: Cannot create download subdirectories:",
      (err as Error).message,
    );
    console.error(
      "[Worker] Fix: chown the torrent volume to uid 1001 and restart the worker.",
    );
    process.exit(1);
  }
})();
...
void (async () => {
  ...
  const BACKFILL_BATCH = parsePositiveInt(
    process.env.TORRENT_BACKFILL_BATCH,
    50,
    1,
    500,
  );
  const attemptsRaw = Number.parseInt(
    process.env.TORRENT_JOB_ATTEMPTS || "3",
    10,
  );
  const attempts =
  ...
  try {
    const queue = new Queue("torrent-processing", {
    ...
    const pendingMovies = candidates
      .filter((m: { id: string; title: string; metadata: unknown }) => {
        const meta = m.metadata as Record<string, unknown> | null;
        return !!meta?.sourceInfoHash;
      })
      .slice(0, BACKFILL_BATCH);

    if (pendingMovies.length === 0) {
      console.log(
        "[Worker] Startup backfill: no orphaned pending movies found",
      );
      await queue.close();
      return;
    }
    ...
    console.log(
      `[Worker] Startup backfill: re-queuing ${pendingMovies.length} orphaned pending movies`,
    );
    let enqueued = 0;
    for (const movie of pendingMovies) {
      const meta = movie.metadata as Record<string, unknown> | null;
      const infoHash = meta?.sourceInfoHash as string | undefined;
      const rawTitle =
        (meta?.sourceRawTitle as string | undefined) ?? movie.title;
    ...
    console.error(
      "[Worker] Startup backfill failed (non-fatal):",
      (err as Error).message,
    );
  }
})();
