...
import {
  importEpubBooksCatalog,
  type EpubBooksImportOptions,
  type EpubBooksImportSort,
} from "../modules/books/external/epubbooks/importer";
import {
...
const AUTO_CURSOR_KEY = (
  process.env.EPUBBOOKS_AUTO_IMPORT_CURSOR_KEY || "np:epubbooks:auto:page"
).trim();
const AUTO_DEFAULT_SORT =
  ((process.env.EPUBBOOKS_AUTO_IMPORT_SORT || "title")
    .trim()
    .toLowerCase() as EpubBooksImportSort) || "title";
const AUTO_DEFAULT_MAX_BOOKS = Number.parseInt(
  process.env.EPUBBOOKS_AUTO_IMPORT_MAX_BOOKS || "8",
  10,
);
const AUTO_DEFAULT_CONCURRENCY = Number.parseInt(
  process.env.EPUBBOOKS_AUTO_IMPORT_CONCURRENCY || "2",
  10,
);
const MAX_RETURN_RESULTS = Number.parseInt(
  process.env.BOOK_IMPORT_MAX_RETURN_RESULTS || "200",
  10,
);
...
const buildSafeReturnValue = (result: unknown) => {
  if (!result || typeof result !== "object") return result;
  const res = result as Record<string, unknown>;
  const list = Array.isArray(res.results) ? res.results : [];
  const cap =
    Number.isFinite(MAX_RETURN_RESULTS) && MAX_RETURN_RESULTS > 0
      ? MAX_RETURN_RESULTS
      : 200;
  return {
    ...res,
...
const enqueueBookCoverJobsBySlugs = async (
  slugs: string[],
): Promise<number> => {
  ...
  for (const book of books) {
    try {
      ...
      console.error(
        `[BookImportWorker] Failed to enqueue cover job for ${book.slug}: ${message}`,
      );
    }
  }
...
const extractSuccessfulSlugs = (result: unknown): string[] => {
  if (!result || typeof result !== "object") return [];
  const res = result as { results?: unknown };
  const rows: Array<{ ok?: boolean; slug?: unknown }> = Array.isArray(
    res.results,
  )
    ? res.results
    : [];
...
const worker = new Worker(
  ...
  async (job) => {
    ...
    if (data.source === "elsci-lightnovels") {
      ...
      const maxBooks =
        typeof opts.maxBooks === "undefined"
          ? 120
          : clampInt(opts.maxBooks, 120, 1, 2000);
      const formatPreference =
        opts.formatPreference === "pdf" ||
        opts.formatPreference === "any" ||
        opts.formatPreference === "epub"
          ? opts.formatPreference
          : "epub";
      const includePattern =
        typeof opts.includePattern === "string"
          ? opts.includePattern.trim()
          : undefined;
      const excludePattern =
        typeof opts.excludePattern === "string"
          ? opts.excludePattern.trim()
          : undefined;
      const rootPath =
      ...
      if (!dryRun) {
        ...
        if (queued > 0) {
          console.log(
            `[BookImportWorker] Queued ${queued} book cover jobs (elsci-lightnovels)`,
          );
        }
      }
    ...
    if (data.mode === "auto") {
      ...
      const sort = (data.options?.sort ||
        AUTO_DEFAULT_SORT) as EpubBooksImportSort;
      const maxBooks = clampInt(
        data.options?.maxBooks,
        AUTO_DEFAULT_MAX_BOOKS,
        1,
        100,
      );
      const concurrency = clampInt(
        data.options?.concurrency,
        AUTO_DEFAULT_CONCURRENCY,
        1,
        6,
      );
      ...
      if (autoQueued > 0) {
        console.log(
          `[BookImportWorker] Queued ${autoQueued} book cover jobs (epubbooks auto)`,
        );
      }
    ...
    const maxBooks =
      typeof opts.maxBooks === "undefined"
        ? undefined
        : clampInt(opts.maxBooks, 100, 1, 5000);
    const dryRun = !!opts.dryRun;
    ...
    if (!dryRun) {
      ...
      if (queued > 0) {
        console.log(
          `[BookImportWorker] Queued ${queued} book cover jobs (epubbooks manual)`,
        );
      }
    }
