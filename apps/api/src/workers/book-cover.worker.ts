...
const worker = new Worker(
  ...
  async (job) => {
    ...
    console.log(
      `[BookCoverWorker] Processing job ${job.id} for book ${payload.bookId}`,
    );
    ...
    if (result.updated) {
      console.log(
        `[BookCoverWorker] Cover ready for book ${payload.bookId}: ${result.coverUrl || result.key || "ok"}`,
      );
    } else {
      console.log(
        `[BookCoverWorker] Cover skipped for book ${payload.bookId}: ${result.reason || "no-change"}`,
      );
    }
  ...
  },
  {
    ...
    concurrency: parsePositiveInt(
      process.env.BOOK_COVER_WORKER_CONCURRENCY,
      2,
      1,
      6,
    ),
