...
import {
  RemoteProvider,
  RemoteStreamResolverService,
} from "../modules/movies/remote-stream-resolver.service";
...
const resolveR2Config = (): R2Config => {
  ...
  if (!endpoint || !accessKeyId || !secretAccessKey || !bucket) {
    throw new Error(
      "R2 config is incomplete. Set S3_ENDPOINT, S3_ACCESS_KEY_ID, S3_SECRET_ACCESS_KEY, and S3_BUCKET.",
    );
  }
...
const REMOTE_INGEST_DIR = (
  process.env.REMOTE_INGEST_TMP_DIR ||
  path.join(os.tmpdir(), "naijaspride-remote-ingest")
).trim();
...
const REMOTE_INGEST_WORKER_CONCURRENCY = (() => {
  const parsed = Number.parseInt(
    process.env.REMOTE_INGEST_WORKER_CONCURRENCY || "2",
    10,
  );
...
const invalidateMovieCaches = async (slug?: string | null): Promise<void> => {
  try {
    ...
    console.warn(
      `[RemoteIngestWorker] Failed to invalidate movie cache: ${toErrorMessage(error)}`,
    );
  }
};
...
const ingestSourceToMp4 = async (
  ...
  const sharedArgs = ["-hide_banner", "-loglevel", "error", "-y"];
  ...
  try {
    ...
    console.warn(
      `[RemoteIngestWorker] Stream-copy failed, falling back to transcode: ${toErrorMessage(error)}`,
    );
  }
...
const worker = new Worker(
  ...
  async (job) => {
    ...
    console.log(
      `[RemoteIngestWorker] Starting job ${job.id} for movie ${payload.movieId}`,
    );
    ...
    try {
      ...
      if (!streamUrl) {
        ...
        const resolved = await resolver.resolveFromPage(payload.sourcePageUrl, {
          provider,
        });
        streamUrl = resolved.streamUrl;
        streamReferer =
          streamReferer || resolved.referer || payload.sourcePageUrl;
      }
      ...
      console.log(
        `[RemoteIngestWorker] Job ${job.id} completed for movie ${payload.movieId}`,
      );
    } catch (error) {
      console.error(
        `[RemoteIngestWorker] Job ${job.id} failed: ${toErrorMessage(error)}`,
      );
...
worker.on("failed", (job, err) => {
  ...
  deadLetterQueue
    ...
    .catch((error) => {
      console.error(
        "[RemoteIngestWorker] Failed to enqueue dead-letter item:",
...
console.log(
  `[RemoteIngestWorker] Concurrency: ${REMOTE_INGEST_WORKER_CONCURRENCY}`,
);
console.log(
  `[RemoteIngestWorker] HLS packaging: ${REMOTE_INGEST_PACKAGE_HLS ? "enabled" : "disabled"}`,
);
console.log(
  `[RemoteIngestWorker] HLS stream gateway: ${REMOTE_INGEST_STREAM_GATEWAY ? "enabled" : "disabled"}`,
);
