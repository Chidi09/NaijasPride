...
export const booksLibraryRoutes = async (app: FastifyInstance) => {
        ...
        include: {
          book: {
            select: {
              ...
              coverUrl: true, format: true, pageCount: true,
              genre: true, year: true, status: true,
            },
          },
        },
  ...
  app.post('/manga/offline', {
    ...
    handler: async (request, reply) => {
      const userId = request.user.userId;
      const body = request.body as z.infer<typeof OfflineMangaChapterBodySchema>;
  ...
  app.post('/books/offline', {
    ...
    handler: async (request, reply) => {
      ...
      const saved = await app.prisma.offlineBook.upsert({
        ...
        update: { fileSizeBytes: body.fileSizeBytes ?? null, format: body.format, savedAt: new Date() },
  ...
  app.post('/manga/chapter-watch', {
    ...
    handler: async (request, reply) => {
      ...
      const record = await app.prisma.mangaNewChapterCheck.upsert({
        ...
        select: { id: true, mangaId: true, mangaTitle: true, lastSeenChapterId: true },
      });
  ...
  app.delete('/manga/chapter-watch/:mangaId', {
    ...
    handler: async (request, reply) => {
      ...
      await app.prisma.mangaNewChapterCheck.deleteMany({ where: { userId, mangaId } });
      return reply.send({ status: 'success', message: 'Unsubscribed from chapter alerts' });
    },
  });
  ...
  app.patch('/manga/chapter-watch/mark-seen', {
    ...
    handler: async (request, reply) => {
      const userId = request.user.userId;
      const { mangaId, lastSeenChapterId, lastSeenAt } = request.body as z.infer<typeof MangaChapterCheckMarkSchema>;
  ...
  app.get('/summary', {
    ...
    handler: async (request, reply) => {
      const userId = request.user.userId;
      const [bookFavCount, mangaFavCount, offlineMangaCount, offlineBookCount, chapterWatchCount] =
        await Promise.all([
      ...
      return reply.send({
        status: 'success',
        data: { bookFavCount, mangaFavCount, offlineMangaCount, offlineBookCount, chapterWatchCount },
      });
    },
  });
};
