...
export abstract class MangaReaderBaseSource extends BaseHtmlSource {
  ...
  async getMangaDetail(mangaId: string): Promise<MangaDetail | null> {
    ...
    try {
      ...
      const detail: MangaDetail = {
        ...
        title:
          this.strip(
            $("h1.entry-title, .entry-title, .seriestucontent h1, h1")
              .first()
              .text(),
          ) ||
        ...
        description:
          ...
          this.strip(
            $(
              "div.entry-content, .summary__content, .description-summary, .summary",
            )
              .first()
              .text(),
          ),
        coverUrl:
          ...
          this.toAbsoluteUrl(
            $(".seriestucontent img, .thumb img, .summary_image img, img")
              .first()
              .attr("src"),
          ),
        status,
        ...
        tags: $(
          '.seriestugenre a, .wd-full .mgen a, .genres-content a, a[href*="genre"]',
        )
  ...
  async getChapters(
    ...
    try {
      ...
      $(
        "#chapterlist > ul > li a, li.wp-manga-chapter a, .eplister li a, .listing-chapters_wrap a",
      ).each((index, el) => {
  ...
  protected extractMangaCards(
    ...
    $(".postbody .listupd .bs .bsx, .listupd .bs .bsx, .utao .uta, .bsx").each(
      (_idx, el) => {
        ...
        results.push({
          id,
          title:
            this.strip($(el).find(".tt, .title, .post-title").first().text()) ||
