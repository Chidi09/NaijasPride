...
export abstract class ZeistMangaBaseSource extends BaseHtmlSource {
  ...
  async searchManga(query?: string, limit = 20, _filters: MangaSearchFilters = {}): Promise<MangaSummary[]> {
    ...
    try {
      const entries = await this.fetchFeedEntries(this.seriesFeedLabel, normalized, 1, Math.min(limit + 5, 50));
  ...
  async getDiscoverManga(limit = 12): Promise<MangaDiscoverResult> {
    ...
    try {
      const entries = await this.fetchFeedEntries(this.seriesFeedLabel, undefined, 1, safeLimit + 5);
  ...
  async getMangaTags(): Promise<MangaTag[]> {
    ...
    try {
      ...
      $('div.filter ul li').each((_idx, el) => {
        ...
        const name = this.strip($(el).find('label').text()) || this.strip($(el).text());
    ...
    try {
      ...
      const detail: MangaDetail = {
        ...
        description:
          ...
          this.strip($('#synopsis, #Sinopse, #sinopas, .sinopsis, .sinopas, .description, .summary').first().text()),
        coverUrl:
          this.toAbsoluteUrl($('meta[property="og:image"]').attr('content')) ||
          this.toAbsoluteUrl($('article img, .separator img, img').first().attr('src')),
        ...
        tags: $('article div.mt-15 a, .info-genre a, dl:contains("Genre") dd a, a[href*="label/"]')
  ...
  async getChapters(mangaId: string, translatedLanguage?: string, limit = 100): Promise<MangaChapter[]> {
    ...
    try {
      ...
      const feedLabel = this.extractChapterFeedLabel(html, $) || this.seriesFeedLabel;
      const entries = await this.fetchFeedEntries(feedLabel, undefined, 1, 999);
      const chapters = this.mapFeedEntriesToChapters(entries, limit, translatedLanguage);
      ...
      try {
        const html = await this.fetchHtml(seriesPath);
        const $ = cheerio.load(html);
        const chapters = this.extractChaptersFromHtml($, limit, translatedLanguage);
        await this.setCache(cacheKey, chapters);
        return chapters;
      } catch {
        return [];
  ...
  protected async fetchFeedEntries(
    ...
    const url = new URL(`/feeds/posts/default/-/${encodeURIComponent(label)}`, this.baseUrl);
    ...
    const parsed = JSON.parse(response.body || '{}') as { feed?: { entry?: BloggerEntry[] } };
    return parsed.feed?.entry || [];
  }

  protected mapFeedEntriesToSummary(entries: BloggerEntry[], limit: number): MangaSummary[] {
  ...
  protected extractChaptersFromHtml($: cheerio.CheerioAPI, limit: number, translatedLanguage?: string): MangaChapter[] {
    const chapters: MangaChapter[] = [];
    const seen = new Set<string>();
  ...
  protected extractChapterFeedLabel(html: string, $: cheerio.CheerioAPI): string | null {
    const direct =
  ...
  protected getEntryAlternateLink(entry: BloggerEntry): string | null {
    const link = (entry.link || []).find((candidate) => candidate.rel === 'alternate');
  ...
  protected parseStatusText(value: string): string | null {
    ...
    if (lowered.includes('cancel') || lowered.includes('drop') || lowered.includes('abandon')) {
      return 'cancelled';
    }
