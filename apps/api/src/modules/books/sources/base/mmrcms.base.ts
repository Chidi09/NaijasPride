...
export abstract class MmrcmsBaseSource extends BaseHtmlSource {
  ...
  constructor(options: {
    ...
    this.updatedCoverSuffix =
      options.updatedCoverSuffix || "/cover/cover_250x350.jpg";
    this.detailStatusSelector =
      options.detailStatusSelector || 'dt:contains("Statut")';
    this.detailTagSelector =
      options.detailTagSelector || 'dt:contains("Catégories")';
  }
  ...
  async getMangaTags(): Promise<MangaTag[]> {
    ...
    try {
      ...
      $("ul.list-category li a").each((_idx, el) => {
        ...
        const id = href.includes("cat=")
          ? href.substring(href.lastIndexOf("cat=") + 4)
      ...
      const detail: MangaDetail = {
        ...
        coverUrl:
          this.extractImageUrl(
            $(
              "div.list-container .boxed img, .boxed img, .col-sm-4 .img-responsive",
            ).first(),
          ) ||
          this.toAbsoluteUrl(
            this.strip($('meta[property="og:image"]').attr("content")),
          ) ||
          this.extractImageUrl($("img").first()),
        status: this.parseStatus(
          this.strip($(this.detailStatusSelector).first().next().text()),
        ),
        year: null,
        ...
        tags: $(this.detailTagSelector)
          ...
          .filter(Boolean),
        latestChapter:
          this.strip($("ul.chapters li h5").first().text()) || null,
        author:
  ...
  async getChapters(
    ...
    try {
      ...
      for (const [index, chapterItem] of chapterItems.entries()) {
        ...
        const title =
          this.strip($(chapterItem).find("h5").first().text()) || null;
        const publishedAt = this.parseChapterDate(
          this.strip(
            $(chapterItem).find("div.date-chapter-title-rtl").first().text(),
          ),
        );
  ...
  protected parseListCards(
    ...
    $("div.media").each((_idx, el) => {
      ...
      cards.push({
        id,
        title:
          this.strip($(el).find("div.media-body h5").first().text()) ||
          this.strip($(el).find("a").first().text()) ||
          "Unknown Title",
        description: "",
  ...
  protected parseUpdatedCards(
    ...
    $("div.manga-item").each((_idx, el) => {
      ...
      const parsedCoverFromCard = this.extractImageUrl(
        $(el).find("img").first(),
      );
      const fallbackCover = slug
        ? this.toAbsoluteUrl(`/uploads/manga/${slug}${this.updatedCoverSuffix}`)
        : null;
      ...
      cards.push({
        id,
        title:
          this.strip($(el).find("h3 a").first().text()) ||
          this.strip($(el).find("a").first().text()) ||
          "Unknown Title",
        description: "",
        coverUrl,
  ...
  protected extractImageUrl(image: cheerio.Cheerio<AnyNode>): string | null {
    const attributes = [
      "data-src",
      "data-cfsrc",
      "data-original",
    ...
    for (const attribute of attributes) {
      ...
      const candidate = attribute.includes("srcset")
        ? (rawValue.split(",")[0] || "").trim().split(/\s+/)[0]
        : rawValue;
      const normalizedCandidate = this.strip(candidate);
      if (!normalizedCandidate) continue;

      const lowered = normalizedCandidate.toLowerCase();
      if (lowered.startsWith("data:") || lowered.startsWith("about:blank"))
        continue;
  ...
  protected parseChapterDate(value: string): string | null {
    ...
    const parsedDate = new Date(
      Date.UTC(Number(yearRaw), monthIndex, Number(dayRaw)),
    );
