...
export class MangaSourceManager {
  ...
  async searchManga(query?: string, limit = 20, filters: MangaSearchFilters = {}): Promise<MangaSummary[]> {
    ...
    return this.runWithResilience(source, 'search', () => source.searchManga(query, limit, filters));
  }
  ...
  async searchMangaBySource(
    ...
    const result = await this.runWithResilience(source, 'search', () => source.searchManga(query, limit, filters));
    return this.withSourceOnSummaries(source.id, result);
  }

  async getDiscoverManga(limit = 12): Promise<MangaDiscoverResult> {
    const source = this.resolveSource();
    return this.runWithResilience(source, 'discover', () => source.getDiscoverManga(limit));
  }

  async getDiscoverMangaBySource(sourceId: string, limit = 12): Promise<MangaDiscoverResult> {
    const source = this.resolveSource(sourceId);
    const result = await this.runWithResilience(source, 'discover', () => source.getDiscoverManga(limit));
    return {
      trending: this.withSourceOnSummaries(source.id, result.trending),
      recentlyUpdated: this.withSourceOnSummaries(source.id, result.recentlyUpdated),
  ...
  async getMangaDetail(mangaId: string): Promise<MangaDetail | null> {
    const source = this.resolveSource();
    return this.runWithResilience(source, 'detail', () => source.getMangaDetail(mangaId));
  }
  ...
  async getMangaDetailBySource(sourceId: string, mangaId: string): Promise<MangaDetail | null> {
    ...
    const result = await this.runWithResilience(source, 'detail', () => source.getMangaDetail(sourceMangaId));
  ...
  async getSimilarManga(mangaId: string, limit = 6): Promise<MangaSummary[]> {
    const source = this.resolveSource();
    return this.runWithResilience(source, 'similar', () => source.getSimilarManga(mangaId, limit));
  }

  async getSimilarMangaBySource(sourceId: string, mangaId: string, limit = 6): Promise<MangaSummary[]> {
    const source = this.resolveSource(sourceId);
    const sourceMangaId = this.toSourceRawId(source.id, mangaId);
    const result = await this.runWithResilience(source, 'similar', () => source.getSimilarManga(sourceMangaId, limit));
    return this.withSourceOnSummaries(source.id, result);
  }

  async getChapters(mangaId: string, translatedLanguage?: string, limit = 100): Promise<MangaChapter[]> {
    const source = this.resolveSource();
    return this.runWithResilience(source, 'chapters', () => source.getChapters(mangaId, translatedLanguage, limit));
  }
  ...
  async getChapterPages(chapterId: string): Promise<MangaPagesResult> {
    const source = this.resolveSource();
    return this.runWithResilience(source, 'pages', () => source.getChapterPages(chapterId));
  }
  ...
  async getChapterPagesBySource(sourceId: string, chapterId: string): Promise<MangaPagesResult> {
    ...
    const result = await this.runWithResilience(source, 'pages', () => source.getChapterPages(sourceChapterId));
  ...
  private async runWithResilience<T>(
    ...
    if (!breaker.canExecute()) {
      ...
      throw new Error(`Source ${source.id} is temporarily unavailable (circuit open) for ${operation}`);
    }

    try {
      const result = await withRetry(action, { maxAttempts: 3, initialDelayMs: 250 });
      breaker.onSuccess();
      ...
      this.lastErrors.set(source.id, error instanceof Error ? error.message : 'unknown-error');
      throw error;
    }
  }
  ...
  private createBreakerForSource(sourceId: string): CircuitBreaker {
    ...
    const failureThreshold = Number.parseInt(process.env[`${prefix}_CB_FAILURES`] || '5', 10);
    const recoveryTimeoutMs = Number.parseInt(process.env[`${prefix}_CB_RECOVERY_MS`] || '30000', 10);

    return new CircuitBreaker({
      failureThreshold: Number.isFinite(failureThreshold) && failureThreshold > 0 ? failureThreshold : 5,
      recoveryTimeoutMs: Number.isFinite(recoveryTimeoutMs) && recoveryTimeoutMs > 0 ? recoveryTimeoutMs : 30_000,
      halfOpenMaxCalls: 1,
    });
  }
  ...
  private toSourceRawId(sourceId: string, id: string): string {
    ...
    if (parsed.sourceId !== sourceId) {
      throw new Error(`ID source mismatch: expected ${sourceId}, received ${parsed.sourceId}`);
    }
    return parsed.rawId;
  }

  private withSourceOnSummaries(sourceId: string, entries: MangaSummary[]): MangaSummary[] {
    return entries.map((entry) => ({ ...entry, id: buildSourceEntityId(sourceId, entry.id) }));
  }

  private withSourceOnDetail(sourceId: string, detail: MangaDetail): MangaDetail {
    return {
      ...detail,
  ...
  private withSourceOnChapters(sourceId: string, chapters: MangaChapter[]): MangaChapter[] {
  ...
  private withSourceOnPages(sourceId: string, pages: MangaPagesResult): MangaPagesResult {
    return {
      ...pages,
