...
export class MangabuddySource extends BaseHtmlSource {
  ...
  async getDiscoverManga(limit = 12): Promise<MangaDiscoverResult> {
    ...
    try {
      ...
      $("div.book-item").each((_idx, el) => {
        ...
        const title =
          this.strip($el.find("div.meta div.title").text()) ||
      ...
      console.error(
        `[MangaBuddy] discover failed: ${summarizeSourceError(error)}`,
      );
  ...
  async searchManga(
    ...
    try {
      ...
      console.error(
        `[MangaBuddy] search failed: ${summarizeSourceError(error)}`,
      );
      return [];
    }
  }
  ...
  async getMangaDetail(mangaId: string): Promise<MangaDetail | null> {
    ...
    try {
      ...
      const title =
        this.strip($("div.detail div.name h1").text()) ||
      ...
      const description =
        this.strip($("div.section-body.summary p.content").text()) ||
      ...
      const statusText = this.strip(
        $('div.detail p:contains("Status") span').text(),
      );
      const status =
        statusText === "Ongoing"
          ? "ongoing"
      ...
      const detail: MangaDetail = {
        ...
        latestChapter: null,
        author:
          this.strip($('div.detail p:contains("Author") a').text()) || null,
        artist: null,
        contentRating: null,
      ...
      console.error(
        `[MangaBuddy] detail failed: ${summarizeSourceError(error)}`,
      );
      return null;
    }
  }
  ...
  async getChapters(
    ...
    try {
      ...
      if (!bookSlug) {
        bookSlug =
          $series("[data-book-slug]").attr("data-book-slug") ??
      ...
      $chapters("ul#chapter-list li").each((index, el) => {
        ...
        const title =
          this.strip($li.find(".chapter-title").text()) ||
      ...
      console.error(
        `[MangaBuddy] chapter fetch failed: ${summarizeSourceError(error)}`,
      );
      return [];
    }
  }
  ...
  async getChapterPages(chapterId: string): Promise<MangaPagesResult> {
    ...
    try {
      ...
      console.error(
        `[MangaBuddy] page fetch failed: ${summarizeSourceError(error)}`,
      );
      return {
        chapterId,
        readerMode: "standard",
