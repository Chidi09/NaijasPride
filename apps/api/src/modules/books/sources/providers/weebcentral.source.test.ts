...
const fixture = (name: string) =>
  readFileSync(
    resolve(
      process.cwd(),
      "src/modules/books/sources/providers/parsers/__fixtures__",
      name,
    ),
    "utf8",
  );
...
test("WeebCentralSource extracts and coerces series IDs to ULID format", () => {
  ...
  assert.equal(
    source.extractSeriesId(`/series/${SERIES_ID}/Absolute-Sword-Sense`),
    SERIES_ID,
  );
  assert.equal(
    source.extractSeriesId(`https://weebcentral.com/series/${SERIES_ID}`),
    SERIES_ID,
  );
  assert.equal(source.coerceSeriesId(SERIES_ID), SERIES_ID);
  assert.equal(source.coerceSeriesId(`weebcentral:${SERIES_ID}`), SERIES_ID);
  assert.equal(
    source.coerceSeriesId(`/series/${SERIES_ID}/Absolute-Sword-Sense`),
    SERIES_ID,
  );
...
test("WeebCentralSource extracts and coerces chapter IDs to ULID format", () => {
  ...
  assert.equal(
    source.extractChapterId(`https://weebcentral.com/chapters/${CHAPTER_ID}/`),
    CHAPTER_ID,
  );
