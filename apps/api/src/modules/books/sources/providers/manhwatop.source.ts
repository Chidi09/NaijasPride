...
export class ManhwaTopSource extends MadaraBaseSource {
  ...
  async getDiscoverManga(limit = 12): Promise<MangaDiscoverResult> {
    ...
    try {
      ...
      $(".page-item-detail.manga, .c-tabs-item__content, .manga-item").each(
        (_idx, el) => {
          if (cards.length >= safeLimit) return;

          const link = $(el)
            .find(".post-title a, h3 a, .manga-title a")
            .first();
          ...
          cards.push({
            ...
            description: this.strip(
              $(el).find(".summary, .description, .excerpt").first().text(),
            ),
            coverUrl,
            status: null,
            year: null,
      ...
      console.error(
        `[ManhwaTop] discover failed: ${summarizeSourceError(error)}`,
      );
      return { trending: [], recentlyUpdated: [], newTitles: [] };
    }
  }
  ...
  async getMangaDetail(mangaId: string): Promise<MangaDetail | null> {
    ...
    try {
      ...
      if (!html || html.length < 100) {
        console.error(
          `[ManhwaTop] Detail page returned empty HTML for ${mangaId}`,
        );
        return null;
      }
      ...
      console.log(
        `[ManhwaTop] Detail parse: title=${titleElements}, desc=${descElements}, cover=${coverElements}`,
      );
      ...
      const detail: MangaDetail = {
        ...
        originalLanguage: null,
        tags: $(
          '.genres-content a, .genres a, a[href*="genre"], a[href*="tag"]',
        )
        ...
        latestChapter: null,
        author:
          this.strip($(".author-content a, .manga-author a").first().text()) ||
          null,
        artist:
          this.strip($(".artist-content a, .manga-artist a").first().text()) ||
          null,
        contentRating: null,
      ...
      console.error(
        `[ManhwaTop] detail failed: ${summarizeSourceError(error)}`,
      );
      return null;
    }
  }
  ...
  async getChapters(
    ...
    try {
      ...
      if (!mangaNodeId) {
        mangaNodeId =
          this.strip($("[data-manga-id]").attr("data-manga-id")) ||
          this.strip($("[data-post-id]").attr("data-post-id"));
      }

      if (!mangaNodeId) {
        console.warn(
          `[ManhwaTop] Could not extract mangaNodeId for ${mangaId}, falling back to default method`,
        );
      ...
      chapterDoc("li.wp-manga-chapter").each((index, el) => {
        ...
        const title =
          this.strip(a.find("p").first().text()) || this.strip(a.text());
        ...
        const dateText =
          li.find("a.c-new-tag").attr("title") ||
      ...
      console.error(
        `[ManhwaTop] chapter fetch failed: ${summarizeSourceError(error)}`,
      );
      return super.getChapters(mangaId, _translatedLanguage, limit);
    }
  }
