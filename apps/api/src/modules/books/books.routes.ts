...
export const bookRoutes = async (
  ...
  const legacySeriesParamSchema = z.object({
    seriesId: z.string().trim().min(1),
    slug: z.string().trim().min(1),
  });

  const getLegacySeriesCandidates = () => {
    const defaultSource = (process.env.MANGA_DEFAULT_SOURCE || '').trim().toLowerCase();
    const unique = new Set<string>([defaultSource, 'weebcentral', 'asura', 'mangadex']);
    return Array.from(unique).filter(Boolean);
  };

  const findLegacyDetail = async (rawSeriesId: string) => {
    for (const sourceId of getLegacySeriesCandidates()) {
      try {
        const detail = await mangaService.getMangaDetailBySource(sourceId, rawSeriesId);
  ...
  const findLegacyChapters = async (rawSeriesId: string, language?: string, limit?: number) => {
    const normalizedLanguage = language?.toLowerCase() === 'all' ? undefined : language;
    for (const sourceId of getLegacySeriesCandidates()) {
      try {
        const chapters = await mangaService.getChaptersBySource(sourceId, rawSeriesId, normalizedLanguage, limit ?? 200);
        if (chapters.length > 0) return chapters;
      } catch {
        // Try next candidate source.
      }
    }
    return [] as Awaited<ReturnType<MangaService['getChaptersBySource']>>;
  };

  const findLegacySimilar = async (rawSeriesId: string, limit?: number) => {
    for (const sourceId of getLegacySeriesCandidates()) {
      try {
        const similar = await mangaService.getSimilarMangaBySource(sourceId, rawSeriesId, limit ?? 6);
        if (similar.length > 0) return similar;
      } catch {
        // Try next candidate source.
      }
    }
    return [] as Awaited<ReturnType<MangaService['getSimilarMangaBySource']>>;
  };

  app.get('/manga/series/:seriesId/:slug', {
    config: { rateLimit: SCRAPE_RATE_LIMIT },
    schema: {
      params: legacySeriesParamSchema,
    },
    handler: async (request, reply) => {
      try {
        const { seriesId, slug } = request.params as z.infer<typeof legacySeriesParamSchema>;
        const rawSeriesId = `/series/${seriesId}/${slug}`;
        const data = await findLegacyDetail(rawSeriesId);
        if (!data) {
          return reply.status(404).send({ status: 'error', message: 'Manga not found' });
        }
        return reply.send({ status: 'success', data });
      } catch (error) {
        return reply.status(500).send({
          status: 'error',
          message: error instanceof Error ? error.message : 'Failed to fetch legacy manga detail',
        });
      }
    },
  });

  app.get('/manga/series/:seriesId/:slug/chapters', {
    config: { rateLimit: SCRAPE_RATE_LIMIT_HEAVY },
    schema: {
      params: legacySeriesParamSchema,
      querystring: mangaChaptersQuerySchema,
    ...
    handler: async (request, reply) => {
      try {
        ...
        const { language, limit } = request.query as z.infer<typeof mangaChaptersQuerySchema>;
        const rawSeriesId = `/series/${seriesId}/${slug}`;
        const data = await findLegacyChapters(rawSeriesId, language, limit);
        return reply.send({ status: 'success', data });
      } catch (error) {
        return reply.status(500).send({
          status: 'error',
          message: error instanceof Error ? error.message : 'Failed to fetch legacy chapters',
        });
      }
    },
  });

  app.get('/manga/series/:seriesId/:slug/similar', {
    config: { rateLimit: SCRAPE_RATE_LIMIT },
    schema: {
      params: legacySeriesParamSchema,
      querystring: mangaSimilarQuerySchema,
    ...
    handler: async (request, reply) => {
      try {
        ...
        const { limit } = request.query as z.infer<typeof mangaSimilarQuerySchema>;
        const rawSeriesId = `/series/${seriesId}/${slug}`;
        const data = await findLegacySimilar(rawSeriesId, limit);
        return reply.send({ status: 'success', data });
      } catch (error) {
        return reply.status(500).send({
          status: 'error',
          message: error instanceof Error ? error.message : 'Failed to fetch legacy similar manga',
        });
      }
    },
  });
