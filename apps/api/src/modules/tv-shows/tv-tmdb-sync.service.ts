...
export class TvTmdbSyncService {
  ...
  async syncCatalog(
    opts: { pagesPerList?: number; maxShows?: number } = {},
  ): Promise<{ scanned: number; upserted: number; failed: number }> {
    ...
    const maxShows =
      opts.maxShows ??
      this.parsePositiveInt(process.env.TV_TMDB_SYNC_MAX_SHOWS_PER_RUN, 300);
    const listPages =
      opts.pagesPerList ??
      this.parsePositiveInt(process.env.TV_TMDB_SYNC_PAGES_PER_LIST, 5);
  ...
  private async upsertShowByTmdbId(
    tmdbId: number,
    apiKey: string,
  ): Promise<void> {
    const detailsRes = await axios.get<TmdbShowDetails>(
    ...
    const genres = this.mapGenres(
      details.genres?.map((entry) => entry.id) ?? [],
    );
    ...
    for (const seasonRef of seasons) {
      if (
        !Number.isInteger(seasonRef.season_number) ||
        seasonRef.season_number < 1
      )
        continue;
      ...
      const season = await this.prisma.tvSeason.upsert({
        ...
        update: {
          ...
          posterUrl: this.tmdbImage(
            seasonDetails.poster_path ?? seasonRef.poster_path,
            "w500",
          ),
        },
        create: {
