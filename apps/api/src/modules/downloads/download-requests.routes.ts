...
export async function downloadRequestRoutes(fastify: FastifyInstance) {
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (!movieId && !showId) {
        return reply
          .status(400)
          .send({ success: false, error: "movieId or showId required" });
      }
      ...
      const req = await fastify.prisma.downloadRequest.upsert({
        where: movieId
          ...
          : { userId_showId: { userId, showId: showId! } },
      ...
      processDownloadRequest(fastify.prisma, req.id).catch((err) => {
        fastify.log.error(
          ...
          "[DownloadRequest] processing failed",
        );
      });
      ...
      return reply.status(202).send({
        ...
        message:
          "Request submitted. We'll notify you when the download is ready.",
      });
    },
  );
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      const req = await fastify.prisma.downloadRequest.findUnique({
        where: { id },
      });
