...
const TOP_NIGERIAN_MUSIC_SOURCES: Array<{
  ...
  {
    url: "https://www.youtube.com/@ChocolateCityMusic",
    artistName: "Chocolate City Music",
  },
];
...
export class YouTubeMusicService {
  ...
  async addChannel(
    ...
    const existing = await this.prisma.musicChannel.findUnique({
      where: { channelId },
    });
    ...
    try {
      ...
      do {
        ...
        const infoMap = new Map<
          string,
          { durationSeconds: number; description: string; publishedAt: string }
        >();
        ...
        for (const item of rawItems) {
          ...
          if (!info) continue;
          if (
            info.durationSeconds < MIN_MUSIC_SECONDS ||
            info.durationSeconds > MAX_MUSIC_SECONDS
          )
            continue;
      ...
      for (const vid of toImport) {
        try {
          ...
          const slugExists = await this.prisma.musicVideo.findUnique({
            where: { slug },
          });
          const finalSlug = slugExists
            ? `${slug}-${vid.youtubeId.slice(0, 6)}`
            : slug;
          ...
          progress.errors.push(
            `${vid.title}: ${err instanceof Error ? err.message : String(err)}`,
          );
        }
        progress.processed++;
      }
      ...
      const imported = await this.prisma.musicVideo.count({
        where: { channelId },
      });
    ...
    try {
      ...
      for (const ch of channels) {
        ...
        try {
          ...
          await this.runImport(ch.channelId, 10, pid, {
            maxImport: 10,
            maxScan: 100,
          });
  ...
  async bootstrapTopNigerianCatalog(): Promise<MusicBootstrapResult> {
    ...
    const configuredMinVideos = Number.parseInt(
      process.env.MUSIC_BOOTSTRAP_MIN_VIDEOS || "80",
      10,
    );
    const minVideos =
      Number.isFinite(configuredMinVideos) && configuredMinVideos > 0
        ? configuredMinVideos
        : 80;

    const configuredPerChannelImport = Number.parseInt(
      process.env.MUSIC_BOOTSTRAP_PER_CHANNEL_IMPORT || "20",
      10,
    );
    const perChannelImport =
      Number.isFinite(configuredPerChannelImport) &&
      configuredPerChannelImport > 0
    ...
    try {
      for (const source of TOP_NIGERIAN_MUSIC_SOURCES) {
        ...
        try {
          const created = await this.addChannel(
            source.url,
            source.artistName,
            source.region || MusicRegion.Nigeria,
          );
        ...
        this.runImport(channelId, 10, progressId, {
          maxImport: perChannelImport,
          maxScan: 120,
        }).catch((error) => {
          const progress = this.importProgress.get(progressId);
          if (progress) {
