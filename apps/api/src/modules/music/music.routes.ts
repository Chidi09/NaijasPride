...
export const musicRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  app.get('/playlist/:id', {
    ...
    if (!playlist) {
      return reply.status(404).send({
        success: false,
        error: { code: 'NOT_FOUND', message: 'Playlist not found or private' },
      });
    }
  ...
  app.get('/recommendations', {
    ...
    const recs = await service.getRecommendations(req.user.id, req.query.limit);
  ...
  app.post('/playlists/:id/items', {
    ...
    try {
      const item = await service.addToPlaylist(req.params.id, req.body.musicId, req.user.id);
      return reply.status(201).send({ success: true, data: item });
    } catch (err) {
      return reply.status(400).send({
        success: false,
        error: { code: 'BAD_REQUEST', message: err instanceof Error ? err.message : 'Unknown error' },
      });
    }
  });
};
