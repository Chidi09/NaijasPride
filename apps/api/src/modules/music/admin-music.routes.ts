...
export const adminMusicRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  app.post('/channels', {
    ...
    const channel = await service.addChannel(req.body.url, req.body.artistName, req.body.region);
  ...
  app.post('/channels/:channelId/import', {
    onRequest: [fastify.authenticate],
    preHandler: [requireAdmin],
    schema: {
    ...
    const progressId = await service.startImport(req.params.channelId, req.body.batchSize);
    return reply.send({ success: true, data: { progressId } });
  });

  // ── GET /api/v1/admin/music/import/:progressId ────────────────────────
  app.get('/import/:progressId', {
    onRequest: [fastify.authenticate],
    preHandler: [requireAdmin],
    schema: {
  ...
  app.post('/monitor', {
    ...
    service.monitorAll().catch((err) => console.error('[AdminMusicMonitor] Error:', err));
    return reply.send({ success: true, message: 'Monitor started in background' });
  });
  ...
  app.post('/videos', {
    ...
    const { youtubeId, title, artist, featuring, year, region, isOfficial, isExplicit } = req.body;
    const artistSlug = req.body.artistSlug ?? title.toLowerCase().replace(/[^a-z0-9]+/g, '-');
