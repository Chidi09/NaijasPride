...
import { subDays, startOfDay, endOfDay, startOfWeek, endOfWeek, startOfMonth, endOfMonth, startOfYear, endOfYear, format } from 'date-fns';
...
export class RevenueService {
  ...
  async getRevenueSummary() {
    ...
    return {
      ...
      total: ((totalSubscriptionRevenue._sum.amount || 0) / 100) + (totalAdRevenue._sum.revenue || 0)
    };
  }
  ...
  async getRevenueBreakdown(period: 'weekly' | 'monthly' | 'yearly') {
    ...
    const transactionData: any[] = await this.prisma.$queryRaw`
      SELECT 
    ...
    const adRevenueData: any[] = await this.prisma.$queryRaw`
      SELECT 
    ...
    // Merge the data
    const map = new Map<string, { period: string, subscription: number, ads: number }>();
   ...
   */
  async recordAdRevenue(date: Date, revenue: number, impressions: number = 0, clicks: number = 0) {
