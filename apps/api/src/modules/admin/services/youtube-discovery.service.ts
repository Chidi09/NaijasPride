...
export class YoutubeDiscoveryService {
  ...
  async runDiscoveryCycle() {
    ...
    try {
      ...
      console.log(`[Youtube Discovery] Found ${trendingVideos.length} trending videos`);
      
      for (const video of trendingVideos) {
        const v = video as any;
        if (v.channelId) {
          await this.channelService.registerDiscoveredChannel(v.channelId, v.channel, v.channel);
        }
      }
      ...
      console.log(`[Youtube Discovery] Found ${keywordResults.length} videos via keyword search`);
