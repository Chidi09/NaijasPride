...
import type { ProviderSource, ProviderSubtitles } from './anime-provider-manager';
...
function parseEpisodePage(html: string): GoGoEpisodeData | null {
  ...
  const sectionRegex = /data-type=["'](sub|dub)["'][^>]*>[\s\S]*?<ul>([\s\S]*?)<\/ul>/gi;
...
function parseServerButtons(html: string): GoGoServerButton[] {
  ...
  const buttonRegex = /<li[^>]*class="[^"]*player-type-link[^"]*"([^>]*)>([\s\S]*?)<\/li>/gi;
  ...
  while ((match = buttonRegex.exec(html)) !== null) {
    ...
    if (type && (enc1 || plainUrl)) {
      buttons.push({ type, label, encryptedUrl1: enc1, encryptedUrl2: enc2, encryptedUrl3: enc3, plainUrl, ref, subtitle, key });
    }
  }
...
function buildSourcesFromButtons(
  ...
  const hianimeButtons = buttons.filter(b => b.type === 'hianime' && b.encryptedUrl1);
  ...
  const bloggerButtons = buttons.filter(b => b.type === 'Blogger' && b.encryptedUrl1);
...
export async function searchGoGoAnimeBy(query: string): Promise<GoGoSearchResult[]> {
  ...
  const linkRegex = /href=["'](https?:\/\/gogoanime\.by\/([^"'\/]+)\/)["'][^>]*>([^<]+)/gi;
  ...
  while ((m = linkRegex.exec(html)) !== null) {
    ...
    if (['category', 'tag', 'page', 'author', 'wp-content'].includes(slug)) continue;
...
export async function getGoGoAnimeByEpisodes(animeSlug: string): Promise<GoGoEpisodeListItem[]> {
...
export async function getGoGoAnimeByWatchSources(
  ...
  console.log(`[GoGoAnime.by] Fetching sources from ${episodeUrl} (${audioType})`);
  ...
  const effectiveButtons = buttons.length > 0
    ? buttons
    : (audioType === 'dub' ? episodeData.sub : episodeData.dub);
...
export async function resolveGoGoAnimeByEpisode(
  ...
  for (const result of results.slice(0, 3)) {
    ...
    const { sources, subtitles } = await getGoGoAnimeByWatchSources(episodeUrl, audioType);
...
export async function checkGoGoAnimeByHealth(): Promise<{ healthy: boolean; message: string }> {
  try {
    const html = await fetchWithFlareSolverr(GOGOANIME_BASE);
    if (!html) return { healthy: false, message: 'FlareSolverr request failed' };
