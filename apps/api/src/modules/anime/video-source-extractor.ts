...
export async function extractVideoSources(embedUrl: string): Promise<VideoSource[]> {
  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      const playButton = document.querySelector('.play-button, [data-play], .vjs-play-control, .plyr__control--overlaid');
    ...
    const pageSources = await page.evaluate(() => {
          ...
          playlist.forEach((item: any) => {
            if (item.sources) {
              item.sources.forEach((src: any) => {
                if (src.file) found.push({url: src.file, quality: src.label});
              });
            }
          });
        }
      ...
      if (plyrData) {
        ...
        sources.forEach((src) => {
          if (src.src) found.push({url: src.src, quality: src.getAttribute('size') || undefined});
        });
      }
      ...
      if ((window as any).videojs) {
        ...
        players.forEach((player) => {
          ...
          if (id) {
            const vjsPlayer = (window as any).videojs(id);
      ...
      // Look for hls.js
      if ((window as any).Hls && (window as any).hls) {
