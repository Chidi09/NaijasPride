...
import {
  resolveGoGoAnimeByEpisode,
  checkGoGoAnimeByHealth,
} from "./gogoanime-by-provider";
...
export async function getSourcesMultiProvider(
  ...
  const providersToTry = preferredProvider
    ? [
        preferredProvider,
        ...DEFAULT_PROVIDER_ORDER.filter((p) => p !== preferredProvider),
      ]
  ...
  for (const provider of providersToTry) {
    try {
      const result = await getSourcesFromProvider(
        primaryQuery,
        episodeNumber,
        provider,
        type,
...
async function getSourcesFromProvider(
  ...
  switch (provider) {
    case "aniwatch": {
      ...
      const { sources, subtitles } = await getAniWatchSources(
        awEpisode.id,
        type,
      );
      return {
    ...
    case "nineanime": {
      ...
      const { sources, subtitles } = await getNineAnimeSources(
        naEpisode.id,
        naSearch[0]!.id,
      );
      return {
...
export async function getProvidersHealth(): Promise<
  Record<ProviderType, { healthy: boolean; message: string }>
> {
  const health: Record<ProviderType, { healthy: boolean; message: string }> = {
    aniwatch: {
      healthy: false,
      message: "Shut down (rebranded to HiAnime, now offline)",
    },
    ...
    embed: {
      healthy: isEmbedProviderAvailable(),
      message: isEmbedProviderAvailable()
        ? "TMDB key configured"
        : "No TMDB key",
    },
  };
