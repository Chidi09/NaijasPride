...
const EMBED_RESOLVE_TIMEOUT_MS = Number(
  process.env.ANIME_EMBED_RESOLVE_TIMEOUT_MS || 20000,
);
const EMBED_RESOLVE_WAIT_MS = Number(
  process.env.ANIME_EMBED_RESOLVE_WAIT_MS || 5000,
);
...
export const extractMediaCandidatesFromText = (
  text: string,
): MediaCandidate[] => {
...
const sniffMediaFromEmbedPage = async (
  embedUrl: string,
  timeoutMs: number,
  ...
  try {
    ...
    await page
      .waitForLoadState("networkidle", {
        timeout: Math.max(2000, Math.floor(timeoutMs * 0.5)),
      })
      .catch(() => undefined);
...
export const resolveDirectMediaFromEmbed = async (
  embedUrl: string,
): Promise<MediaCandidate | null> => {
