...
type AnimepaheLinksEntry = Record<
  string,
  { kwik?: string; audio?: string; filesize?: number }
>;
...
const ANIMEPAHE_SITE_ORIGIN =
  process.env.ANIMEPAHE_SITE_ORIGIN || "https://animepahe.si";
const ANIMEPAHE_API_URL =
  process.env.ANIMEPAHE_API_URL || `${ANIMEPAHE_SITE_ORIGIN}/api?m=`;
const ANIMEPAHE_TIMEOUT_MS = Number(process.env.ANIMEPAHE_TIMEOUT_MS || 15000);
const ANIMEPAHE_BROWSER_WAIT_MS = Number(
  process.env.ANIMEPAHE_BROWSER_WAIT_MS || 5000,
);
const ANIMEPAHE_BROWSER_REFRESH_MS = Number(
  process.env.ANIMEPAHE_BROWSER_REFRESH_MS || 10 * 60 * 1000,
);
...
const ensureBrowserHandle = async (): Promise<AnimepaheBrowserHandle> => {
  if (
    browserHandle &&
    Date.now() - browserHandleCreatedAt > ANIMEPAHE_BROWSER_REFRESH_MS
  ) {
...
const browserBackedFetchText = async (
  url: string,
): Promise<{
  ok: boolean;
  status: number;
...
export const computeAnimepaheReleasePage = (
  episode: number,
  pages: number,
...
const animepaheRequest = async <T>(url: string, json = true): Promise<T> => {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Animepahe request failed with status ${response.status}`,
      );
    }
    const text = await response.text();
    if (!json) {
      if (isDdosGuardChallenge(text)) {
        ...
        if (!browserResponse.ok) {
          throw new Error(
            `Animepahe browser request failed with status ${browserResponse.status}`,
          );
        }
...
const animepaheSearch = async (
  query: string,
): Promise<AnimepaheSearchEntry[]> => {
  const encoded = encodeURIComponent(query);
  const payload = await animepaheRequest<AnimepaheSearchResponse>(
    `${ANIMEPAHE_API_URL}search&q=${encoded}`,
  );
  return payload.data || [];
};

const pickBestAnimepaheMatch = (
  entries: AnimepaheSearchEntry[],
  titles: string[],
): AnimepaheSearchEntry | null => {
    ...
    .map((entry) => {
      ...
      if (
        normalizedTitles.some(
          (title) => entryTitle.includes(title) || title.includes(entryTitle),
        )
      )
        score += 40;
...
const animepaheReleasePage = async (
  animeSession: string,
  page?: number,
): Promise<AnimepaheReleaseResponse> => {
  const query = new URLSearchParams({ id: animeSession, sort: "episode_asc" });
  if (page && page > 1) {
    query.set("page", String(page));
  }
  return animepaheRequest<AnimepaheReleaseResponse>(
    `${ANIMEPAHE_API_URL}release&${query.toString()}`,
  );
};

const findAnimepaheEpisodeRelease = async (
  animeSession: string,
  episodeNumber: number,
): Promise<AnimepaheReleaseEntry | null> => {
  const firstPage = await animepaheReleasePage(animeSession);
  const total = Number(firstPage.total || 0);
  const lastPage = Number(firstPage.last_page || 1);

  const fromFirst = (firstPage.data || []).find(
  ...
  const targetPage = computeAnimepaheReleasePage(
    episodeNumber,
    lastPage,
    total || episodeNumber,
  );
  ...
  return (
    (pageData.data || []).find((entry) => entry.episode === episodeNumber) ||
    null
  );
};
...
export const extractAnimepaheM3u8FromHtml = (html: string): string | null => {
  ...
  const plyrScript = scriptBlocks.find(
    (script) => script.includes("Plyr") && script.includes(".split"),
  );
...
const animepaheLinks = async (
  releaseSession: string,
): Promise<AnimepaheLinksEntry[]> => {
  const query = new URLSearchParams({ id: releaseSession, p: "kwik" });
  const payload = await animepaheRequest<AnimepaheLinksResponse>(
    `${ANIMEPAHE_API_URL}links&${query.toString()}`,
  );
  return payload.data || [];
};
...
const fetchKwikHtml = async (kwikUrl: string): Promise<string> => {
  ...
  try {
    ...
    if (!response.ok)
      throw new Error(`Kwik page fetch failed with status ${response.status}`);
...
export async function resolveAnimepaheWatchByTitles(
  ...
  const release = await findAnimepaheEpisodeRelease(
    match.session,
    episodeNumber,
  );
