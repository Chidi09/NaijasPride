...
export async function scrapeWithScrapingBee(
  ...
  try {
    ...
    console.log(
      `[ScrapingBee] Scraping ${targetUrl} with wait_for=${waitForSelector}`,
    );
    ...
    if (data.xhr && Array.isArray(data.xhr)) {
      for (const xhr of data.xhr) {
        if (
          xhr.url &&
          (xhr.url.includes(".m3u8") || xhr.url.includes(".mp4"))
        ) {
          xhrSources.push({
            url: xhr.url,
...
export async function scrapeWithScrapingAnt(
  ...
  try {
    ...
    const response = await fetch(
      `${SCRAPINGANT_URL}?${queryParams.toString()}`,
    );
 ...
 */
export async function checkScrapingBeeHealth(): Promise<{
  healthy: boolean;
  message: string;
  credits?: number;
}> {
  try {
    const response = await fetch(
      `https://app.scrapingbee.com/api/v1/usage?api_key=${SCRAPINGBEE_API_KEY}`,
    );
