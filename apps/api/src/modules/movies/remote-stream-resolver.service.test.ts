...
test("normalizeAllowedHosts trims, lowercases, and deduplicates", () => {
  const normalized = normalizeAllowedHosts([
    "  MixDrop.CO  ",
    "hydrax.net",
    "mixdrop.co",
    "",
...
test("isCandidateResponse detects HLS and MP4 media responses", () => {
  ...
  assert.equal(
    isCandidateResponse("https://host/video/file.mp4", 200, "video/mp4"),
    true,
  );
  assert.equal(
    isCandidateResponse(
  ...
  assert.equal(
    isCandidateResponse("https://doubleclick.net/track", 200, "video/mp4"),
    false,
  );
});
