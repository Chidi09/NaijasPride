...
export const movieRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  const movieSignedUrlTtlSecondsRaw = Number.parseInt(
    process.env.MOVIE_DOWNLOAD_URL_TTL_SECONDS || "21600",
    10,
  );
  const movieSignedUrlTtlSeconds =
    Number.isFinite(movieSignedUrlTtlSecondsRaw) &&
    movieSignedUrlTtlSecondsRaw >= 3600
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        fastify.log.warn(
          { error, key },
          "[Movies] Download gateway failed, falling back to signed URL redirect",
        );
        const url = await storageService.getDownloadUrl(key, {
          expiresInSeconds: movieSignedUrlTtlSeconds,
        });
  ...
  app.get("/stream/:movieId/*", async (request, reply) => {
    ...
    if (!movieId || !tail || tail.includes("..") || tail.startsWith("/")) {
      throw new BadRequestError("Invalid stream path");
    }
    ...
    try {
      ...
      throw new NotFoundError("Stream segment");
    }
  });
    ...
    async (request, reply) => {
      ...
      if (!hasCover) {
        try {
          ...
          fastify.log.warn(
            { metaError, movieId: movie.id },
            "[MovieCreate] TMDB auto-sync failed, continuing without poster",
          );
        }
      }
      ...
      if (!movieFileIngestEnabled) {
        return reply.status(410).send({
          ...
          error: {
            ...
            message:
              "Movie file ingest to R2 is disabled. Use embed/YouTube sources instead.",
          },
        });
      }
        ...
        return reply.status(422).send({
          ...
          message:
            error instanceof Error
              ? error.message
              : "Failed to resolve playable stream URL",
        });
      }
    },
  );
  ...
  app.patch(
    ...
    async (request, reply) => {
      ...
      const movie = await service.updateStatus(
        id,
        status,
        toQualityEnum(quality),
      );
      ...
      if (!result.success) {
        ...
        return reply.status(statusCode).send({
          ...
          error: {
            ...
            message: result.message || "Unable to sync metadata",
          },
        });
      }
