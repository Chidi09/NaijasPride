...
const MEDIA_HOST_BLOCKLIST = [
  /doubleclick\./i,
  /googlesyndication\./i,
  /googlevideo\.com\/(?:generate_204|api\/stats)/i,
];
...
const isBlockedHost = (url: string): boolean =>
  MEDIA_HOST_BLOCKLIST.some((pattern) => pattern.test(url));

export const isCandidateResponse = (
  url: string,
  status: number,
  contentType: string,
): boolean => {
...
export class RemoteStreamResolverService {
    ...
    return retryWithBackoff(
      ...
      },
      {
        ...
        onRetry: (attempt, error, delay) => {
          console.warn(
            `[RemoteStream] Browser launch retry ${attempt} after ${delay}ms: ${error.message}`,
          );
        },
      },
    );
  ...
  async resolveFromPage(
    pageUrl: string,
    options: ResolverOptions = {},
  ): Promise<ResolveStreamResult> {
    if (this.isInCooldown()) {
      throw new Error(
        "Stream resolver is in cooldown due to consecutive failures. Please try again later.",
      );
    }
    ...
    const captureWindowMs =
      Number.isFinite(options.captureWindowMs) &&
      (options.captureWindowMs as number) > 0
    ...
    const maxIframeHops =
      provider === "soap2day"
        ? parseInt(
            process.env.SOAP2DAY_MAX_IFRAME_HOPS ||
              `${SOAP2DAY_MAX_IFRAME_HOPS}`,
            10,
          )
        : 0;

    const providerHosts =
      provider === "soap2day" ? this.soap2dayAllowedMirrors : [];
    ...
    try {
      ...
      const clickPlaybackSelectors = async (
        selectors: string[],
      ...
      const collectSoap2daySourceUrls = async (): Promise<string[]> => {
        const urls = await page.evaluate(() => {
          ...
          document
            .querySelectorAll("#serverSelect option")
            .forEach((option) => {
      ...
      const playSelectors =
        provider === "soap2day" ? SOAP2DAY_PLAY_SELECTORS : PLAY_SELECTORS;
      ...
      if (!winner && provider === "soap2day") {
        ...
        for (const sourceUrl of iframeSources.slice(
          0,
          SOAP2DAY_MAX_IFRAME_HOPS,
        )) {
      ...
      const hostAllowed =
        !allowedHosts.length ||
        allowedHosts.includes(winner.host.toLowerCase());
