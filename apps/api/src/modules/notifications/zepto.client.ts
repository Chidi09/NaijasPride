...
export class ZeptoMailClient {
  ...
  // SMTP Configuration
  private static readonly SMTP_HOST =
    process.env.SMTP_HOST || "smtp.zeptomail.com";
  private static readonly SMTP_PORT = parseInt(
  ...
  private static readonly SMTP_FROM =
    process.env.SMTP_FROM || "noreply@naijaspride.com";
  ...
  private static async sendViaAPI(payload: EmailPayload): Promise<boolean> {
    ...
    try {
      ...
      if (err.response) {
        console.error("ZeptoMail response:", err.response.data);
      }
      return false;
    }
  }
