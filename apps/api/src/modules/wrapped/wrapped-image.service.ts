...
export class WrappedImageService {
  ...
  private loadTemplates() {
    const baseSource = fs.readFileSync(
      path.join(TEMPLATES_DIR, "base.hbs"),
      "utf-8",
    );
  ...
  private transformStatsToTemplateData(stats: WrappedStats): TemplateData {
    return {
      ...
      musicHours: stats.topSong?.minutes
        ? Math.round(stats.topSong.minutes / 60)
        : 0,
      bookHours: stats.topBook?.minutes
        ? Math.round(stats.topBook.minutes / 60)
        : 0,
