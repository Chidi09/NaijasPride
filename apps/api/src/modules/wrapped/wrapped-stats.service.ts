...
function parsePeriod(period: string): { start: Date; end: Date; label: string; isAnnual: boolean } {
  ...
  const label = start.toLocaleString('en-NG', { month: 'long', year: 'numeric', timeZone: 'UTC' });
  return { start, end, label, isAnnual: false };
}

function computeStreaks(dates: Date[]): StreakInfo {
  if (dates.length === 0) return { longestStreak: 0, currentStreak: 0, totalActiveDays: 0 };
  ...
  const daysSinceLast = (new Date(today).getTime() - new Date(lastDay).getTime()) / 86_400_000;
...
export class WrappedStatsService {
  ...
  async computeForUser(userId: string, period: string): Promise<WrappedStats> {
    ...
    for (const g of movieStats.genres) genreMap.set(g.name, (genreMap.get(g.name) ?? 0) + g.count * 2); // weight movies higher
    for (const g of musicStats.genres) genreMap.set(g.name, (genreMap.get(g.name) ?? 0) + g.count);
    for (const g of bookStats.genres)  genreMap.set(g.name, (genreMap.get(g.name) ?? 0) + g.count);

    const totalGenreCount = Array.from(genreMap.values()).reduce((a, b) => a + b, 0) || 1;
  ...
  private async getMovieStats(userId: string, start: Date, end: Date) {
    const history = await this.prisma.watchHistory.findMany({
      ...
      include: {
        movie: {
          select: {
            id: true, title: true, thumbnailUrl: true, posterUrl: true,
            genre: true, durationMinutes: true,
          },
        },
      },
    });
    ...
    for (const h of history) {
      ...
      const watchedFraction = h.duration > 0 ? Math.min(h.progress / h.duration, 1) : 0.5;
    ...
    return {
      ...
      topMovie: {
        ...
        count: 1,
        minutes: Math.round((top.movie.durationMinutes ?? 90) * Math.min(top.progress / Math.max(top.duration, 1), 1)),
      } as TopItem,
      genres: Array.from(genreMap.entries()).map(([name, count]) => ({ name, count, percentage: 0 })),
    };
  }
  ...
  private async getMusicStats(userId: string, start: Date, end: Date) {
      ...
      include: {
        music: {
          select: {
            ...
            thumbnailUrl: true, hdThumbnailUrl: true, durationSeconds: true,
          },
        },
      },
    });
    ...
    if (history.length === 0) {
      return { totalPlays: 0, totalMinutes: 0, topSong: null, topArtist: null, genres: [] };
    }
    ...
    // Top song
    const topSongEntry = [...history].sort((a, b) => b.playCount - a.playCount)[0];

    // Top artist (aggregate across songs)
    const artistMap = new Map<string, { plays: number; imageUrl: string | null }>();
    for (const h of history) {
      const existing = artistMap.get(h.music.artist);
      artistMap.set(h.music.artist, {
        plays: (existing?.plays ?? 0) + h.playCount,
        imageUrl: existing?.imageUrl ?? h.music.hdThumbnailUrl ?? h.music.thumbnailUrl,
      });
    }
    const topArtistEntry = Array.from(artistMap.entries()).sort((a, b) => b[1].plays - a[1].plays)[0];
    ...
    return {
      ...
      topSong: {
        ...
        imageUrl: topSongEntry.music.hdThumbnailUrl ?? topSongEntry.music.thumbnailUrl,
        count: topSongEntry.playCount,
        minutes: Math.round(((topSongEntry.music.durationSeconds ?? 210) / 60) * topSongEntry.playCount),
      } as TopItem,
  ...
  private async getMangaStats(userId: string, start: Date, end: Date) {
    ...
    const seriesMap = new Map<string, { chapters: number; lastChapterId: string }>();
    for (const p of progress) {
    ...
    const topMangaId = Array.from(seriesMap.entries()).sort((a, b) => b[1].chapters - a[1].chapters)[0]?.[0];

    let topSeries: TopItem | null = null;
    if (topMangaId) {
      const fav = await this.prisma.mangaFavorite.findFirst({ where: { userId, mangaId: topMangaId } });
      // Also check offline manga for title if not favorited
      const offlineEntry = !fav ? await this.prisma.offlineMangaChapter.findFirst({ where: { userId, mangaId: topMangaId } }) : null;
  ...
  private async getHighlightCount(userId: string, start: Date, end: Date): Promise<number> {
    return this.prisma.bookHighlight.count({
      where: { userId, createdAt: { gte: start, lte: end } },
    });
  }

  private async getDownloadCount(userId: string, start: Date, end: Date): Promise<number> {
    return this.prisma.download.count({
      where: { userId, timestamp: { gte: start, lte: end } },
    });
  }

  private async getAllActivityDates(userId: string, start: Date, end: Date): Promise<Date[]> {
    const [movieDates, musicDates, bookDates, mangaDates] = await Promise.all([
      ...
      this.prisma.musicWatchHistory.findMany({
        where: { userId, lastPlayedAt: { gte: start, lte: end } },
      ...
      this.prisma.mangaReadingProgress.findMany({
        where: { userId, lastReadAt: { gte: start, lte: end } },
