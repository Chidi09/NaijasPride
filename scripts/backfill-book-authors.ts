...
const isUnknown = (author: string | null) =>
  !author || ['unknown', 'various', 'n/a', ''].includes(author.toLowerCase().trim());

// ── Google Books ──────────────────────────────────────────────────────────────

async function fetchGoogleBooksAuthor(title: string, year?: number | null): Promise<string | null> {
  try {
    const q = `intitle:${encodeURIComponent(seriesTitle(title))}`;
    const { data } = await axios.get<{ items?: Array<{ volumeInfo: { title: string; authors?: string[]; publishedDate?: string } }> }>(
      `${GOOGLE_BOOKS_BASE}?q=${q}&maxResults=5`,
      { timeout: 10_000 },
    );
    ...
    for (const item of data.items) {
      ...
      else if (norm.includes(searchNorm) || searchNorm.includes(norm)) score += 50;
...
async function fetchOpenLibraryAuthor(title: string): Promise<string | null> {
  try {
    const { data } = await axios.get<{ docs?: Array<{ author_name?: string[] }> }>(
      OPEN_LIBRARY_BASE,
      {
        params: { title: seriesTitle(title), limit: 3, fields: 'author_name,title' },
        timeout: 10_000,
      },
    );
...
async function main() {
  ...
  console.log(`Found ${books.length} books with unknown/empty author (dry-run: ${DRY_RUN})\n`);
  ...
  for (const book of books) {
    ...
    if (author && !isUnknown(author)) {
      ...
      if (!DRY_RUN) {
        try {
          await prisma.book.update({ where: { id: book.id }, data: { author } });
          updated++;
        } catch (e) {
