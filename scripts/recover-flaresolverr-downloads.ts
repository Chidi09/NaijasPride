...
const main = async () => {
  ...
  const allFiles = fs.readdirSync(DOWNLOADS_DIR).filter((f) =>
    /\.(epub|pdf)$/i.test(f) && !f.includes('.crdownload'),
  );
  ...
  console.log(`[Recover] ${allFiles.length} files found, ${byNorm.size} unique titles after dedup`);
  ...
  for (const [norm, { file }] of byNorm) {
    ...
    if (!book) {
      ...
      if (!partialMatch) {
        ...
        if (!DRY_RUN) {
          ...
          try {
            ...
            console.error(`[Recover]   Upload failed: ${err instanceof Error ? err.message : String(err)}`);
            errors++;
          }
        }
        continue;
      ...
      console.log(`[Recover] PARTIAL MATCH: "${file}" → "${b.title}" (${storageKey})`);
  ...
  if (unmatchedFiles.length) {
    console.log('');
    console.log('Unmatched files (uploaded to books/recovered/ for manual review):');
